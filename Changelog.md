...
- Add the ``overview`` plot type (``eos.figure.OverviewPlot``), which compares predictions and EOS constraints for a set of user-defined observables, one row per observable and each with its own LaTeX label, optionally normalized to a chosen source; Carolina plot (C. Bolognani)

...
- **Breaking:** Rethrow the first exception raised while evaluating an observable in ``ObservableCache::update()``, on the calling thread and only after every pending computation has been waited on, in lieu of logging it and writing a ``NaN`` into the affected prediction; every evaluation path now catches exceptions of any type, not only ``eos::Exception``, so that an exception escaping a worker thread no longer terminates the process. Code that relied on ``update()`` never throwing must now handle the exception, and a single unevaluable point of a large grid surfaces as an error in lieu of a ``NaN`` log-posterior that a sampler steps straight past (D. van Dyk)
...
- Add the class ``eos::ResolutionConvolution``, which convolves a signal sampled on a uniform tensor-product grid of rank 1 to 4 with a fixed resolution kernel, through the discrete Fourier transform; the kernel is supplied in centred order and renormalised to unit sum, and the convolution is circular, so that the grid must be padded with a region in which both the signal and the resolution are negligible (D. van Dyk)
- Add the class ``eos::DetectorLevelPDF`` and its Python counterpart ``eos.DetectorLevelPDF``: a truth-level ``SignalPDF`` of rank 1 to 4 convolved with a detector resolution, which is itself a ``SignalPDF`` and can therefore be evaluated, sampled, and plotted like any other PDF. The truth PDF is sampled on the grid through a single ``ObservableCache`` batch, and the resolution is supplied either as a ``SignalPDF`` over the per-axis offset variables or as a pre-computed kernel grid (D. van Dyk)
- Add batch computations to ``ObservableCache``: ``add_batch()`` registers a group of observables that are evaluated into one contiguous, pre-allocated block of memory, partitioned across the available threads, and ``operator[](BatchId)`` retrieves their predictions as a ``std::span`` (D. van Dyk)
- Add the unbinned log-likelihood blocks ``LogLikelihoodBlock::Unbinned1D`` through ``Unbinned4D``, and export them to Python as ``eos.LogLikelihoodBlock.Unbinned1D`` through ``Unbinned4D``. Each takes an ``eos::DetectorLevelPDF`` whose rank matches, together with the individually observed events, and sums the logarithm of the normalized detector-level density over them; the grid is convolved once per evaluation and interpolated at every event (D. van Dyk)
- Add the ``eos.data.UnbinnedLikelihood`` data object, a directory holding the individually observed events, the detector resolution, and the description of the native sampling grid that the resolution supports; the rank of the problem is detected from the number of axes rather than declared (D. van Dyk)
- Add the ``type: unbinned`` analysis-file likelihood component, which builds an unbinned likelihood from an ``eos.data.UnbinnedLikelihood`` directory and the qualified name of the truth-level signal PDF fitted to it, and adds it to the posterior. Its optional ``axes`` key crops the data object's native grid to a window without changing the node spacing; because the underlying real discrete Fourier transform requires an even number of points per axis, a crop that yields an odd count is reported as a diagnostic of severity ``ERROR`` and raises a ``ValueError`` in lieu of being silently adjusted (D. van Dyk)
- Add a ``type`` key to the analysis-file likelihood components. It is presently required only for ``type: 'unbinned'``; where it is absent, the kind of a component is inferred as before from the presence of the ``constraints``, ``manual_constraints``, or ``pyhf`` key (D. van Dyk)
- Add a user-guide notebook on detector-level PDFs, which builds the truth-level 2D PDF of ``B->Dlnu`` in ``q2`` and ``cos(theta_l)``, convolves it with a Gaussian ``q2`` resolution assembled from EOS expressions, validates the result against manually smeared samples, and fits the form factor shape parameters to the smeared events through an unbinned likelihood in an analysis file (D. van Dyk)
- Export ``SignalPDF.evaluate_linear`` to Python, which returns the unnormalized density on the linear scale (D. van Dyk)
...
- Let the relative tolerance govern the numerical integrations of the ``B->Plnu`` observables. Their integrands span some sixteen orders of magnitude, and the absolute tolerance of ``1e-9`` exceeded all but a handful of the integrals, so that the very first cubature rule satisfied the convergence test and the integration domain was never subdivided; the results were accidentally accurate over the physical support alone and could be off by several percent over any wider domain. The integrated leptonic forward-backward asymmetry of ``B->Dlnu`` changes by 3.3%, and the isospin-specific ``R_pi`` ratios by 0.08% (D. van Dyk)

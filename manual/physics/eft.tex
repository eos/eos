...
\\
  & + \frac{4 G_F}{\sqrt{2}} \lambda_{t}^{(s)} \left[
    ...
    + \sum_{i=3}^{8} \wilson{i} \op{i}
    + \sum_{i=3}^{6}  \wilson{iQ} \op{iQ}
    + \wilson{b}\op{b} 
    + \sum_{\ell = e,\mu,\tau} \sum_{i=9}^{10}  \wilson[\ell]{i} \op{i}^\ell    
    \right] + \mbox{h.c.} \,.
\end{aligned}
\end{equation}
...
\begin{align}
  \op{1}^U & 
  = (\bar{s} \gamma_\mu P_L T^a U) (\bar{U} \gamma^\mu P_L T^a b) \,, &
  \op{2}^U & 
  = (\bar{s} \gamma_\mu P_L U) (\bar{U} \gamma^\mu P_L b) \,, 
\end{align}
...
\end{aligned}
\end{equation}
where $Q_q$ denotes the quark charges as multiples of $e$, i.e. $Q_u = +2/3$
and $Q_d = -1/3$. Further, an additional operator has to be considered
\begin{align}
...
\begin{align}
  \tildeop{7} & 
  = \frac{e}{g_s^2} \overline{m}_b [\bar{s} \sigma^{\mu \nu} P_R b] F_{\mu \nu} \,, &
  \tildeop{8} & 
  = \frac{1}{g_s}   \overline{m}_b [\bar{s} \sigma^{\mu \nu} P_R T^a b] G_{\mu \nu}^a \,,
\end{align}
receive contributions from penguin-type diagrams with on-shell photons and gluons
respectively. The appearing $b$-quark mass is renormalized in the $\overline{\mbox{MS}}$ 
scheme. 
...
(It should be kept in mind that at higher orders the definition of evanescent
operators implicitly enters the results of the initial Wilson coefficients and
anomalous dimensions. Such scheme dependences will cancel however in the final
observable.) 

For the tilde operators the normalization to $(4\pi/g_s)^2$, the QCD coupling, 
has been chosen for practical reasons such that the leading SM 1-loop correction
to the initial Wilson coefficients counts formally as a strong correction rather
then an electro-magnetic one. The initial Wilson coefficients are known up to
NNLO in QCD and NLO in EW corrections
\begin{align}
  \wilson{i}(\mu_0) = \dots
\end{align}
...
The full basis of semi-leptonic operators is spanned by $\op{9,10}^\ell$ as above, and
additionally
\begin{align}
  ...
  \op{T5}^{\ell}  & = \frac{e^2}{(4\pi)^2} (\bar{s} \sigma_{\mu\nu} b)
                      (\bar{\ell} \sigma^{\mu\nu} \gamma_5 \ell) \,.
\end{align}
Further there are chirality-flipped dipole operators
\begin{align}
  \op{7'} & 
  = \frac{e}{(4\pi)^2}   \overline{m}_b [\bar{s} \sigma^{\mu \nu} P_L b] F_{\mu \nu} \,, &
  \op{8'} & 
  = \frac{g_s}{(4\pi)^2} \overline{m}_b [\bar{s} \sigma^{\mu \nu} P_L T^a b] G_{\mu \nu}^a \,.
\end{align}
...
The coefficients $\wilson[(U)]{j}$ with $j\in\lbrace 1,2\rbrace$ are presently

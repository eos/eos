...
The ``masks`` key contains a list of *named* masks. Each mask contains two mandatory keys:
  ...
  - ``logical_combination`` (**mandatory** if more than one observable used in the mask) --- The logical combination to take of the expressions in the mask. The options are ``and`` and ``or``.

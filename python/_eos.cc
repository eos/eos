...
BOOST_PYTHON_MODULE(_eos)
{
    ...
    class_<ExpressionReferences>("ExpressionReferences", R"(
            Represents the observables and parameters that an observable expression refers to.

            Objects of this class cannot be created directly; they are returned by
            :func:`analyze_expression <eos.analyze_expression>`.
        )",
                                 no_init)
            .add_property("observables", make_getter(&ExpressionReferences::observables, return_value_policy<return_by_value>()),
                          R"(
            The names of the observables that the expression refers to.

            :rtype: list of eos.QualifiedName
        )")
            .add_property("parameters", make_getter(&ExpressionReferences::parameters, return_value_policy<return_by_value>()), R"(
            The names of the parameters that the expression refers to.

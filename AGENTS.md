...
Do not invoke `./<name>_TEST` directly. C++ test binaries are libtool wrapper scripts
whose rpath includes the configured installation prefix (`--prefix`/`$PREFIX` at
`configure` time); if a prior `make install` populated that prefix, it can **shadow**
the freshly built in-tree `.libs/*.so`, so running the binary directly can silently
load the old installed library and source edits appear to have no effect. If a direct
run is unavoidable:

```
LD_LIBRARY_PATH="$(pwd)/eos/<sub>/.libs:$LD_LIBRARY_PATH" ./eos/<sub>/.libs/<name>_TEST
```

Confirm with `ldd`. (Clean/CI builds do not have this shadowing.)
...
version is pinned in `.pre-commit-config.yaml` (currently `v19.1.2`); the `clang-format`
...
## Ancillary files for issues and PRs

Scratch and working material produced while addressing a tracked item goes into a
per-item directory at the repository root, named by its number alone (no `#`):

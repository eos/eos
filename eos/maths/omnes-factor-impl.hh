...
namespace eos
{
        ...
        _scattering_phase(scattering_phase)
    {
        ...
        if ((_sys == nullptr) || (_sys2 == nullptr) || (_V == nullptr) || (_S == nullptr) || (_work == nullptr) || (_x == nullptr) || (_b == nullptr))
        {
            throw std::bad_alloc();
        }
    ...
    // Deallocate memory
    template <unsigned order_, unsigned nints_> OmnesFactor<order_, nints_>::~OmnesFactor()
    {
        if (_sys)
        {
            gsl_matrix_free(_sys);
        }
        _sys = nullptr;

        if (_sys2)
        {
            gsl_matrix_free(_sys2);
        }
        _sys2 = nullptr;
    ...
    OmnesFactor<order_, nints_>::solve_sys(const double & bc_pos)
    {
    ...
    OmnesFactor<order_, nints_>::lq_sum(const double & z) const
    {
    ...
    template <unsigned order_, unsigned nints_>
    std::array<double, order_>
    OmnesFactor<order_, nints_>::p_ab(const double & z, const double & a, const double & b) const
    {
    ...
    template <unsigned order_, unsigned nints_>
    std::array<double, order_>
    OmnesFactor<order_, nints_>::p_inf(const double & z, const double & a) const
    {
        std::array<double, order_> ret_vec{};
        if (std::abs(z) > 1e-10)
        {
    ...
    template <unsigned order_, unsigned nints_>
    std::array<double, order_>
    OmnesFactor<order_, nints_>::rr_ab(const double & z, const double & a, const double & b) const
    {
        std::array<double, order_>       ret_vec{};
    ...
    template <unsigned order_, unsigned nints_>
    std::array<double, order_>
    OmnesFactor<order_, nints_>::rr_inf(const double & z, const double & a) const
    {
        std::array<double, order_>       ret_vec{};
    ...
    template <unsigned order_, unsigned nints_>
    double
    OmnesFactor<order_, nints_>::omnes_abs(const double & s) const
    {
        double res = 0.0;
    ...
    template <unsigned order_, unsigned nints_>
    complex<double>
    OmnesFactor<order_, nints_>::evaluate_omnes(const double & s) const
    {
        if (s > _intervals[0])
        {
            ...
            if (std::abs(_scattering_phase(s) - M_PI / 2.0) < 1e-8)
            {
                ...
                double phase_deriv =
                        (-_scattering_phase(s + 2.0 * _eps) + 8.0 * _scattering_phase(s + _eps) - 8.0 * _scattering_phase(s - _eps) + _scattering_phase(s - 2.0 * _eps));

...
namespace eos
{
    ...
    ResolutionConvolution::Interpolator::~Interpolator() = default;
    ...
    ResolutionConvolution::make(const std::vector<AxisGeometry> & axes)
    {
        switch (axes.size())
        {
            case 1:  return std::make_unique<ConcreteResolutionConvolution<1>>(axes);
            case 2:  return std::make_unique<ConcreteResolutionConvolution<2>>(axes);
            case 3:  return std::make_unique<ConcreteResolutionConvolution<3>>(axes);
            case 4:  return std::make_unique<ConcreteResolutionConvolution<4>>(axes);
            default: throw InternalError(std::format("ResolutionConvolution: dimensionality D = {} is not supported (must be in [1, 4])", axes.size()));
        }
    }
} // namespace eos

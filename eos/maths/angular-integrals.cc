...
namespace eos
{
    double wigner_3j(const int & j1, const int & m1, const int & j2, const int & m2, const int & j3, const int & m3)
    {
        ...
        const double pref = (((j1 - j2 - m3) % 2) ? -1.0 : 1.0)
                    * std::sqrt(std::tgamma(j1 + j2 - j3 + 1.0)) * std::sqrt(std::tgamma(j1 - j2 + j3 + 1.0))
                    * std::sqrt(std::tgamma(-j1 + j2 + j3 + 1.0)) / std::sqrt(std::tgamma(1 + j1 + j2 + j3 + 1.0))
                    * std::sqrt(std::tgamma(j1 + m1 + 1.0)) * std::sqrt(std::tgamma(j1 - m1 + 1.0)) * std::sqrt(std::tgamma(j2 + m2 + 1.0))
                    * std::sqrt(std::tgamma(j2 - m2 + 1.0)) * std::sqrt(std::tgamma(j3 + m3 + 1.0)) * std::sqrt(std::tgamma(j3 - m3 + 1.0));
        ...
        {
            ret += ((i % 2) ? -1.0 : 1.0) / (std::tgamma(i + 1.0) * std::tgamma(j1 + j2 - j3 - i + 1.0) * std::tgamma(j1 - m1 - i + 1.0) * std::tgamma(j2 + m2 - i + 1.0) * std::tgamma(j3 - j2 + m1 + i + 1.0) * std::tgamma(j3 - j1 - m2 + i + 1.0));
        }
        return ret * pref;
    }
    ...
    double legendre_integral(const int & l, const int & m)
    {
        ...
        const double reflection = (m >= 0) ? 1.0 : std::tgamma(l + m + 1.0) / std::tgamma(l - m + 1.0) * ((absm % 2) ? -1.0 : 1.0);
        const double sign = (l % 2) ? -1.0 : 1.0;
        return reflection * sign * absm * std::pow(2.0, absm - 1) * std::tgamma(l / 2.0) * std::tgamma((l + absm + 1) / 2.0) / std::tgamma((l + 3) / 2.0) / std::tgamma((l - absm) / 2 + 1.0);
    }
    ...
    double two_legendre_integral(const int & l1, const int & m1, const int & l2, const int & m2)
    {
        ...
        if ((m1 == m2) && (l1 == l2)) return 2.0 * std::tgamma(l1 + m1 + 1.0) / std::tgamma(l1 - m1 + 1.0) / (2 * l1 + 1);
        if ((l2 == 0) && (m2 == 0)) return legendre_integral(l1, m1);
        if ((l1 == 0) && (m1 == 0)) return legendre_integral(l2, m2);

        double ret = 0.0;
        const int m12 = m1 + m2;
        const double pref = ((m12 % 2) ? -1.0 : 1.0) * std::sqrt(std::tgamma(l1 + m1 + 1.0) * std::tgamma(l2 + m2 + 1.0) / (std::tgamma(l1 - m1 + 1.0) * std::tgamma(l2 - m2 + 1.0)));

        const int lowerlim = std::max({ m12, l1 - l2, l2 - l1 });
        for (int l12 = lowerlim; l12 <= l1 + l2; l12++)
        {
            ret += (2 * l12 + 1) * legendre_integral(l12, m12) * wigner_3j(l1, 0, l2, 0, l12, 0) * wigner_3j(l1, m1, l2, m2, l12, -m12) * std::sqrt(std::tgamma(l12 - m12 + 1.0) / std::tgamma(l12 + m12 + 1.0));
        }
        return pref * ret;
    }
    ...
    double three_legendre_integral(const int & l1, const int & m1, const int & l2, const int & m2, const int & l3, const int & m3)
    {
        ...
        for (int l12 = lowerlim; l12 <= l1 + l2; l12++)
        {
            ret += (2 * l12 + 1) * two_legendre_integral(l12, m12, l3, m3) * wigner_3j(l1, 0, l2, 0, l12, 0) * wigner_3j(l1, m1, l2, m2, l12, -m12)
                * std::sqrt(std::tgamma(l12 - m12 + 1.0) / std::tgamma(l12 + m12 + 1.0));
        }
        return pref * ret;
    }
}

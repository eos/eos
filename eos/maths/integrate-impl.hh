...
namespace eos
{
    namespace cubature
    {
        ...
        int
        integrand_wrapper(unsigned ndim, const double * x, void * data, unsigned fdim, double * fval)
        {
            assert(ndim == ndim_);
    ...
    integrate(const cubature::integrand<ndim_, fdim_, T_> & f, const typename cubature::integrand_traits<ndim_, fdim_, T_>::argument_type & a,
              const typename cubature::integrand_traits<ndim_, fdim_, T_>::argument_type & b, const cubature::Config & config)
    {
        ...
        if (hcubature(nintegrands,
                      &integrand_wrapper<ndim_, fdim_, T_>,
                      &const_cast<integrand &>(f),
                      ndim_,
                      integrand_traits::pointer_from_arguments(a),
                      integrand_traits::pointer_from_arguments(b),
                      config.maxeval(),
                      config.epsabs(),
                      config.epsrel(),
                      ERROR_L2,
                      integrand_traits::pointer_from_buffer(result_buffer),
                      integrand_traits::pointer_from_buffer(error_buffer)))
        {

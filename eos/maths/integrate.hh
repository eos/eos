...
namespace eos
{
    ...
    double integrate(const std::function<double(const double &)> & f, const double & a, const double & b, const typename Method_::Config & config = typename Method_::Config());
    ...
    namespace cubature
    {
        template <size_t ndim_, size_t fdim_, typename ResultT_> struct integrand_traits
        {
                ...
                static void
                copy_arguments(const double * from, argument_type & to)
                {
                    std::copy(from, from + ndim_, to.begin());
                }

                static void
                copy_result(result_type & from, double * to)
                {
                    std::copy(from.cbegin(), from.cend(), to);
                }

                static const double *
                pointer_from_arguments(const argument_type & arguments)
                {
                    return arguments.data();
                }

                static double *
                pointer_from_buffer(buffer_type & buffer)
                {
                    return buffer.data();
                }

                static result_type
                contruct_result(buffer_type & buffer)
                {
                    return result_type(buffer);
                }
        ...
        template <size_t ndim_, typename ResultT_> struct integrand_traits<ndim_, 1, ResultT_>
        {
                ...
                copy_arguments(const double * from, argument_type & to)
                {
                    std::copy(from, from + ndim_, to.data());
                }
        ...
        template <> struct integrand_traits<1, 1, complex<double>>
        {
                ...
                contruct_result(buffer_type & buffer)
                {
                    return result_type(buffer[0], buffer[1]);
                }
        ...
        template <size_t ndim_, size_t fdim_ = 1, typename T_ = double> using integrand = typename integrand_traits<ndim_, fdim_, T_>::function_type;
    ...
    typename cubature::integrand_traits<ndim_, fdim_, T_>::result_type
    integrate(const cubature::integrand<ndim_, fdim_, T_> & f, const typename cubature::integrand_traits<ndim_, fdim_, T_>::argument_type & a,
              const typename cubature::integrand_traits<ndim_, fdim_, T_>::argument_type & b, const cubature::Config & config = cubature::Config());

    class IntegrationError : public Exception
    {
        public:

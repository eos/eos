...
namespace eos
{
    namespace dft
    {
        namespace impl
        {
            ...
            template <std::size_t rank_>
            inline void
            check_dimensions(const std::array<std::size_t, rank_> & dimensions)
            {
            ...
            template <std::size_t rank_, Direction direction_> struct PlanTraits;

            template <> struct PlanTraits<1, Direction::Forward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 1> & dimensions, const dft::Container<double, 1> & time_domain_container,
                          const dft::Container<std::complex<double>, 1> & frequency_domain_container)
                    {
            ...
            template <> struct PlanTraits<1, Direction::Backward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 1> & dimensions, const dft::Container<double, 1> & time_domain_container,
                          const dft::Container<std::complex<double>, 1> & frequency_domain_container)
            ...
            template <> struct PlanTraits<2, Direction::Forward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 2> & dimensions, const dft::Container<double, 2> & time_domain_container,
                          const dft::Container<std::complex<double>, 2> & frequency_domain_container)
                    {
                        static const unsigned flags = FFTW_MEASURE | FFTW_DESTROY_INPUT;
            ...
            template <> struct PlanTraits<2, Direction::Backward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 2> & dimensions, const dft::Container<double, 2> & time_domain_container,
                          const dft::Container<std::complex<double>, 2> & frequency_domain_container)
            ...
            template <> struct PlanTraits<3, Direction::Forward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 3> & dimensions, const dft::Container<double, 3> & time_domain_container,
                          const dft::Container<std::complex<double>, 3> & frequency_domain_container)
                    {
                        static const unsigned flags = FFTW_MEASURE | FFTW_DESTROY_INPUT;
            ...
            template <> struct PlanTraits<3, Direction::Backward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 3> & dimensions, const dft::Container<double, 3> & time_domain_container,
                          const dft::Container<std::complex<double>, 3> & frequency_domain_container)
            ...
            template <> struct PlanTraits<4, Direction::Forward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 4> & dimensions, const dft::Container<double, 4> & time_domain_container,
                          const dft::Container<std::complex<double>, 4> & frequency_domain_container)
                    {
                        static const unsigned flags = FFTW_MEASURE | FFTW_DESTROY_INPUT;

                        std::array<int, 4> n;
                        std::transform(dimensions.begin(), dimensions.end(), n.begin(), [](std::size_t d) { return static_cast<int>(d); });
            ...
            template <> struct PlanTraits<4, Direction::Backward>
            {
                    static fftw_plan
                    alloc(const std::array<std::size_t, 4> & dimensions, const dft::Container<double, 4> & time_domain_container,
                          const dft::Container<std::complex<double>, 4> & frequency_domain_container)
    ...
    template <std::size_t rank_, dft::Direction direction_> struct Implementation<dft::Plan<rank_, direction_>>
    {
    ...
    namespace dft
    {
        ...
        template <std::size_t rank_, Direction direction_> Plan<rank_, direction_>::Plan(Plan && other) = default;

        template <std::size_t rank_, Direction direction_> Plan<rank_, direction_> & Plan<rank_, direction_>::operator= (Plan && other) = default;

        template <std::size_t rank_, Direction direction_> Plan<rank_, direction_>::~Plan() = default;

        template <std::size_t rank_, Direction direction_>
        const std::array<std::size_t, rank_> &
        Plan<rank_, direction_>::dimensions() const
        {
        ...
        template <std::size_t rank_, Direction direction_>
        void
        Plan<rank_, direction_>::transform()
        {
        ...
        Plan<rank_, direction_>::time_domain_container()
        {
        ...
        Plan<rank_, direction_>::frequency_domain_container()
        {

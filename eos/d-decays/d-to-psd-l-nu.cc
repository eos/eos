...
namespace eos
{
    ...
    struct Implementation<DToPseudoscalarLeptonNeutrino>
    {
        ...
        // { D, q, I } -> { process, m_D, m_P, c_I }
        // D: d, s; the quark flavor in the weak transition
        ...
        inline std::string _process() const
        {
            ...
            if (p == process_map.end())
                throw InternalError("Unsupported combination of D=" + stringify(D) + ", q=" + q + ", I=" + I);
        ...
        Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
            ...
            mu(p[opt_D.value() + "c" + opt_l.str() + "nu" + opt_l.str() + "::mu"], u),
            ...
            cp_conjugate(destringify<bool>(o.get("cp-conjugate", "false")))
        {
            ...
            if ('d' == opt_D.value()[0])
            {
                m_D_msbar = std::bind(&ModelComponent<components::QCD>::m_d_msbar, model.get(), _1);
                v_cD      = std::bind(&ModelComponent<components::CKM>::ckm_cd, model.get());
        ...
        d_to_psd_l_nu::Amplitudes amplitudes(const double & s) const
        {
            ...
            const double mDatmu = m_D_msbar(mu);
        ...
        // normalized (|V_cD| = 1) two-fold-distribution, cf. [DDS:2014A], eq. (12), p. 6
        ...
        // normalized to |V_cD = 1|, obtained using cf. [DSD:2014A], eq. (12), agrees with Sakaki'13 et al cf. [STTW:2013A]
        ...
        {
            return normalized_differential_decay_width(s) * std::norm(v_cD());
        }
        ...
        // "normalized" (|V_cD|=1) differential branching_ratio
    ...
    {
        { { 'd', 'u', "1"   }, { "D->pi",    "D_u", "pi^-", 1.0                  } }, // all parameters are only listed for "pi^-", rather than "pi^+".
        { { 'd', 'd', "1"   }, { "D->pi",    "D_d", "pi^0", 1.0 / std::sqrt(2.0) } },
    ...
    // normalized (|V_cD|=1) two-fold-distribution, cf. [DDS:2014A], eq. (13), p. 6
    double
    ...
    // normalized_differential_branching_ratio (|V_cD|=1)
    double
    ...
    // normalized (|V_cD|=1) integrated branching_ratio
    double
    ...
    // normalized (|V_cD|=1) integrated decay_width
    double
    ...
    The decay D->P l nu, where both D=(c qbar) and P=(D qbar) are pseudoscalars, and l=e,mu,tau is a lepton.";

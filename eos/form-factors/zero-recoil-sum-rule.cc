...
namespace eos
{
    template <> struct Implementation<ZeroRecoilSumRule<LambdaBToC>>
    {
            ...
            Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
                ...
                ff_2595(FormFactorFactory<OneHalfPlusToOneHalfMinus>::create("Lambda_b->Lambda_c(2595)@" + o.get("form-factors"_ok, "BBGIOvD2017"_ov).str(), p)),
                ff_2625(FormFactorFactory<OneHalfPlusToThreeHalfMinus>::create("Lambda_b->Lambda_c(2625)@" + o.get("form-factors"_ok, "BBGIOvD2017"_ov).str(), p)),
            ...
            xiV() const
            {
                ...
                const double xiVnlo1 = (3.0 * mb2 + 2.0 * mc * mb + 3.0 * mc2) / (2.0 * (mb2 - mc2)) * log((mu + wb) / (mu + wc)) - 2.0;
                const double xiVnlo2 = 4.0 / (3.0 * mu2) * (mc * wb - mb * wc) / (mb - mc) + 2.0 / 3.0 * (mc / wb - mb / wc) / (mb - mc)
                                       - 1.0 / 3.0 * (wb / mb - wc / mc) / (mb - mc) + 2.0 * mc * mb / (mb2 - mc2) * (1.0 / wb - 1.0 / wc)
                                       + 1.0 / (6.0 * (mc + mb)) * (wc / mc * (3.0 - mb / mc) + wb / mb * (3.0 - mc / mb))
                                       + 4.0 * mc * mb / (3.0 * (mc + mb)) * (mb / wb3 + mc / wc3) + mu / 6.0 * power_of<2>(1.0 / mc - 1.0 / mb)
    ...
    const std::vector<OptionSpecification> Implementation<ZeroRecoilSumRule<LambdaBToC>>::options{};
    ...
    const std::set<ReferenceName> ZeroRecoilSumRule<LambdaBToC>::references{};

...
namespace eos
{
    ...
    SEFormFactors<Process_, OneHalfPlusToThreeHalfMinus>::SEFormFactors(const Parameters & p, const Options &) :
        ...
        _a_perp32_v{ UsedParameter(p[_par_name("perp32", "V", 0)], *this),
        ...
        _a_perp32_a{ UsedParameter(p[_par_name("perp32", "A", 0)], *this),
        ...
        _a_perp32_t{ UsedParameter(p[_par_name("perp32", "T", 0)], *this),
        ...
        _a_perp32_t5{ UsedParameter(p[_par_name("perp32", "T5", 1)], *this),
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, OneHalfPlusToThreeHalfMinus>::_phi(const double & s, const double & t_p, const double & chi, const double & A, const double & B, const double & d,
                                                               const double & e, const double & f, const double & g, const double & n) const
    {
        using std::pow;
        using std::sqrt;
        ...
        const double norm = pow(t_pb, 0.5 * A) * pow(t_mb, 0.5 * B) * pow(1 - z, n + g - 0.5 * (e + f + 3.0)) * sqrt(1 + z) * sqrt(4.0 * (t_p - _traits.t0))
                            / sqrt(16.0 * d * M_PI * M_PI * chi);
    ...
    template <typename Process_> const std::set<ReferenceName> SEFormFactors<Process_, OneHalfPlusToThreeHalfMinus>::references{ "ABR:2022A"_rn };

    template <typename Process_> const std::vector<OptionSpecification> SEFormFactors<Process_, OneHalfPlusToThreeHalfMinus>::options{};

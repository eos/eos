...
namespace eos
{
    template <> struct Implementation<AntiKaonLCDAs>
    {
            ...
            f3K(const double & mu) const
            {
                ...
                return f3K_0 * std::pow(c_rge, 55.0 / 9.0) + 2.0 / 19.0 * (std::pow(c_rge, 4.0) - std::pow(c_rge, 55.0 / 9.0)) * f_K * (m_s_0 + m_q_0)
                       + 6.0 / 65.0 * (std::pow(c_rge, 55.0 / 9.0) - std::pow(c_rge, 68.0 / 9.0)) * f_K * (m_s_0 - m_q_0) * a1K_0;
            }

            double
            omega3K(const double & mu) const
            {
                const double c_rge = this->c_rge(mu);
                const double mu_0  = 1.0; // initial state is fixed at 1 GeV
                const double m_s_0 = model->m_s_msbar(mu_0);
                ...
                return (f3K_0 * omega3K_0 * std::pow(c_rge, 104.0 / 9.0) + 1.0 / 170.0 * (std::pow(c_rge, 4.0) - std::pow(c_rge, 104.0 / 9.0)) * f_K * (m_s_0 + m_q_0)
            ...
            double
            lambda3K(const double & mu) const
            {
                const double c_rge = this->c_rge(mu);
                const double mu_0  = 1.0; // initial state is fixed at 1 GeV
                const double m_s_0 = model->m_s_msbar(mu_0);
                ...
                return (f3K_0 * lambda3K_0 * std::pow(c_rge, 139.0 / 18.0) - 14.0 / 67.0 * (std::pow(c_rge, 4.0) - std::pow(c_rge, 139.0 / 18.0)) * f_K * (m_s_0 - m_q_0)
                        + 14.0 / 5.0 * (std::pow(c_rge, 68.0 / 9.0) - std::pow(c_rge, 139.0 / 18.0)) * f_K * (m_s_0 + m_q_0) * a1K_0
                        - 4.0 / 11.0 * (std::pow(c_rge, 86.0 / 9.0) - std::pow(c_rge, 139.0 / 18.0)) * f_K * (m_s_0 - m_q_0) * a2K_0)
                       / f3K(mu);
            }

            inline double
            eta3K(const double & mu) const
            {
                return f3K(mu) / (f_K() * muK(mu));
            }

            double
            delta4K(const double & mu) const
            {
                const double c_rge = this->c_rge(mu);
    ...
    AntiKaonLCDAs::phi3p(const double & u, const double & mu) const
    {
        ...
        return 1.0 + 3.0 * rhopK * (1.0 + 6.0 * a2K) - 9.0 * rhomK * a1K + c1 * (27.0 / 2.0 * rhopK * a1K - rhomK * (3.0 / 2.0 + 27.0 * a2K))
               + c2 * (30.0 * eta3K + 15.0 * rhopK * a2K - 3.0 * rhomK * a1K) + c3 * (10.0 * eta3K * lambda3K - 9.0 / 2.0 * rhomK * a2K) + c4 * (-3.0 * eta3K * omega3K)
               + 3.0 / 2.0 * (rhopK + rhomK) * (1.0 - 3.0 * a1K + 6.0 * a2K) * std::log(u) + 3.0 / 2.0 * (rhopK - rhomK) * (1.0 + 3.0 * a1K + 6.0 * a2K) * std::log(1.0 - u);
    }
    ...
    AntiKaonLCDAs::phi3s(const double & u, const double & mu) const
    {
        ...
        return 6.0 * u * ubar
                       * (1.0 + 3.0 / 2.0 * rhopK + 15.0 * rhopK * a2K - 15.0 / 2.0 * rhomK * a1K + c1 * (3.0 * rhopK * a1K - 15.0 / 2.0 * rhomK * a2K)
                          + c2 * (5.0 * eta3K - 1.0 / 2.0 * eta3K * omega3K + 3.0 / 2.0 * rhopK * a2K) + c3 * (eta3K * lambda3K))
    ...
    AntiKaonLCDAs::phi4(const double & u, const double & mu) const
    {
        ...
        const double phi4T4 =
                200.0 / 3.0 * delta4K * u2 * ubar2 + 20.0 * u2 * ubar2 * x * (4.0 * theta1K - 5.0 * theta2K)
                + 21.0 * delta4K * omega4K
                          * (u * ubar * (2.0 + 13.0 * u * ubar) + (2.0 * u3 * (6.0 * u2 - 15.0 * u + 10.0) * lnu) + (2.0 * ubar3 * (6.0 * ubar2 - 15.0 * ubar + 10.0) * lnubar))
                + 40.0 * phi2K * (u * ubar * x * (2.0 - 3.0 * u * ubar) - (2.0 * u3 * (u - 2.0) * lnu) + (2.0 * ubar3 * (ubar - 2.0) * lnubar));
        const double phi4WW =
                16.0 / 3.0 * m_K * m_K * kappa4K * (u * ubar * x * (1.0 - 2.0 * u * ubar) + (5.0 * (u - 2.0) * u3 * lnu) - (5.0 * (ubar - 2.0) * ubar3 * lnubar))
                + 4.0 * f3K / f_K * (m_s + m_ud) * u * ubar
                          * (30.0 * (1.0 - x * (m_s - m_ud) / (m_s + m_ud)) + 10.0 * lambda3K * (x * (1.0 - u * ubar) - (m_s - m_ud) / (m_s + m_ud) * (1.0 - 5.0 * u * ubar))
                             - omega3K * (3.0 - 21.0 * u * ubar + 28.0 * u2 * ubar2 + 3.0 * x * (m_s - m_ud) / (m_s + m_ud) * (1.0 - 7.0 * u * ubar)))
                - 36.0 / 5.0 * m_K * m_K * a2K
                          * (1.0 / 4.0 * u * ubar * (4.0 - 9.0 * u * ubar + 110.0 * u2 * ubar2) + (u3 * (10.0 - 15.0 * u + 6.0 * u2) * lnu)
                             + (ubar3 * (10.0 - 15.0 * ubar + 6.0 * ubar2) * lnubar))
    ...
    AntiKaonLCDAs::phi4_d1(const double & u, const double & mu) const
    {
        ...
        const double phi4T4_d1 = 20.0 * ubar2 * lnubar * (8.0 * (1.0 + 2.0 * u) * phi2K - 63.0 * u2 * omega4K * delta4K)
                                 ...
                                 - 20.0 / 3.0 * u
                                           * (-20.0 * (1.0 - 3.0 * u + 2.0 * u2) * delta4K + 12.0 * (-8.0 + 23.0 * u - 30.0 * u2 + 15.0 * u3) * phi2K
    ...
    AntiKaonLCDAs::phi4_d2(const double & u, const double & mu) const
    {
        ...
        // Twist 4 derivatives contributions
        const double phi4T4_d2 =
                400.0 / 3.0 * (1.0 - 6.0 * u + 6.0 * u2) * delta4K
                - 20.0
                          * (24.0 * phi2K * (-1.0 + 7.0 * u - 15.0 * u2 + 10.0 * u3) + (-1.0 + 12.0 * u - 30.0 * u2 + 20.0 * u3) * (-8.0 * theta1K + 10.0 * theta2K)
    ...
    AntiKaonLCDAs::psi4(const double & u, const double & mu) const
    {
        ...
        const double psi4WW = c0 * m_K * m_K * (1.0 + 6.0 * rhopK * (1.0 + 6.0 * a2K) - 18.0 * rhomK * a1K)
                              ...
                              + c4 * (-9.0 / 28.0 * m_K * m_K * a2K - 6.0 * f3K * omega3K / f_K * (m_s + m_ud)) + 6.0 * m_ud * (m_s + m_ud) * (1.0 + 3.0 * a1K + 6.0 * a2K) * lnubar
    ...
    AntiKaonLCDAs::psi4_i(const double & u, const double & mu) const
    {
        ...
        // Twist 4 contributions
        const double psi4T4_i = -5.0 / 3.0 * u * ubar * (delta4K * (8.0 * u - 4.0) + 3.0 * (1.0 - 5.0 * u + 5.0 * u2) * (5.0 * theta1K - theta2K));
        // clang-format off
    ...
    template <> struct Implementation<KaonLCDAs>
    {
            std::shared_ptr<Model> model;

...
namespace eos
{
    template <> struct Implementation<RhoLCDAs>
    {
            ...
            {
                return (std::pow(c_rge(mu), (205 - std::sqrt(865)) / 18.0)
                        * (6.0 * std::sqrt(865.0) * (-1.0 + std::pow(c_rge(mu), std::sqrt(865.0) / 9.0)) * omega3para_0
                           + (865.0 - 26.0 * std::sqrt(865.0) + (865.0 + 26.0 * std::sqrt(865.0)) * std::pow(c_rge(mu), std::sqrt(865.0) / 9.0)) * omega3paratilde_0))
                       / 1730.0;
            }
            ...
            {
                return (std::pow(c_rge(mu), (205.0 - std::sqrt(865.0)) / 18.0)
                        * ((1730.0 + 52.0 * std::sqrt(865.0) + (1730.0 - 52.0 * std::sqrt(865.0)) * std::pow(c_rge(mu), std::sqrt(865.0) / 9.0)) * omega3para_0
                           + 63.0 * std::sqrt(865.0) * (-1.0 + std::pow(c_rge(mu), std::sqrt(865.0) / 9.0)) * omega3paratilde_0))
                       / 3460.0;
            }
            ...
            {
                return 3.0 * std::pow(-1.0 + 2.0 * u, 2) + (3.0 * a2perp(mu) * std::pow(-1.0 + 2.0 * u, 2) * (-3.0 + 5.0 * std::pow(-1.0 + 2.0 * u, 2))) / 2.0
            ...
            psi3perp(const double & u, const double & mu) const
            {
                return 6.0 * (1.0 - u) * u
                       * (1.0
                          + (-1.5 + (15.0 * std::pow(-1.0 + 2.0 * u, 2)) / 2.0)
                                    * (a2para(mu) / 6.0 + (5.0 * omega3para(mu)) / 12.0 - (5.0 * omega3paratilde(mu)) / 24.0 + (10.0 * zeta3para(mu)) / 9.0));
            }

            inline double
            phi3perp(const double & u, const double & mu) const
            {
                return (3.0 * (1.0 + std::pow(-1.0 + 2.0 * u, 2))) / 4.0
                       + ((9.0 * a2para(mu)) / 112.0 + (15.0 * omega3para(mu)) / 32.0 - (15.0 * omega3paratilde(mu)) / 64.0)
                                 * (3.0 - 30.0 * std::pow(-1.0 + 2.0 * u, 2) + 35.0 * std::pow(-1.0 + 2.0 * u, 4))
                       + (-1.0 + 3.0 * std::pow(-1.0 + 2.0 * u, 2)) * ((3.0 * a2para(mu)) / 7.0 + 5.0 * zeta3para(mu));
            }
            ...
            psi4paraWW(const double & u, const double & mu) const
            {
                ...
                return 1.0 + (-1.0 - 2.0 / 7.0 * a2para(mu) + 40.0 / 3.0 * zeta3para(mu)) * c2
            ...
            phi4paraWW(const double & u, const double & mu) const
            {
                ...
                return 30.0 * u * u * (1.0 - u) * (1.0 - u)
                               * (4.0 / 5.0 * (1.0 + 1.0 / 21.0 * a2para(mu) + 10.0 / 9.0 * zeta3para(mu))
                                  + 1.0 / 10.0 * (9.0 / 7.0 * a2para(mu) + 1.0 / 9.0 * zeta3para(mu) + 7.0 / 6.0 * omega3para(mu) - 3.0 / 4.0 * omega3paratilde(mu)) * c2)
                       + 2.0 * (-2.0 * a2para(mu) - 14.0 / 3.0 * zeta3para(mu) + 3.0 * omega3para(mu))
                                 * (1.0 / 8.0 * u * (1.0 - u) * (21.0 - 13.0 * x * x) + power_of<3>(u) * (10.0 - 15.0 * u + 6.0 * u * u) * log(u)
                                    + power_of<3>(1.0 - u) * (10.0 - 15.0 * (1.0 - u) + 6.0 * power_of<2>(1.0 - u)) * log(1.0 - u));
            }
    };

...
class BToPiPiFvDV2018FormFactorsTest : public TestCase
{
    public:
        ...
        run() const
        {
            ...
            TEST_CHECK_NEARLY_EQUAL(imag(ff->f_time(0.60, 19.0, 1.0)) - imag(time_pw[0] + std::sqrt(3.0) * time_pw[1] + std::sqrt(5.0) * time_pw[2] + std::sqrt(7.0) * time_pw[3]),
                                    0.0,
                                    eps);

            auto long_pw = ff->f_long(0.60, 19.0);
            TEST_CHECK_NEARLY_EQUAL(imag(ff->f_long(0.60, 19.0, 1.0)) - imag(long_pw[0] + std::sqrt(3.0) * long_pw[1] + std::sqrt(5.0) * long_pw[2] + std::sqrt(7.0) * long_pw[3]),
                                    0.0,
                                    eps);

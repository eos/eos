...
namespace eos
{
    ...
    BHKMNR2026FormFactors<VacuumToPiPi>::BHKMNR2026FormFactors(const Parameters & p, const Options & o) :
        // clang-format off
        _a_fp_I1{ { UsedParameter(p[_par_name("+", "1", "4")], *this),
        ...
        _M_fp_I0{ { UsedParameter(p["0->pipi::M_(+,0,0)@BHKMNR2026"], *this),
                    UsedParameter(p["0->pipi::M_(+,0,1)@BHKMNR2026"], *this) } },
        _G_fp_I0{ { UsedParameter(p["0->pipi::Gamma_(+,0,0)@BHKMNR2026"], *this),
        ...
        _init_M(gsl_matrix_alloc(4, 4)),
        _L(gsl_vector_alloc(4)),
        _work(gsl_vector_alloc(4)),
    ...
    BHKMNR2026FormFactors<VacuumToPiPi>::constrained_a_fp_I1() const
    {
        ...
        // Solve the system for the constrained coefficients
        int signum = 0;
        gsl_permutation_init(_perm);
        gsl_matrix_memcpy(_init_M, _M);
        gsl_linalg_LU_decomp(_M, _perm, &signum);
        gsl_linalg_LU_solve(_M, _perm, _L, _constrained_coefficents);
        // Refine the result once
        gsl_linalg_LU_refine(_init_M, _M, _perm, _L, _constrained_coefficents, _work);

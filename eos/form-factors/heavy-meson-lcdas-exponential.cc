    ...
    namespace heavy_meson_lcdas
    {
        ...
        Exponential::parameter(const char * _name, const bool & flvd2022) const
        {
            ...
            if (flvd2022)
            {
                ...
                return QualifiedName(flvd2022_prefixes.at(std::make_tuple(opt_Q.value(), opt_q.value())),
                        qnp::Name(_name), qnp::Suffix("FLvD2022")).str();
            }
        ...
        Exponential::xi_1(const double & omega) const
        {
            ...
            return 2.0 / 3.0 * kappa * (lambda_E2 + 2.0 * lambda_H2) * bracket
                - 2.0 * omega * phi_minus_ww + 3.0 * omega * omega * f + omega * omega * omega * f_prime;
        }

...
namespace eos
{
    ...
    KKRvD2024FormFactors<VacuumToPiPi>::dFdq2_q2eq0() const
    {
        ...
        const double dFdz_z0 = sum1 * xprime_z0 + sum2 / (phitilde_z0 * std::norm(z0 - zr)); // the weight factor has been absorbed into 1 / phi
    ...
    double
    KKRvD2024FormFactors<VacuumToPiPi>::r_pi() const
    {
        return sqrt(6.0 * this->dFdq2_q2eq0()) * this->_hbarc();
    }
    ...
    double KKRvD2024FormFactors<VacuumToPiPi>::saturation() const
    {
        ...
        return integrate1D(f, 512, -M_PI, M_PI) / (2.0 * M_PI);
    }

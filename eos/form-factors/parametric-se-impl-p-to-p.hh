...
namespace eos
{
    ...
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string> SEFormFactorTraits<Process_, PToP>::resonance_0p_names{
    ...
    template <typename Process_>
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string> SEFormFactorTraits<Process_, PToP>::resonance_1m_names{
    ...
    template <typename Process_> SEFormFactors<Process_, PToP>::~SEFormFactors() = default;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToP>::_phi(const double & t, const double & threshold_tp, const double & chi, const int & A, const unsigned B, const unsigned C, const unsigned k,
                                        const unsigned p, const unsigned n, const unsigned m) const
    {
        // [GRvDV:2023A]
        const double z = _traits.calc_z(t, threshold_tp, _traits.t0), kinematic_tp = power_of<2>(_mB + _mP);
        const double norm = std::sqrt(Process_::eta * k * pow(kinematic_tp, A) * pow(_traits.tm(), B) * pow(4 * _mB * _mP, C) / 96 / M_PI / M_PI / chi);
    ...
    template <typename Process_> const std::set<ReferenceName> SEFormFactors<Process_, PToP>::references{ "BFW:2010A"_rn };

    template <typename Process_> const std::vector<OptionSpecification> SEFormFactors<Process_, PToP>::options{};

...
namespace eos
{
    template <> struct Implementation<AntiKStarLCDAs>
    {
            ...
            inline double
            zeta3para(const double & mu) const
            {
                return (zeta3para_0 * std::pow(c_rge(mu), 77.0 / 9.0) * fpara
                        ...
                        + (2.0 * (std::pow(c_rge(mu), 16.0 / 3.0) - std::pow(c_rge(mu), 77.0 / 9.0)) * fperp_0 * (mq0 + ms0)) / (29.0 * M_V))
                       / fpara;
            }

            inline double
            kappa3para(const double & mu) const
            {
            ...
            inline double
            kappa3perp(const double & mu) const
            {
                return (std::pow(c_rge(mu), 55.0 / 9.0) * fperp_0 * kappa3perp_0
                        ...
                        + (12.0 * a1para_0 * std::pow(c_rge(mu), 55.0 / 9.0) * (-1.0 + std::pow(c_rge(mu), 13.0 / 9.0)) * fpara * (mq0 + ms0)) / (65.0 * M_V))
                       / fperp(mu);
            }

            inline double
            omega3perp(const double & mu) const
            {
                return ((-42.0 * a1para_0 * (-1.0 + std::pow(c_rge(mu), 5.0 / 9.0)) * std::pow(c_rge(mu), 68.0 / 9.0) * fpara * (mq0 - ms0)) / (25.0 * M_V)
                        ...
                        + std::pow(c_rge(mu), 73.0 / 9.0) * fperp_0 * omega3perp_0)
                       / fperp(mu);
            }

            inline double
            lambda3perp(const double & mu) const
            {
                return (std::pow(c_rge(mu), 4.0)
                        * (3.0 * fpara * (mq0 - ms0) + 68.0 * a2para_0 * std::pow(c_rge(mu), 5.555555555555555) * fpara * (mq0 - ms0)
                           - 51.0 * a1para_0 * std::pow(c_rge(mu), 3.5555555555555554) * fpara * (mq0 + ms0)
                           + std::pow(c_rge(mu), 7.555555555555555)
                                     * (255.0 * fperp_0 * lambda3perp_0 * M_V + (-3.0 + 51.0 * a1para_0 - 68.0 * a2para_0) * fpara * mq0
                                        + (3.0 + 51.0 * a1para_0 + 68.0 * a2para_0) * fpara * ms0)))
                       / (255.0 * fperp(mu) * M_V);
            }

            inline double
            omega3para(const double & mu) const
            {
                // clang-format off
            ...
            inline double
            omega3paratilde(const double & mu) const
            {
                // clang-format off
            ...
            inline double
            lambda3para(const double & mu) const
            {
                // clang-format off
            ...
            inline double
            lambda3paratilde(const double & mu) const
            {
                // clang-format off
            ...
            inline double
            zeta4para(const double & mu) const
            {
                return zeta4para_0 * std::pow(c_rge(mu), 32.0 / 9.0);
            }

            inline double
            omega4paratilde(const double & mu) const
            {
            ...
            inline double
            zeta4perp(const double & mu) const
            {
            ...
            inline double
            zeta4perptilde(const double & mu) const
            {
                return 1.0 / 2.0 * (std::pow(c_rge(mu), 49.0 / 9.0) - std::pow(c_rge(mu), 20.0 / 3.0)) * zeta4perp_0
                       + 1.0 / 2.0 * (std::pow(c_rge(mu), 49.0 / 9.0) + std::pow(c_rge(mu), 20.0 / 3.0)) * zeta4perptilde_0;
            }
            ...
            inline double
            kappa4para(const double & mu) const
            {
                return -3.0 / 20.0 * std::pow(c_rge(mu), 32.0 / 9.0) * a1para_0 - std::pow(c_rge(mu), 16.0 / 3.0) * fperp_0 / fpara * (ms0 - mq0) / (4.0 * M_V)
                       + std::pow(c_rge(mu), 8.0) * (std::pow(ms0, 2.0) - std::pow(mq0, 2.0)) / (2.0 * std::pow(M_V, 2.0));
            }

            inline double
            kappa4perp(const double & mu) const
            {
                return 1.0 / 10.0 * std::pow(c_rge(mu), 8.0 / 3.0) * a1perp_0 + std::pow(c_rge(mu), 8.0 / 3.0) * fpara / fperp_0 * (ms0 - mq0) / (12.0 * M_V)
            ...
            inline double
            psi3para(const double & u, const double & mu) const
            {
                // clang-format off
                       ...
                       - (3.0 * fpara * (model->m_s_msbar(mu) - model->m_ud_msbar(mu) / 2.0) * (-1.0 + 2.0 * u)
                          * (2.0 + 9.0 * a1para(mu) * (-1.0 + 2.0 * u) + 2.0 * a2para(mu) * (11.0 - 30.0 * (1.0 - u) * u) + (1.0 + 3.0 * a1para(mu) + 6.0 * a2para(mu)) * log(1 - u)
                             + (1.0 - 3.0 * a1para(mu) + 6.0 * a2para(mu)) * log(u)))
                                 / (2.0 * fperp(mu) * M_V)
                       + (3.0 * fpara * (model->m_s_msbar(mu) + model->m_ud_msbar(mu) / 2.0)
                          * (1.0 + 8.0 * a1para(mu) * (-1.0 + 2.0 * u) + 3.0 * a2para(mu) * (7.0 - 30.0 * (1.0 - u) * u)
                             + (1.0 + 3.0 * a1para(mu) + 6.0 * a2para(mu)) * (-1.0 + 2.0 * u) * log(1.0 - u)
                             - (1.0 - 3.0 * a1para(mu) + 6.0 * a2para(mu)) * (-1.0 + 2.0 * u) * log(u)))
                                 / (2.0 * fperp(mu) * M_V);
            }

            inline double
            psi3perp(const double & u, const double & mu) const
            {
                // clang-format off
            ...
            inline double
            psi0para(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            psi1para(const double & mu) const
            {
                return 7.0 / 12.0 * zeta4para(mu);
            }

            inline double
            psi2para(const double & mu) const
            {
                return -7.0 / 20.0 * a1para(mu) * zeta4para(mu);
            }

            inline double
            psi0paratilde(const double & mu) const
            {
                return 1.0 / 3.0 * zeta4para(mu);
            }

            inline double
            psi1paratilde(const double & mu) const
            {
                return 7.0 / 4.0 * a1para(mu) * zeta4para(mu);
            }

            inline double
            psi2paratilde(const double & mu) const
            {
                return -7.0 / 12.0 * zeta4para(mu);
            }

            inline double
            phi0para(const double & mu) const
            {
                return 1.0 / 3.0 * zeta4para(mu);
            }

            inline double
            phi1para(const double & mu) const
            {
                return -7.0 / 18.0 * zeta4para(mu);
            }

            inline double
            phi2para(const double & mu) const
            {
                return -7.0 / 9.0 * zeta4para(mu);
            }

            inline double
            theta0para(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            theta1para(const double & mu) const
            {
                return -7.0 / 10.0 * a1para(mu) * zeta4para(mu);
            }

            inline double
            theta2para(const double & mu) const
            {
                return 7.0 / 5.0 * a1para(mu) * zeta4para(mu);
            }

            inline double
            xi0para(const double & mu) const
            {
            ...
            inline double
            Psi4para(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 120.0 * u1 * u2 * u3 * (psi0para(mu) + psi1para(mu) * (u1 - u2) + psi2para(mu) * (3.0 * u3 - 1.0));
            }

            inline double
            Psi4paratilde(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 120.0 * u1 * u2 * u3 * (psi0paratilde(mu) + psi1paratilde(mu) * (u1 - u2) + psi2paratilde(mu) * (3.0 * u3 - 1.0));
            }

            inline double
            Phi4para(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 30.0 * u3 * u3
                       * (theta0para(mu) * (1.0 - u3) + theta1para(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2) + theta2para(mu) * (u3 * (1.0 - u3) - 3.0 / 2.0 * (u1 * u1 + u2 * u2))
                          - (u1 - u2) * (phi0para(mu) + u3 * phi1para(mu) + 1.0 / 2.0 * (5.0 * u3 - 3.0) * phi2para(mu)));
            }

            inline double
            Phi4paratilde(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 30.0 * u3 * u3
                       * (phi0para(mu) * (1.0 - u3) + phi1para(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2) + phi2para(mu) * (u3 * (1.0 - u3) - 3.0 / 2.0 * (u1 * u1 + u2 * u2))
                          - (u1 - u2) * (theta0para(mu) + u3 * theta1para(mu) + 1.0 / 2.0 * (5.0 * u3 - 3.0) * theta2para(mu)));
            }

            inline double
            Xi4para(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 840.0 * u1 * u2 * u3 * u3 * u3 * xi0para(mu);
            }

            // definition of chiral odd G conserving parameters appearing in three-particle twist 4 LCDAs, [BBL:2007A], eq. 4.13, setting zeta4perptilde = -zeta4perp (renormalon
            // model)
            inline double
            psi0perp(const double & mu) const
            {
                return zeta4perp(mu);
            }

            inline double
            psi0perptilde(const double & mu) const
            {
                return -zeta4perp(mu);
            }
            ...
            inline double
            phi1perp(const double & mu) const
            {
                return 63.0 / 220.0 * Q1(mu) - 119.0 / 44.0 * Q3(mu);
            }

            inline double
            phi1perptilde(const double & mu) const
            {
                return -63.0 / 220.0 * Q1(mu) - 35.0 / 44.0 * Q3(mu);
            }

            inline double
            psi1perp(const double & mu) const
            {
                return 49.0 / 110.0 * Q1(mu) - 7.0 / 22.0 * Q3(mu);
            }

            inline double
            psi1perptilde(const double & mu) const
            {
                return -49.0 / 110.0 * Q1(mu) + 7.0 / 22.0 * Q3(mu);
            }

            inline double
            psi2perp(const double & mu) const
            {
                return 28.0 / 55.0 * Q1(mu) + 7.0 / 11.0 * Q3(mu);
            }

            inline double
            psi2perptilde(const double & mu) const
            {
                return -28.0 / 55.0 * Q1(mu) - 7.0 / 11.0 * Q3(mu);
            }
            ...
            inline double
            Q1(const double & mu) const
            {
                return -10.0 / 3.0 * zeta4perp(mu);
            }

            inline double
            Q3(const double & mu) const
            {
            ...
            inline double
            phi0perp(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            phi0perptilde(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            phi2perp(const double & mu) const
            {
                return -21.0 / 20.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            phi2perptilde(const double & mu) const
            {
                return -21.0 / 20.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            theta0perp(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            theta0perptilde(const double & /*mu*/) const
            {
                return 0.0;
            }

            inline double
            theta1perp(const double & mu) const
            {
                return -21.0 / 10.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            theta1perptilde(const double & mu) const
            {
                return 21.0 / 10.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            theta2perp(const double & mu) const
            {
                return 21.0 / 5.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            theta2perptilde(const double & mu) const
            {
                return -21.0 / 5.0 * zeta4perp(mu) * a1perp(mu);
            }

            inline double
            xi0perp(const double & mu) const
            {
            ...
            inline double
            Psi4perp(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 30.0 * u3 * u3
                       * (psi0perp(mu) * (1.0 - u3) + psi1perp(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2) + psi2perp(mu) * (u3 * (1.0 - u3) - 3.0 / 2.0 * (u1 * u1 + u2 * u2))
                          - (u1 - u2) * (theta0perp(mu) + u3 * theta1perp(mu) + 1.0 / 2.0 * (5.0 * u3 - 3.0) * theta2perp(mu)));
            }

            inline double
            Psi4perptilde(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 30.0 * u3 * u3
                       * (psi0perptilde(mu) * (1.0 - u3) + psi1perptilde(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2)
            ...
            inline double
            Phi4perp1(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 120.0 * u1 * u2 * u3 * (phi0perp(mu) + phi1perp(mu) * (u1 - u2) + phi2perp(mu) * (3.0 * u3 - 1.0));
            }

            inline double
            Phi4perp2(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return -30.0 * u3 * u3
                       * (theta0perptilde(mu) * (1.0 - u3) + theta1perptilde(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2)
            ...
            inline double
            Phi4perp3(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return -120.0 * u1 * u2 * u3 * (phi0perptilde(mu) + phi1perptilde(mu) * (u1 - u2) + phi2perptilde(mu) * (3.0 * u3 - 1.0));
            }

            inline double
            Phi4perp4(const double & u1, const double & u2, const double & u3, const double & mu) const
            {
                return 30.0 * u3 * u3
                       * (theta0perp(mu) * (1.0 - u3) + theta1perp(mu) * (u3 * (1.0 - u3) - 6.0 * u1 * u2) + theta2perp(mu) * (u3 * (1.0 - u3) - 3.0 / 2.0 * (u1 * u1 + u2 * u2))
            ...
            inline double
            psi4paraT4(const double & u, const double & mu) const
            {
            ...
            inline double
            psi4paraWW(const double & u, const double & mu) const
            {
                ...
                return 1.0 + (12.0 * kappa4para(mu) + 9.0 / 5.0 * a1para(mu)) * c1 + (-1.0 - 2.0 / 7.0 * a2para(mu) + 40.0 / 3.0 * zeta3para(mu)) * c2
                       ...
                       + 6.0 * (ms - mud) / M_V * fperp(mu) / fpara
                                 * (x + a1perp(mu) / 2.0 * (3.0 * x * x - 1.0) + a2perp(mu) / 2.0 * x * (5.0 * x * x - 3.0) + 5.0 / 2.0 * kappa3perp(mu) * (3.0 * x * x - 1.0)
                                    + 5.0 / 6.0 * omega3perp(mu) * x * (5.0 * x * x - 3.0) - 1.0 / 16.0 * lambda3perp(mu) * (35.0 * power_of<4>(x) - 30.0 * x * x + 3.0));
            }

            inline double
            phi4paraT4(const double & u, const double & mu) const
            {
                ...
                return 30.0 * u * u * (1.0 - u) * (1.0 - u) * (20.0 / 9.0 * zeta4para(mu) + (-8.0 / 15.0 * theta1para(mu) + 2.0 / 3.0 * theta2para(mu)) * c1)
                       - 84.0 * omega4paratilde(mu)
                                 * (1.0 / 8.0 * u * (1.0 - u) * (21.0 - 13.0 * x * x) + power_of<3>(u) * (10.0 - 15.0 * u + 6.0 * u * u) * log(u)
                                    + power_of<3>(1.0 - u) * (10.0 - 15.0 * (1.0 - u) + 6.0 * power_of<2>(1.0 - u)) * log(1.0 - u))
                       + 80.0 * psi2para(mu)
                                 * (power_of<3>(u) * (2.0 - u) * log(u) - power_of<3>(1.0 - u) * (2.0 - (1.0 - u)) * log(1.0 - u)
                                    + u * (1.0 - u) * (-1.0 + u / 2.0 + 9.0 / 2.0 * u * u - 3.0 * u * u * u));
            }

            inline double
            phi4paraWW(const double & u, const double & mu) const
            {
    ...
    template <> struct Implementation<KStarLCDAs>
    {
            ...
            {
                return -1.0
                       * (std::pow(c_rge(mu), 77.0 / 9.0) * fpara * kappa3para_0
                          - (2.0 * std::pow(c_rge(mu), 16.0 / 3.0) * (-1.0 + std::pow(c_rge(mu), 29.0 / 9.0)) * fperp_0 * (mq0 - ms0)) / (29.0 * M_V)
                          + (6.0 * a1perp_0 * (-1.0 + std::pow(c_rge(mu), 5.0 / 9.0)) * std::pow(c_rge(mu), 8.0) * fperp_0 * (mq0 + ms0)) / (25.0 * M_V))
                       / fpara;
            }
            ...
            {
                return -1.0
                       * (std::pow(c_rge(mu), 55.0 / 9.0) * fperp_0 * kappa3perp_0
                          - (4.0 * std::pow(c_rge(mu), 4.0) * (-1.0 + std::pow(c_rge(mu), 19.0 / 9.0)) * fpara * (mq0 - ms0)) / (19.0 * M_V)
                          + (12.0 * a1para_0 * std::pow(c_rge(mu), 55.0 / 9.0) * (-1.0 + std::pow(c_rge(mu), 13.0 / 9.0)) * fpara * (mq0 + ms0)) / (65.0 * M_V))
                       / fperp(mu);
            }
            ...
            {
                return -1.0
                       * (-3.0 / 20.0 * std::pow(c_rge(mu), 32.0 / 9.0) * a1para_0 - std::pow(c_rge(mu), 16.0 / 3.0) * fperp_0 / fpara * (ms0 - mq0) / (4.0 * M_V)
                          + std::pow(c_rge(mu), 8.0) * (std::pow(ms0, 2.0) - std::pow(mq0, 2.0)) / (2.0 * std::pow(M_V, 2.0)));
            }
            ...
            {
                return -1.0
                       * (1.0 / 10.0 * std::pow(c_rge(mu), 8.0 / 3.0) * a1perp_0 + std::pow(c_rge(mu), 8.0 / 3.0) * fpara / fperp_0 * (ms0 - mq0) / (12.0 * M_V)
                          - std::pow(c_rge(mu), 8.0) * (std::pow(ms0, 2.0) - std::pow(mq0, 2.0)) / (4.0 * std::pow(M_V, 2.0)));
            }
            ...
            psi4paraWW(const double & u, const double & mu) const
            {
                ...
                return 1.0 + (12.0 * kappa4para(mu) + 9.0 / 5.0 * a1para(mu)) * c1 + (-1.0 - 2.0 / 7.0 * a2para(mu) + 40.0 / 3.0 * zeta3para(mu)) * c2
                       + (-9.0 / 5.0 * a1para(mu) - 20.0 / 3.0 * kappa3para(mu) - 16.0 / 3.0 * kappa4para(mu)) * c3
                       + (-27.0 / 28.0 * a2para(mu) + 5.0 / 4.0 * zeta3para(mu) - 15.0 / 8.0 * omega3para(mu) - 15.0 / 16.0 * omega3paratilde(mu)) * c4
                       + 6.0 * (-1.0) * (ms - mud) / M_V * fperp(mu) / fpara
                                 * (x + a1perp(mu) / 2.0 * (3.0 * x * x - 1.0) + a2perp(mu) / 2.0 * x * (5.0 * x * x - 3.0) + 5.0 / 2.0 * kappa3perp(mu) * (3.0 * x * x - 1.0)
                                    + 5.0 / 6.0 * omega3perp(mu) * x * (5.0 * x * x - 3.0) - 1.0 / 16.0 * lambda3perp(mu) * (35.0 * power_of<4>(x) - 30.0 * x * x + 3.0));
            }

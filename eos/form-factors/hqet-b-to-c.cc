...
namespace eos
{
    template <> struct Implementation<HQETBToC>
    {
            ...
            /* auxilliary functions from [N:1993A] */

            // r(omega) is defined in [N:1993A] eq. (3.104), p. 63
            inline static double
            r(const double & omega)
            {
                if (omega < 1.0)
                {
            ...
            // f(omega) is defined in [N:1993A] eq. (3.117), p. 65
            inline static double
            f(const double & omega)
            {
                if (omega < 1.0)
                {
            ...
            // g(z, omega) is defined in [N:1993A] eq. (3.129), p. 70
            inline static double
            g(const double & z, const double & omega)
            {
                if (omega < 1.0)
                {
                ...
                return omega / sqrt(omega * omega - 1.0) * (L2_m - L2_p) - z / (1.0 - 2.0 * omega * z + z * z) * ((omega * omega - 1.0) * r_omega + (omega - z) * log(z));
            }

            /* anomalous dimensions and auxilliaries for next-to-leading log terms */

            // a_hh(omega) is defined in [N:1993A] eq. (3.119), p. 66
            inline static double
            a_hh(const double & omega)
            {
                return 8.0 / 27.0 * (omega * r(omega) - 1.0);
            }

            // Z_hh(omega) is defined in [N:1993A] eq. (3.119), p. 66
            // Note that we use only the Taylor expansion in (omega - 1) up to second order.
            inline static double
            Z_hh(const double & omega)
            {
                return 8.0 / (81.0) * (94.0 / 9.0 - M_PI * M_PI) * (omega - 1.0) - 4.0 / 135.0 * (92.0 / 9.0 - M_PI * M_PI) * power_of<2>(omega - 1.0);
            }

            // S_{1,2,3}^{(5)} for the two currents are defined in [N:1993A] eq. (3.145)
            inline static double
            S_1(const double & x, const double & omega)
            {
            ...
            inline static double
            S_1_5(const double & x, const double & omega)
            {
                return omega * (17.0 / 27.0 - 5.0 / 9.0 * pow(x, -6.0 / 25.0) - 2.0 / 27.0 * pow(x, -9.0 / 25.0) + 8.0 / 25.0 * log(x))
                       - (1.0 / 6.0 - 5.0 / 9.0 * pow(x, -6.0 / 25.0) + 4.0 / 9.0 * pow(x, -9.0 / 25.0) - 1.0 / 18.0 * pow(x, -12.0 / 25.0));
            }
            ...
            inline static double
            S_2(const double & x, const double & omega)
            {
            ...
            inline static double
            S_2_5(const double & x, const double & omega)
            {
                return +omega * (14.0 / 27.0 + 10.0 / 9.0 * pow(x, -6.0 / 25.0) - 44.0 / 27.0 * pow(x, -9.0 / 25.0))
                       + (2.0 / 3.0 + 5.0 / 9.0 * pow(x, -6.0 / 25.0) + 2.0 / 9.0 * pow(x, -9.0 / 25.0) - 13.0 / 9.0 * pow(x, -12.0 / 25.0));
            }
            ...
            // h_2(z, omega) is defined in [N:1993A] eq. (3.129), p. 70
            inline static double
            h_2(const double & z, const double & omega)
            {
                const double denom = 1.0 - 2.0 * omega * z + z * z;

                return z / power_of<2>(denom)
                               * (2.0 * (omega - 1.0) * z * (1.0 + z) * log(z)
                                  - ((omega + 1.0) - 2.0 * omega * (2.0 * omega + 1.0) * z + (5.0 * omega + 2.0 * omega * omega - 1.0) * z * z - 2.0 * z * z * z) * r(omega))
                       - z / denom * (log(z) - 1.0 + z);
            }

            // h_2_5(z, omega) is defined in [N:1993A] eq. (3.129), p. 70
            inline static double
            h_2_5(const double & z, const double & omega)
            {
                const double denom = 1.0 - 2.0 * omega * z + z * z;
            ...
            // cf. [N:1993A] eq. (3.142), p. 74
            double
            Chat_1_v(const double & omega) const
            {
                ...
                return A(omega)
                       * (1.0 + (alpha_s_mb - alpha_s_mc) / M_PI * Z_4 + alpha_s_mc / M_PI * (Z_hh(omega) + 2.0 / 3.0 * (f(omega) + r(omega))) + z * S_1(x, omega)
                          + 2.0 * alpha_s_m / (3.0 * M_PI) * G);
            }

            double
            Chat_2_v(const double & omega) const
            {
                const double alpha_s_mb = model->alpha_s(m_b_msbar());
                const double alpha_s_mc = model->alpha_s(m_c_msbar());
                const double alpha_s_m  = model->alpha_s(mu_match());
                ...
                return A(omega) * (+2.0 * alpha_s_mb / (3.0 * M_PI) - 4.0 * alpha_s_mc / (3.0 * M_PI) * r(omega) + z * S_2(x, omega) - 2.0 * alpha_s_m / (3.0 * M_PI) * H1);
            }

            double
            Chat_3_v(const double & omega) const
            {
                const double alpha_s_mb = model->alpha_s(m_b_msbar());
                const double alpha_s_mc = model->alpha_s(m_c_msbar());
                const double alpha_s_m  = model->alpha_s(mu_match());
                ...
                return -A(omega) * (+z * S_3 + 2.0 * alpha_s_m / (3.0 * M_PI) * H2);
            }

            double
            Chat_1_a(const double & omega) const
            {
                static const double Z_4 = -9403.0 / 7500.0 - 7.0 * M_PI * M_PI / 225.0;
                ...
                return A(omega)
                       * (1.0 + (alpha_s_mb - alpha_s_mc) / M_PI * Z_4 + alpha_s_mc / M_PI * (Z_hh(omega) + 2.0 / 3.0 * (f(omega) - r(omega))) + z * S_1_5(x, omega)
                          + 2.0 * alpha_s_m / (3.0 * M_PI) * G);
            }

            double
            Chat_2_a(const double & omega) const
            {
                const double alpha_s_mb = model->alpha_s(m_b_msbar());
                const double alpha_s_mc = model->alpha_s(m_c_msbar());
                const double alpha_s_m  = model->alpha_s(mu_match());
                ...
                return A(omega) * (-2.0 * alpha_s_mb / (3.0 * M_PI) - 4.0 * alpha_s_mc / (3.0 * M_PI) * r(omega) + z * S_2_5(x, omega) - 2.0 * alpha_s_m / (3.0 * M_PI) * H1);
            }

            double
            Chat_3_a(const double & omega) const
            {
                const double alpha_s_mb = model->alpha_s(m_b_msbar());
                const double alpha_s_mc = model->alpha_s(m_c_msbar());
                const double alpha_s_m  = model->alpha_s(mu_match());
                ...
                return +A(omega) * (+z * S_3_5(x) + 2.0 * alpha_s_m / (3.0 * M_PI) * H2);
            }
    ...
    const std::set<ReferenceName> HQETBToC::references{};
} // namespace eos

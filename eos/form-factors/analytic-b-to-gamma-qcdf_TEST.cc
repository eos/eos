        ...
        virtual void run() const
        {
            // Regression tests at the inputs of [BBJW:2018A], Table 1, with sigmahat_1 = 0.
            {
                    ...
                    } })
                {
                    ...
                    {
                        const Options common{ { "lcda-model"_ok, qnp::OptionValue(lcda_model) } };
                        AnalyticFormFactorPToGammaQCDF<BToGamma> ff_none(p, common + Options{ { "contributions"_ok, "none"_ov } });
                ...
                TEST_CHECK_NO_THROW(AnalyticFormFactorPToGammaQCDF<BToGamma>(p, Options{
                            { "lcda-model"_ok, "FLvD2022+profile"_ov }, { "contributions"_ok, "soft-tw-3+4"_ov }
                        }));
                ...
                // defined by each model, so this compares two implementations of one object.
                {
                    ...
                    {
                        const Options common{ { "lcda-model"_ok, qnp::OptionValue(lcda_model) } };
                        AnalyticFormFactorPToGammaQCDF<BToGamma> ff_none(q, common + Options{ { "contributions"_ok, "none"_ov } });
                        AnalyticFormFactorPToGammaQCDF<BToGamma> ff_tw(q, common + Options{ { "contributions"_ok, "soft-tw-5+6"_ov } });
                ...
                {
                    AnalyticFormFactorPToGammaQCDF<BToGamma> ff_profile_all(p, Options{
                            { "lcda-model"_ok, "FLvD2022+profile"_ov }, { "contributions"_ok, "all"_ov }
                        });
                    AnalyticFormFactorPToGammaQCDF<BToGamma> ff_profile_none(p, Options{
                            { "lcda-model"_ok, "FLvD2022+profile"_ov }, { "contributions"_ok, "none"_ov }
                        });
                    AnalyticFormFactorPToGammaQCDF<BToGamma> ff_profile_tw_3_4(p, Options{
                            { "lcda-model"_ok, "FLvD2022+profile"_ov }, { "contributions"_ok, "soft-tw-3+4"_ov }
                        });
                    ...
                    TEST_CHECK_NEARLY_EQUAL(ff_profile_all.F_V(2.0) - ff_flvd_all.F_V(2.0),
                            ff_profile_tw_3_4.F_V(2.0) - ff_profile_none.F_V(2.0), 1e-12);
                    TEST_CHECK_NEARLY_EQUAL(ff_profile_all.F_A(2.0) - ff_flvd_all.F_A(2.0),
                            ff_profile_tw_3_4.F_A(2.0) - ff_profile_none.F_A(2.0), 1e-12);
                ...
                {
                    const Options common{ { "lcda-model"_ok, qnp::OptionValue(lcda_model) } };
                    AnalyticFormFactorPToGammaQCDF<BToGamma> ff_sum_none(p, common + Options{ { "contributions"_ok, "none"_ov } });
                    const double F_V_none = ff_sum_none.F_V(2.0);

                    const auto term = [&] (const char * contribution) -> double
                    {
                        AnalyticFormFactorPToGammaQCDF<BToGamma> ff(p, common + Options{ { "contributions"_ok, qnp::OptionValue(contribution) } });
                ...
                for (const auto & [lcda_model, contributions, expected] :
                        std::array<std::tuple<const char *, const char *, std::array<double, 4>>, 20>{ {
                ...
                for (const auto & [lambda_B, expected] : std::array<std::pair<double, double>, 3>{ {
                        { 0.20, +0.006630321674412021 },
                        { 0.35, +0.004272383522116230 },
                        { 0.50, +0.003003337577852400 }
                    } })
                {
                    p["B_u::omega_0@FLvD2022"] = lambda_B;
                    const Options common{ { "lcda-model"_ok, "exponential"_ov } };
                    ...
                    const double delta_xi_soft = 0.5 * ((ff_all.F_V(2.0) - ff_all.F_A(2.0))
                        - (ff_ht.F_V(2.0) - ff_ht.F_A(2.0)));
            ...
            // Independent numerical reference
            {
                ...
                AnalyticFormFactorPToGammaQCDF<BToGamma> ff_profile(p, Options{
                ...
                TEST_CHECK_NEARLY_EQUAL( ff_profile.F_V(k["E_gamma"]) - ff.F_V(k["E_gamma"]),
                        ff_profile_tw_3_4.F_V(k["E_gamma"]) - ff_profile_none.F_V(k["E_gamma"]), 1e-12 );
                TEST_CHECK_NEARLY_EQUAL( ff_profile.F_A(k["E_gamma"]) - ff.F_A(k["E_gamma"]),
                        ff_profile_tw_3_4.F_A(k["E_gamma"]) - ff_profile_none.F_A(k["E_gamma"]), 1e-12 );
            }

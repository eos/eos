    ...
    public:
        ...
        virtual void run() const
        {
            // Independent numerical reference
            {
                ...
                TEST_CHECK_NEARLY_EQUAL( ff.F_V(k["E_gamma"]),      0.2320700136016959,     1e-8 );
                TEST_CHECK_NEARLY_EQUAL( obs_F_V->evaluate(),       0.2320700136016959,     1e-8 );

                TEST_CHECK_NEARLY_EQUAL( ff.F_A(k["E_gamma"]),      0.2020508835188878,     1e-8 );
                TEST_CHECK_NEARLY_EQUAL( obs_F_A->evaluate(),       0.2020508835188878,     1e-8 );


                // Math integrity test: cross-check complete form factors against Mathematica implementation

                TEST_CHECK_NEARLY_EQUAL( ff.F_V(4.0),               0.112285762382138,     1e-8 );
                TEST_CHECK_NEARLY_EQUAL( ff.F_V(12.0),              0.02616884028923805,   1e-8 );

                TEST_CHECK_NEARLY_EQUAL( ff.F_A(4.0),               0.1094956492529053,    1e-8 );
                TEST_CHECK_NEARLY_EQUAL( ff.F_A(12.0),              0.02852642445686912,   1e-8 );
            }
        }

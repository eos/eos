...
namespace eos
{
    ...
    {
        static double
        rho_1(const double & s, const double & mb, const double & mu)
        {
            ...
            double result = s / 2 * (1.0 - x)
                            * ((1.0 - x) * (4.0 * re_li2_x + 2.0 * lnx * ln1mx - (5.0 - 2.0 * x) * ln1mx) + (1.0 - 2.0 * x) * (3.0 - x) * lnx + 3.0 * (1.0 - 3.0 * x) * 2.0 * lnmumb
                               + (17.0 - 33.0 * x) / 2.0);
        ...
        static double
        delta_1(const double & mb, const double & mu, const double & Mprime2)
        {
        ...
        static double
        delta_1_Mprime2_deriv(const double & mb, const double & mu, const double & Mprime2)
        {
            const double mb2 = mb * mb, mu2 = mu * mu;
            const double gamma = gsl_sf_gamma_inc(0.0, mb2 / Mprime2);
    ...
    template <QuarkFlavor q1_, QuarkFlavor q2_, QuarkFlavor qs_> struct DKMMO2008Base;

    // B^- -> pi^0
    template <> struct DKMMO2008Base<QuarkFlavor::bottom, QuarkFlavor::up, QuarkFlavor::down>
    {
            ...
            double
            m_q_msbar(const double & mu) const
            {
                return this->model->m_d_msbar(mu);
            }

            double
            decay_constant_power_correction() const
            {
                ...
                const double result = -mb * cond_qq_mu * (1.0 + 4.0 * alpha_s_mu / (3.0 * pi) * dkmmo2008::delta_1(mb, mu, Mprime2))
                                      - mb * cond_qq_1 * m02 / (2.0 * Mprime2) * (1.0 - mb2 / (2 * Mprime2)) + cond_GG / 12.0
            ...
            double
            decay_constant_power_correction_Mprime2_deriv() const
            {
                static const double pi = M_PI;
                ...
                const double result = -mb * cond_qq_mu * 4.0 * alpha_s_mu / (3.0 * pi) * dkmmo2008::delta_1_Mprime2_deriv(mb, mu, Mprime2)
    ...
    template <> struct DKMMO2008Base<QuarkFlavor::bottom, QuarkFlavor::up, QuarkFlavor::strange>
    {
            ...
            decay_constant_power_correction_Mprime2_deriv() const
            {
                ...
                const double result = -mbplusmq2 / mb * cond_qq_mu * 4.0 * alpha_s_mu / (3.0 * pi) * dkmmo2008::delta_1_Mprime2_deriv(mb, mu, Mprime2)
                                      - mb * cond_qq_1 * m02 / (2.0 * Mprime2) * (mb2 - Mprime2)
                                      + 16.0 * pi * alpha_s_1 * cond_qq_1 * cond_qq_1 * r_vac / (27.0 * 4.0 * Mprime4) * (4.0 * Mprime4 - 2.0 * Mprime2 * mb2 - mb4);
    ...
    struct Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>> : public DKMMO2008Base<q1_, q2_, qs_>
    {
            ...
            _decay_constant_sum_rule() const
            {
                ...
                std::function<double(const double &)> integrand([&](const double & s) -> double
                ...
                double result = std::exp(MB2 / Mprime2) / MB4 * (3.0 * mb2 / (8.0 * pi2) * integral + mb2 * std::exp(-mb2 / Mprime2) * (this->decay_constant_power_correction()));
            ...
            MB_svz() const
            {
                ...
                std::function<double(const double &)> integrand_numerator([&](const double & s) -> double
                ...
                std::function<double(const double &)> integrand_denominator([&](const double & s) -> double
                { return std::exp(-s / Mprime2) * ((s - mb2) * (s - mb2) / s + 4.0 * alpha_s_mu / (3.0 * pi) * dkmmo2008::rho_1(s, mb, mu)); });
                const double                          integral_denominator = integrate<GSL::QAGS>(integrand_denominator, (mb + mq) * (mb + mq) + eps, sprime0B, config);

                double numerator = 3.0 * mb2 / (8.0 * pi2) * integral_numerator + mb4 * std::exp(-mb2 / Mprime2) * (this->decay_constant_power_correction())
                                   + mb2 * std::exp(-mb2 / Mprime2) * (this->decay_constant_power_correction_Mprime2_deriv());
                double denominator = 3.0 * mb2 / (8.0 * pi2) * integral_denominator + mb2 * std::exp(-mb2 / Mprime2) * (this->decay_constant_power_correction());

                return std::sqrt(numerator / denominator);
            }

            double
            F_lo_tw2_integrand(const double & u, const double & q2, const double _M2, const double & _select_weight) const
            {
            ...
            double
            F_lo_tw2(const double & q2, const double & _M2, const double & _select_weight = 0.0, const double & _select_corr = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double s0 = s0B(q2) * (1.0 - _select_corr) + s0tilB(q2) * _select_corr;
                const double u0 = std::max(1e-10, (mb2 - q2) / (s0 - q2));

                std::function<double(const double &)> integrand(std::bind(&Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::F_lo_tw2_integrand,
                                                                          this,
                                                                          std::placeholders::_1,
                                                                          q2,
                                                                          _M2,
            ...
            double
            F_lo_tw3_integrand(const double & u, const double & q2, const double & _M2, const double & _select_weight) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP;
                ...
                auto I3bar = [&](const double & u) -> double
                {
                    ...
                    return 5.0 / 2.0 * u3 * ubar2 * (-12.0 + 24.0 * u - (3.0 + -6.0 * u) * omega3 + (6.0 - 28.0 * u + 28.0 * u2) * lambda3);
                };
                auto I3bar_d1 = [&](const double & u) -> double
                {
                    ...
                    return 15.0 / 2.0 * u2 * (1.0 - u)
                           * (-12.0 * (3.0 - 13.0 * u + 12.0 * u2) + (-9.0 + 39.0 * u - 36.0 * u2) * omega3 + 2.0 * (9.0 - 71.0 * u + 154.0 * u2 - 98.0 * u3) * lambda3);
                };

                const double u2   = u * u;
                const double tw3a = lcdas->phi3p(u, mu)
                                    + (lcdas->phi3s(u, mu) / u - (mb2 + q2 - u2 * mP2) / (2 * (mb2 - q2 + u2 * mP2)) * lcdas->phi3s_d1(u, mu)
                                       - (2 * u * mP2 * mb2) / power_of<2>(mb2 - q2 + u2 * mP2) * lcdas->phi3s(u, mu))
                                              / 3.0;
                const double tw3b = 2.0 / u * (mb2 - q2 - u2 * mP2) / (mb2 - q2 + u2 * mP2) * (I3_d1(u) - (2.0 * u * mP2) / (mb2 - q2 + u2 * mP2) * I3(u));
                const double tw3c = 3.0 * mP2 / (mb2 - q2 + u2 * mP2) * (I3bar_d1(u) - (2.0 * u * mP2) / (mb2 - q2 + u2 * mP2) * I3bar(u));
                ...
                return std::exp(-(mb2 - q2 * (1.0 - u) + mP2 * u * (1.0 - u)) / (u * _M2)) * weight * (mu3 / mb * tw3a - lcdas->f3(mu) / (mb * fP) * (tw3b + tw3c));
            }

            double
            F_lo_tw3(const double & q2, const double & _M2, const double & _select_weight = 0.0, const double & _select_corr = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double s0 = s0B(q2) * (1.0 - _select_corr) + s0tilB(q2) * _select_corr;
                const double u0 = std::max(1e-10, (mb2 - q2) / (s0 - q2));

                std::function<double(const double &)> integrand(std::bind(&Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::F_lo_tw3_integrand,
                                                                          this,
                                                                          std::placeholders::_1,
                                                                          q2,
                                                                          _M2,
            ...
            double
            F_lo_tw4(const double & q2, const double & _M2, const double & _select_weight = 0.0, const double & _select_corr = 0.0) const
            {
                ...
                auto I4 = [&](const double & u) -> double
                {
                    ...
                    return -1.0 / 24.0 * u * ubar
                           * (mP2 * (54.0 * u3 - 81.0 * u2 + 27.0 * ubar + 27.0 * a2pi * (16.0 * u3 - 29.0 * u2 + 13.0 * u - 1.0)) + 16.0 * u * (20.0 * u - 30.0) * delta4);
                };
                auto I4_d1 = [&](const double & u) -> double
                {
                    ...
                    return 1.0 / 24
                           * (27.0 * mP2 * ((10.0 * u4 - 20.0 * u3 + 6.0 * u2 + 4.0 * u - 1.0) + a2pi * (80.0 * u4 - 180.0 * u3 + 126.0 * u2 - 28.0 * u + 1))
                ...
                auto I4bar = [&](const double & u) -> double
                {
                    ...
                    return 1.0 / 48.0 * u * ubar
                           * (mP2 * (-(54.0 * u3 - 81.0 * u2 - 27.0 * u + 27.0) + 27.0 * a2pi * (32.0 * u3 - 43.0 * u2 + 11.0 * u + 1.0))
                              - 20.0 * u * ((12.0 - 20.0 * u) + (378.0 * u2 - 567.0 * u + 189.0) * omega4) * delta4);
                };
                auto I4barI = [&](const double & u) -> double
                {
                    ...
                    return 1.0 / 96.0 * u2 * ubar2
                           * (mP2 * (9.0 * (3.0 + 2.0 * ubar * u) + 9.0 * a2pi * (32.0 * u2 - 26.0 * u - 3.0)) + 40.0 * u * (4.0 + 63.0 * ubar * omega4) * delta4);
                };
                auto I4bar_d1 = [&](const double & u) -> double
                {
                    ...
                    return 1.0 / 48.0
                           * (27.0 * mP2 * ((10.0 * u4 - 20.0 * u3 + 6.0 * u2 + 4.0 * u - 1.0) - a2pi * (160.0 * u4 - 300.0 * u3 + 162.0 * u2 - 20.0 * u - 1.0))
                              + 40.0 * u * ((-40.0 * u2 + 48.0 * u - 12.0) + 189.0 * (5.0 * u3 - 10.0 * u2 + 6.0 * u - 1.0) * omega4) * delta4);
                };
                std::function<double(const double &)> integrand([&](const double & u) -> double
                {
                    const double u2 = u * u;

                    const double tw4psi = u * lcdas->psi4(u, mu) + (mb2 - q2 - u2 * mP2) / (mb2 - q2 + u2 * mP2) * lcdas->psi4_i(u, mu);
                    const double tw4phi = (lcdas->phi4_d2(u, mu) - 6.0 * u * mP2 / (mb2 - q2 + u2 * mP2) * lcdas->phi4_d1(u, mu)
                    ...
                    return std::exp(-(mb2 - q2 * (1.0 - u) + mP2 * u * (1.0 - u)) / (u * _M2)) * weight * (tw4psi - tw4phi - tw4I4 - tw4I4bar1 - tw4I4bar2) / (mb2 - q2 + u2 * mP2);
                });
            ...
            double
            F_nlo_tw2(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                ...
                auto T1tw2theta1mrho = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca2 =
                            power_of<2>(r1 - r2) * ((-125.0 + r1 * 155.0 - r12 * 43.0 + r13) + r2 * (220.0 - r1 * 224.0 + r12 * 40.0) + r22 * (-108.0 + 72.0 * r1) + r23 * 12.0);
                    const double ca4 = (-3087.0 + r1 * 6804.0 - r12 * 5096.0 + r13 * 1484.0 - r14 * 136.0 + r15)
                                       + r2 * (8631.0 - 17024.0 * r1 + 10836.0 * r12 - 2424.0 * r13 + 131.0 * r14) + r22 * (-8750.0 + 14700.0 * r1 - 7200.0 * r12 + 950.0 * r13)
                                       + r23 * (3850.0 - r1 * 5000.0 + r12 * 1450.0) + r24 * (-675.0 + r1 * 525.0) + r25 * 30.0;

                    const double cb0 = power_of<4>(r1 - r2);
                    const double cb2 = power_of<2>(r1 - r2) * (15.0 - r1 * 10.0 + r12 + r2 * (-20.0 + r1 * 8.0) + r22 * 6.0);
                    const double cb4 = (210.0 - r1 * 336.0 + r12 * 168.0 - r13 * 28.0 + r14) + r2 * (-504.0 + r1 * 672.0 - r12 * 252.0 + r13 * 24.0)
                                       + r22 * (420.0 - r1 * 420.0 + r12 * 90.0) + r23 * (-140.0 + r1 * 80.0) + r24 * 15.0;

                    return ((r1 - r2) * (L - 1.0 / r2) * (ca0 + ca2 * a2pi + ca4 * a4pi) + (r1 - 1.0) * (1.0 / r2 - 1.0) * (r2 - r1) * (cb0 + cb2 * a2pi + cb4 * a4pi)
                ...
                auto T1tw2thetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double r22 = r2 * r2, r23 = r22 * r2, r24 = r22 * r22, r25 = r24 * r2, r26 = r23 * r23, r27 = r24 * r23, r28 = r24 * r24;
                    const double Lr2 = std::log(r2), Lr2m1 = std::log(r2 - 1.0), Lmu = std::log(mb2 / (mu * mu));

                    const double ca00    = (-r1 * 4. + r12 * 4.) + r2 * (3. + r1 * 12. - r12 * 12.) + r22 * (-13. - r1 * 4. + r12 * 8.) + r23 * (13. - r1 * 4.) - r24 * 3.;
                    const double ca0mu   = r2 * (1. - r1 * 3. + r12 * 2.) + r22 * (r1 * 2. - r12 * 2.) + r23 * (-1. + r1);
                    const double ca0r2   = r2 * (-1. + r12) + r22 * (3. - r1 * 4. + r12);
                    const double ca0r2m1 = 2.0 * ca0mu;

                    const double ca20 = (r1 * 1680. - r12 * 3120 + r13 * 1728 - r14 * 288.) + r2 * (-1500. - r1 * 8675. + r12 * 17308. - r13 * 8208. + r14 * 864.)
                                        + r22 * (10895. + r1 * 2160. - r12 * 21084. + r13 * 10080. - r14 * 576.) + r23 * (-19396. + r1 * 15264. + r12 * 5412. - r13 * 3600.)
                                        + r24 * (12516. - r1 * 12880. + r12 * 1484.) + r25 * (-2576. + r1 * 2451.) + r26 * 61.;
                    const double ca2mu = r2 * (-180. + r1 * 1740. - r12 * 2712. + r13 * 1296. - r14 * 144.) + r22 * (-840. - r1 * 1536. + r12 * 4248. - r13 * 2016. + r14 * 144.)
                                         + r23 * (2448. - r1 * 1944. - r12 * 1224. + r13 * 720.) + r24 * (-1800. + r1 * 2112. - r12 * 312.) + r25 * (372. - r1 * 372.);
                    const double ca2r2 = r2 * (180. + r1 * 840. - r12 * 1728. + r13 * 720. - r14 * 72.) + r22 * (-1740. + r1 * 1536. + r12 * 144. + r13 * 432. - r14 * 72.)
                                         + r23 * (1992. - r1 * 2448. + r12 * 1512. - r13 * 576.) + r24 * (-216. - r1 * 672. + r12 * 168.) + r25 * (-300. + r1 * 300.);
                    ...
                    const double ca40 = r1 * 98910. - r12 * 281610. + r13 * 294000. - r14 * 136500. + r15 * 27000. - r16 * 1800.
                                        ...
                                        + r25 * (-1293760. + r1 * 2102595. - r12 * 890655. + r13 * 63725.) + r26 * (307725. - r1 * 414708. + r12 * 137664.)
                                        + r27 * (-23987. + r1 * 23980) + r28 * 181.;
                    const double ca4mu = r2 * (-6300. + r1 * 107730. - r12 * 271530. + r13 * 266700. - r14 * 115950. + r15 * 20250. - r16 * 900.)
                                         ...
                                         + r25 * (169650. - r1 * 304200. + r12 * 147150. - r13 * 12600.) + r26 * (-40950. + r1 * 62820. - r12 * 21870.)
                                         + r27 * (3180. - r1 * 3180.);
                    const double ca4r2 = r2 * (6300. + r1 * 63630. - r12 * 204750. + r13 * 210000. - r14 * 87750. + r15 * 12600. - r16 * 450.)
                                         ...
                                         + r24 * (-172200. + r1 * 300600. - r12 * 165600. + r13 * 71400. - r14 * 23700.) + r25 * (34050. - r1 * 16650. - r12 * 54900. + r13 * 8100.)
                                         + r26 * (8100. - r1 * 38520. + r12 * 17820.) + r27 * (-2730. + r1 * 2730.);
                ...
                // clang-format on
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    // _select_weight:
                    //  0.0 -> regular integral
                    //  1.0 -> integral of derivative w.r.t. -1/M^2
                    ...
                    return -2.0 * (T1tw2thetarhom1(r1, r2) + T1tw2theta1mrho(r1, r2) + T1tw2delta(r1, r2)) * weight * std::exp(-mb2 * r2 / _M2);
                });
            ...
            double
            F_nlo_tw3(const double & q2, const double _M2, const double & _select_weight = 0.0) const
            {
                // Reminder: q2 is the kinematic variable associated with the momentum
                // transfer, while s is the kinematic variable in which the function is
                // analytically continued. See also comment at beginning of Appendix B
                ...
                auto T1tw3pthetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double dl2   = std::real(-dilog(r1 / r2) + dilog(r1) - 2.0 * dilog((r2 - 1.0) / (r1 - 1.0))) - logr2 * logr2 / 2.0 + logr2 * std::log(r2 - r1)
                                       - 2.0 * std::log((r2 - r1) / (1.0 - r1)) * std::log(r2 - 1.0);

                    return (dl1 * (1.0 + r1 + r2) + dl2 * (4.0 * r1 - 1.0) + ((r1 + r2) * (r2 - 1.0) + (r1 * (2.0 - 3.0 * r2) + r2) * logr2) / (2.0 * r2)
                ...
                auto T1tw3pdeltarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    return (6.0 - 2.0 * r1 - pi2 / 6.0 * (1.0 + 4.0 * r1) + lr2 * (l1mr1 * r1 - lr2m1 * 2.0 * r1)
                            + lr2m1 * (lr2m1 * (1.0 + 2.0 * r1) - 4.0 + 2.0 * r1 * (r2 - 1.0) / r2 - l1mr1 * 2.0 * r1 + lmu * (1.0 + r1)) + lmu * 3.0 / 2.0 * (r1 - 3.0)
                ...
                auto T1tw3sigmadeltarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    return (dl1 * 6.0 * (r1 * (3.0 - 4.0 * r2) + r2) + dl2 * (-30.0 * r2 + 6.0 * r1 * (-7.0 + 2.0 * r1 + 10.0 * r2))
                            + l1 * l2 * (-12.0 * r2 + 6.0 * r1 * (-2.0 + r1 + 3.0 * r2))
                            + lr2m1 * (lmu * (-18.0 * r2 + 6.0 * r1 * (-5.0 + r1 + 7.0 * r2)) - 12.0 * (r2 + r1 * (2.0 - r1 - 3.0 * r2 + r2 * r2)) / r2)
                            - l1mr1 * 6.0 * ((-2.0 + r1) * r1 - 2.0 * r2 + r1 * (5.0 + r1) * r2 + (2.0 - 5.0 * r1) * r2 * r2) / r2
                            + lmu * (-3.0 * r1 * (-17.0 + r1 - 5.0 * r2) + 9.0 * r2) + r1 * (-72.0 + pi2 * (-5.0 + 4.0 * r1))
                            + r2 * (6.0 * (-1.0 + r1) * r1 + pi2 * (-7.0 + 8.0 * r1)) - 6.0 * (1.0 + 3.0 * r2))
                           / ((r1 - r2) * (r1 - r2) * (r1 - r2));
                };
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    ...
                    return (2.0 / (r2 - r1) * (T1tw3pthetarhom1(r1, r2) + T1tw3ptheta1mrho(r1, r2) + T1tw3pdeltarhom1(r1, r2))
...
// for diagnostics purposes only
            ...
            std::function<double (const double &)> iT1tw3sigmadeltarhom1(
                ...
                return fP * mu3 * mb
                       * (integrate<GSL::QAGS>(integrand, 1.0 + eps, s0B(q2) / mb2, config)
                          - (2.0 / (1.0 - r1) * (4.0 - 3.0 * lmu) + 2.0 * (1.0 + r1) / power_of<2>(1.0 - r1) * (4.0 - 3.0 * lmu)) * weight * std::exp(-mb2 / _M2));
            }

            // expressions for the \tilde{F}

            double
            Ftil_lo_tw3_integrand(const double & u, const double & q2, const double _M2, const double _select_weight) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP;
                const double mu3     = lcdas->mu3(mu);
                const double omega3  = lcdas->omega3(mu);
                ...
                auto I3til = [&](const double & u) -> double
                {
                    ...
                    return 5.0 / 2.0 * u2 * ubar2 * (36.0 - 24.0 * u + (9.0 - 34.0 * u + 28.0 * u2) * omega3 + (-18.0 + 52.0 * u - 28.0 * u2) * lambda3);
                };
                auto I3til_d1 = [&](const double & u) -> double
                {
                    ...
                    return 15.0 * u * (1.0 - u)
                           * (4.0 * (3.0 - 9.0 * u + 5.0 * u2) + (3.0 - 23.0 * u + 47.0 * u2 - 28.0 * u3) * omega3 + (-6.0 + 38.0 * u - 62.0 * u2 + 28.0 * u3) * lambda3);
                };

                const double u2   = u * u;
                const double tw3a = lcdas->phi3p(u, mu) / u + 1 / (6 * u) * lcdas->phi3s_d1(u, mu);
                const double tw3b = mP2 / (mb2 - q2 + u2 * mP2) * (I3til_d1(u) - (2.0 * u * mP2) / (mb2 - q2 + u2 * mP2) * I3til(u));
                ...
                return std::exp(-(mb2 - q2 * (1.0 - u) + mP2 * u * (1.0 - u)) / (u * _M2)) * weight * (mu3 / mb * tw3a + lcdas->f3(mu) / (mb * fP) * tw3b);
            }

            double
            Ftil_lo_tw3(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double u0 = std::max(1e-10, (mb2 - q2) / (s0tilB(q2) - q2));

                std::function<double(const double &)> integrand(std::bind(&Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::Ftil_lo_tw3_integrand,
                                                                          this,
                                                                          std::placeholders::_1,
                                                                          q2,
                                                                          _M2,
            ...
            double
            Ftil_lo_tw4(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP, mP4 = mP2 * mP2;
                const double u0     = std::max(1e-10, (mb2 - q2) / (s0tilB(q2) - q2));
                const double a2pi   = lcdas->a2(mu);
                ...
                std::function<double(const double &)> integrand([&](const double & u) -> double
                {
                    ...
                    const double tw4I4bar = (-I4bar_d1(u) + (6.0 * u * mP2) / (mb2 - q2 + u2 * mP2) * I4bar(u) + (12.0 * u2 * mP4) / power_of<2>(mb2 - q2 + u2 * mP2) * I4barI(u))
                                            * 2.0 * u * mP2 / (mb2 - q2 + u2 * mP2);
                    ...
                    return std::exp(-(mb2 - q2 * (1.0 - u) + mP2 * u * (1.0 - u)) / (u * _M2)) * weight * (tw4psi + tw4I4bar) / (mb2 - q2 + u2 * mP2);
                });
            ...
            double
            Ftil_nlo_tw2(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                // Reminder: q2 is the kinematic variable associated with the momentum
                // transfer, while s is the kinematic variable in which the function is
                // analytically continued. See also comment at beginning of Appendix B
                ...
                auto T1tiltw2theta1mrho = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca0 = -r1 + 2.0 * r12 - r13 + r2 * (1.0 - r1 - r12 + r13) + r22 * (-1.0 + 2.0 * r1 - r12);
                    const double ca2 = -15.0 + 40.0 * r1 - 36.0 * r12 + 12.0 * r13 - r14 + r2 * (35.0 - 88.0 * r1 + 72.0 * r12 - 20.0 * r13 + r14)
                                       + r22 * (-26.0 + 60.0 * r1 - 42.0 * r12 + 8.0 * r13) + r23 * (6.0 - 12.0 * r1 + 6.0 * r12);
                    const double ca4 = -210.0 + 756.0 * r1 - 1050.0 * r12 + 700.0 * r13 - 225.0 * r14 + 30.0 * r15 - r16
                                       ...
                                       + r23 * (560.0 - 1620.0 * r1 + 1650.0 * r12 - 680.0 * r13 + 90.0 * r14) + r24 * (-155.0 + 390.0 * r1 - 315.0 * r12 + 80.0 * r13)
                                       + r25 * (15.0 - 30.0 * r1 + 15.0 * r12);

                    return -6.0 / (r2 * power_of<7>(r1 - r2)) * (power_of<3>(r1 - r2) * ca0 + power_of<2>(r1 - r2) * ca2 * a2pi + ca4 * a4pi);
                };
                auto T1tiltw2thetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca00  = 1 - 2.0 * r1 + r2 * (-1.0 + 4.0 * r1) + r22 * (-1.0 - 2.0 * r1) + r23;
                    const double ca0r2 = -r2 * r1 + r22 * (1.0 + r1) - r23;

                    const double ca20 = (15.0 - 40.0 * r1 + 36.0 * r12 - 12.0 * r13) + r2 * (-35.0 + 93.0 * r1 - 87.0 * r12 + 24.0 * r13)
                                        + r22 * (21.0 - 45.0 * r1 + 96.0 * r12 - 12.0 * r13) + r23 * (-6.0 - 29.0 * r1 - 45.0 * r12) + r24 * (-16.0 + 21.0 * r1) + r25 * (21.0);
                    const double ca2r2 = r2 * (-6 * r13) + r22 * (6.0 * r13 + 18.0 * r12) + r23 * (12.0 * r1 + 12.0 * r12) + r24 * (-24.0 - 12.0 * r1) + r25 * (-6.0);
                    ...
                    const double ca40 = 420.0 - 1512.0 * r1 + 2100.0 * r12 - 1400.0 * r13 + 450.0 * r14 - 60.0 * r15
                                        ...
                                        + r23 * (-1015.0 + 2820.0 * r1 - 2040.0 * r12 + 2240.0 * r13 - 780.0 * r14) + r24 * (450.0 - 1200.0 * r1 - 1080.0 * r12 - 1320.0 * r13)
                                        + r25 * (-660.0 - 243.0 * r1 + 630.0 * r12) + r26 * (313.0 + 975.0 * r1) + r27 * (135.0);
                    const double ca4r2 = r2 * (-15.0 * r15) + r22 * (75.0 * r14 + 15.0 * r15) + r23 * (690.0 * r13 + 135.0 * r14) + r24 * (150.0 * r12 + 150.0 * r13)
                                         + r25 * (-705.0 * r1 - 150.0 * r12) + r26 * (-195.0 - 135.0 * r1) + r27 * (-15.0);

                    return -6.0 / (r2 * power_of<7>(r1 - r2))
                           * (power_of<4>(r1 - r2) * (ca00 + ca0r2 * Lr2) + power_of<2>(r1 - r2) * (ca20 + ca2r2 * Lr2) * a2pi + (ca40 / 2.0 + ca4r2 * Lr2) * a4pi);
                };
                auto T1tiltw2delta = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca0r1 = r1 - 2.0 * r12 + r13 + r2 * (-1.0 + r1 + r12 - r13) + r22 * (1.0 - 2 * r1 + r12);

                    const double ca20 = 5.0 * r1 - 10.0 * r12 + 6.0 * r13 - r14 + r2 * (-5.0 + 12.0 * r12 - 8.0 * r13 + r14) + r22 * (10.0 - 12.0 * r1 + 2.0 * r13)
                                        + r23 * (-6.0 + 8.0 * r1 - 2.0 * r12) + r24 * (1.0 - r1);
                    const double ca2r1 = 5.0 * r1 - 15.0 * r12 + 16.0 * r13 - 7.0 * r14 + r15 + r2 * (-5.0 + 5.0 * r1 + 12.0 * r12 - 20.0 * r13 + 9.0 * r14 - r15)
                                         + r22 * (10.0 - 22.0 * r1 + 12.0 * r12 + 2.0 * r13 - 2.0 * r14) + r23 * (-6.0 + 14.0 * r1 - 10.0 * r12 + 2.0 * r13)
                                         + r24 * (1.0 - 2.0 * r1 + r12);

                    const double ca40 = 42.0 * r1 - 126.0 * r12 + 140.0 * r13 - 70.0 * r14 + 15.0 * r15 - r16
                                        + r2 * (-42.0 + 210.0 * r12 - 280.0 * r13 + 135.0 * r14 - 24.0 * r15 + r16)
                                        + r22 * (126.0 - 210.0 * r1 + 150.0 * r13 - 75.0 * r14 + 9.0 * r15) + r23 * (-140.0 + 280.0 * r1 - 150.0 * r12 + 10.0 * r14)
                                        + r24 * (70.0 - 135.0 * r1 + 75.0 * r12 - 10.0 * r13) + r25 * (-15.0 + 24.0 * r1 - 9.0 * r12) + r26 * (1.0 - r1);
                    const double ca4r1 = 42.0 * r1 - 168.0 * r12 + 266.0 * r13 - 210.0 * r14 + 85.0 * r15 - 16.0 * r16 + r17
                                         ...
                                         + r24 * (70.0 - 205.0 * r1 + 210.0 * r12 - 85.0 * r13 + 10.0 * r14) + r25 * (-15.0 + 39.0 * r1 - 33.0 * r12 + 9.0 * r13)
                                         + r26 * (1.0 - 2.0 * r1 + r12);

                    return -6.0 / (r1 * r1 * power_of<7>(r1 - r2))
                           * (power_of<4>(r1 - r2) * (ca00 * r1 + ca0r1 * L1mr1) + 6.0 * power_of<2>(r1 - r2) * (ca20 * r1 + ca2r1 * L1mr1) * a2pi
                ...
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    ...
                    return (T1tiltw2theta1mrho(r1, r2) + T1tiltw2thetarhom1(r1, r2) + T1tiltw2delta(r1, r2)) * weight * std::exp(-mb2 * r2 / _M2);
                });
            ...
            double
            Ftil_nlo_tw3(const double & q2, const double _M2, const double & _select_weight = 0.0) const
            {
                // Reminder: q2 is the kinematic variable associated with the momentum
                // transfer, while s is the kinematic variable in which the function is
                // analytically continued. See also comment at beginning of Appendix B
                ...
                auto T1tiltw3sigmathetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double lr2 = std::log(r2), lr2m1 = std::log(r2 - 1.0);
                    const double lr2mr1 = std::log(r2 - r1);
                    const double dil    = -2.0
                                       * (2.0 * std::real(dilog(1 / r1)) + 4.0 * std::real(dilog((r2 - 1.0) / (r1 - 1.0))) + std::real(dilog(1 / r2)) + 2.0 * std::real(dilog(r2))
                                          - 2.0 * std::real(dilog(r2 / r1)) + 4.0 * std::log((r1 - r2) / (r1 - 1)) * std::log(r2 - 1.0))
                                       * (r2 - r1) * r2;
                ...
                auto T1tiltw3sigmadeltarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double dl4 =
                            (4.0 * r12 - 2.0 * r13 + (-r13 - 4.0 * r12 + r1) * r2 + (3.0 * r12 - 2.0 * r1 + 1.0) * r22 + 2.0 * r12 * r2 * (-2.0 + r1 + r2) * (2.0 * lr2m1 - lr2))
                            * l1mr1 / r12;
                ...
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    ...
                    try
                    {
                        return (1.0 / (r2 * (r2 - r1)) * (T1tiltw3pthetarhom1(r1, r2) + T1tiltw3ptheta1mrho(r1, r2) + T1tiltw3pdeltarhom1(r1, r2))
            ...
            double
            FT_lo_tw2_integrand(const double & u, const double & q2, const double & _M2, const double & _select_weight) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP;
            ...
            double
            FT_lo_tw2(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double u0 = std::max(1e-10, (mb2 - q2) / (s0TB(q2) - q2));

                std::function<double(const double &)> integrand(std::bind(&Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::FT_lo_tw2_integrand,
                                                                          this,
                                                                          std::placeholders::_1,
                                                                          q2,
                                                                          _M2,
            ...
            double
            FT_lo_tw3_integrand(const double & u, const double & q2, const double & _M2, const double & _select_weight) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP;
                const double mu3 = lcdas->mu3(mu);
                const double u2  = u * u;
            ...
            double
            FT_lo_tw3(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double u0 = std::max(1e-10, (mb2 - q2) / (s0TB(q2) - q2));

                std::function<double(const double &)> integrand(std::bind(&Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::FT_lo_tw3_integrand,
                                                                          this,
                                                                          std::placeholders::_1,
                                                                          q2,
                                                                          _M2,
            ...
            double
            FT_lo_tw4(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb, mP2 = mP * mP, mP4 = mP2 * mP2;
                const double u0     = std::max(1e-10, (mb2 - q2) / (s0TB(q2) - q2));
                const double a2pi   = lcdas->a2(mu);
                ...
                std::function<double(const double &)> integrand([&](const double & u) -> double
                {
                    ...
                    const double tw4phi2 = -mb2 * u
                                           * (lcdas->phi4_d2(u, mu) - 6.0 * u * mP2 * lcdas->phi4_d1(u, mu) / (mb2 - q2 + u2 * mP2)
                                              + 12.0 * u * mP4 * lcdas->phi4(u, mu) / power_of<2>(mb2 - q2 + u2 * mP2))
                                           / (4.0 * (mb2 - q2 + u2 * mP2));
                    ...
                    return weight * std::exp(-(mb2 - q2 * (1.0 - u) + mP2 * u * (1.0 - u)) / (u * _M2)) * (tw4phi1 + tw4phi2 + tw4I4T) / (mb2 - q2 + u2 * mP2);
                });
            ...
            double
            FT_nlo_tw2(const double & q2, const double & _M2, const double & _select_weight = 0.0) const
            {
                // Reminder: q2 is the kinematic variable associated with the momentum
                // transfer, while s is the kinematic variable in which the function is
                // analytically continued. See also comment at beginning of Appendix B
                ...
                auto T1Ttw2theta1mrho = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca2 = power_of<2>(r1 - r2)
                                       * (-2.0 * (r1 * 55.0 - r12 * 65.0 + 16.0 * r13) + r2 * (95.0 - r1 * 15.0 - r12 * 45.0 + r13) + r22 * 2.0 * (-35.0 + r1 * 13.0 + r12 * 4.0)
                                          + r23 * 6.0 * (1.0 + r1));
                    const double ca4 = (-2877.0 * r1 + 6258.0 * r12 - r13 * 4592.0 + r14 * 1288.0 - r15 * 107.0)
                                       ...
                                       + r22 * 6.0 * (-791.0 + r1 * 889.0 - r12 * 21.0 - r13 * 131.0 + r14 * 4.0) + r23 * 10.0 * (266.0 - r1 * 280.0 + r12 * 35.0 + r13 * 9.0)
                                       + r24 * 10.0 * (-49.0 + r1 * 26.0 + r12 * 8.0) + r25 * 15.0 * (1.0 + r1);

                    const double cb0 = power_of<4>(r1 - r2) * (-1.0 - r1 + 2.0 * r2);
                    const double cb2 = power_of<2>(r1 - r2)
                                       * (-15.0 - r1 * 85.0 + r12 * 119.0 - r13 * 31.0 + r2 * 2.0 * (65.0 - r1 * 34.0 - r12 * 13.0) + r22 * 12.0 * (-8.0 + r1 * 5.0) + r23 * 12.0);
                    const double cb4 = (-210.0 - r1 * 2331.0 + r12 * 5754.0 - r13 * 4396.0 + r14 * 1259.0 - r15 * 106.0)
                                       + r2 * 3.0 * (1127.0 - r1 * 728.0 - r12 * 1358.0 + r13 * 1252.0 - r14 * 243.0) + r22 * 30.0 * (-189.0 + r1 * 245.0 - r12 * 52.0 - r13 * 14.0)
                                       + r23 * 20.0 * (161.0 - r1 * 193.0 + 47.0 * r12) + r24 * 15.0 * (-43.0 + 33.0 * r1) + r25 * 30.0;

                    return -(ca0 + ca2 * a2pi + ca4 * a4pi - L * r2 * (cb0 + cb2 * a2pi + cb4 * a4pi)) * (r1 - 1.0) * (r2 - 1.0) * 3.0 / (power_of<8>(r1 - r2) * r2);
                };
                auto T1Ttw2thetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double ca00    = -60.0 * (r1 * 2.0 + r2 * (-1.0 - r1 * 12.0 + r12 * 4.0) + r22 * 2.0 * (5.0 - r1) + r23 * (-1.0));
                    const double ca0mu   = -1.0 + 2.0 * r1 - r2;
                    const double ca0r2   = 1.0 + r12 + r2 * (-3.0 - r1 * 2.0 - r12 * 3.0) + r22 * (4.0 + r1 * 2.0);
                    const double ca0r2m1 = 2.0 * ca0mu;

                    const double ca20 = -5.0
                                        * (24.0 * (r1 * 55.0 - r12 * 90.0 + r13 * 36.0) + r2 * (-1140.0 - r1 * 7475.0 + r12 * 13780.0 - r13 * 5544.0 + r14 * 288.0)
                                           + r22 * (8915.0 - r1 * 3467.0 - r12 * 8672.0 + r13 * 2520.0) + r23 * (-10097.0 + r1 * 10501.0 - r12 * 836.0)
                                           + r24 * 5.0 * (-351.0 * r1 + 599.0) + r25 * (-37.0));
                    const double ca2mu = -15.0 + r1 * 130.0 - r12 * 96.0 + r13 * 12.0 + r2 * (-85.0 - r1 * 68.0 + r12 * 60) + r22 * (119.0 - r1 * 26.0) + r23 * (-31.0);
                    const double ca2r2 = 15.0 + r1 * 70.0 - r12 * 144.0 + r13 * 60.0 + r14 * 6.0 + r2 * (-145.0 + r1 * 128.0 + r12 * 12.0 - r13 * 24.0 - r14 * 18.0)
                                         + r22 * (166.0 - r1 * 204.0 + r12 * 54.0 - r13 * 72.0) + r23 * (-18.0 + r1 * 40.0 + r12 * 38.0) + r24 * (-1.0 + r1 * 37.0);
                    const double ca2r2m1 = 2.0 * ca2mu;

                    const double ca40 = 2.0
                                        * (-30.0 * (r1 * 2877.0 - r12 * 7875.0 + r13 * 7700.0 - r14 * 3150.0 + r15 * 450.0)
                                           ...
                                           + r24 * (-1010261.0 + r1 * 1718047.0 - r12 * 769551.0 + r13 * 40025.0) + r25 * (290999.0 + 2.0 * (-r1 * 215674.0 + 51507.0 * r12))
                                           + r26 * 2.0 * (-14213.0 + 9245.0 * r1) + r27 * 121.0);
                    const double ca4mu = -210.0 + r1 * 3381.0 - r12 * 5670.0 + r13 * 3220.0 - r14 * 645.0 + r15 * 30.0
                                         + r2 * (-2331.0 - r1 * 2184.0 + r12 * 7350.0 - r13 * 3860.0 + r14 * 495.0) + r22 * (5754.0 - r1 * 4074.0 - r12 * 1560.0 + r13 * 940.0)
                                         + r23 * (-4396.0 + r1 * 3756.0 - r12 * 420.0) + r24 * (1259.0 - r1 * 729.0) + r25 * (-106.0);
                    const double ca4r2 = 210.0 + r1 * 2121.0 - r12 * 6825.0 + r13 * 7000.0 - r14 * 2925.0 + r15 * 420.0 + r16 * 15.0
                                         ...
                                         + r23 * (-5740.0 + r1 * 10020.0 - r12 * 5520.0 + r13 * 1480.0 - r14 * 1090.0) + r24 * (1135.0 - r1 * 555.0 + r12 * 180.0 + r13 * 570.0)
                                         + r25 * (270.0 - r1 * 354.0 + r12 * 864.0) + r26 * (-31.0 + 121.0 * r1);
                    const double ca4r2m1 = 2.0 * ca4mu;

                    return -1.0 / (20.0 * r2 * power_of<8>(r1 - r2))
                           * (power_of<4>(r1 - r2) * (C0 * ca00 + Cl * ca0mu * Lmu + Clr2 * ca0r2 * Lr2 + Cl * ca0r2m1 * Lr2m1)
                ...
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    ...
                    return 2.0 * (T1Ttw2thetarhom1(r1, r2) + T1Ttw2theta1mrho(r1, r2) + T1Ttw2delta(r1, r2)) * weight * std::exp(-mb2 * r2 / _M2);
                });
            ...
            double
            FT_nlo_tw3(const double & q2, const double _M2, const double & _select_weight = 0.0) const
            {
                // Reminder: q2 is the kinematic variable associated with the momentum
                // transfer, while s is the kinematic variable in which the function is
                // analytically continued. See also comment at beginning of Appendix B
                ...
                auto T1Ttw3pthetarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double dl = -3.0
                                      * (std::real(dilog(1.0 / r1)) + std::real(dilog(r2)) - std::real(dilog(r2 / r1)) + 2.0 * std::real(dilog((r2 - 1.0) / (r1 - 1.0)))
                                         + lr2 * (lr1 + lr2m1 - lr2mr1 - lr2 / 2.0));
                ...
                auto T1Ttw3pdeltarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    return (-5.0 * pi2 / 6.0 + (4.0 - 1.0 / r1 + 1.0 / r2 - l1mr1) * l1mr1 + (-2.0 - 2.0 / r2 - 2.0 * l1mr1 + 3.0 * lr2m1) * lr2m1 + (l1mr1 - 2.0 * lr2m1) * lr2
                            + 2.0 * l * lmu + dl);
                };
                auto T1Ttw3sigmatheta1mrho = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    return 3.0
                           * ((r1 - 1.0) * (-4.0 + r2 * (3.0 - lr2 + lmu + 2.0 * lr2m1))
                              + +(r1 - r2) * r2 * (lr2m1 * (1.0 + 3.0 * lr2 - 6.0 * lr2m1 + 6.0 * lr2mr1 - 3.0 * lmu) + lr2mr1 * (-1.0 - 3.0 * lr2 + 3.0 * lmu)));
                };
                // clang-format off
                ...
                auto T1Ttw3sigmadeltarhom1 = [&](const double & r1, const double & r2) -> double
                {
                    ...
                    const double dl1    = r2
                                       * ((-4.0 + r1 + 3.0 * r2) * l1mr1 * l1mr1 + (-4.0 + 5.0 * r1 - r2) * lr2m1 * lr2m1 + (-4.0 + 3.0 * r1 + r2) * l1mr1 * lr2
                ...
                std::function<double(const double &)> integrand([&](const double & r2) -> double
                {
                    ...
                    return (2.0 / power_of<2>(r2 - r1) * (T1Ttw3pthetarhom1(r1, r2) + T1Ttw3ptheta1mrho(r1, r2) + T1Ttw3pdeltarhom1(r1, r2))
                ...
                return fP * mu3
                       * (integrate<GSL::QAGS>(integrand, 1.0 + eps, s0TB(q2) / mb2, config)
                          - 4.0 * (4.0 - 3.0 * lmu) * weight * std::exp(-mb2 / _M2) / power_of<2>(1.0 - q2 / mb2));
            }

            inline double
            _no_rescale_factor(const double &) const
            {
                return 1.0;
            }

            double
            _rescale_factor_p(const double & q2) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
                const double u0_q2   = std::max(1e-10, (mb2 - q2) / (s0B(q2) - q2));
                const double u0_zero = std::max(1e-10, mb2 / s0B(q2));

                std::function<double(const double &)> integrand_numerator_q2([&](const double & u) -> double
                { return u * (F_lo_tw2_integrand(u, q2, this->M2(), 0.0) + F_lo_tw3_integrand(u, q2, this->M2, 0.0)); });
                std::function<double(const double &)> integrand_denominator_q2([&](const double & u) -> double
                { return (F_lo_tw2_integrand(u, q2, this->M2(), 0.0) + F_lo_tw3_integrand(u, q2, this->M2(), 0.0)); });
                std::function<double(const double &)> integrand_numerator_zero([&](const double & u) -> double
                { return u * (F_lo_tw2_integrand(u, 0.0, this->M2(), 0.0) + F_lo_tw3_integrand(u, 0.0, this->M2(), 0.0)); });
                std::function<double(const double &)> integrand_denominator_zero([&](const double & u) -> double
            ...
            double
            _rescale_factor_0(const double & q2) const
            {
            ...
            double
            _rescale_factor_T(const double & q2) const
            {
                const double mb = this->m_b_msbar(mu), mb2 = mb * mb;
            ...
            double
            MBp_lcsr(const double & q2) const
            {
            ...
            double
            MB0_lcsr(const double & _q2) const
            {
            ...
            double
            MBT_lcsr(const double & q2) const
            {
                const double M2_rescaled = this->M2() * this->rescale_factor_p(q2);
                const double alpha_s     = model->alpha_s(mu);
            ...
            f_0(const double & q2) const
            {
                ...
                return std::exp(MB2 / M2_rescaled) / (2.0 * MB2 * fB)
                       * (2.0 * q2 / (MB2 - mP2) * (Ftil_lo + alpha_s / (3.0 * M_PI) * Ftil_nlo) + (1.0 - q2 / (MB2 - mP2)) * (F_lo + alpha_s / (3.0 * M_PI) * F_nlo));
            }
            ...
            diagnostics() const
            {
                ...
                results.add(Diagnostics::Entry{ this->decay_constant(), "f_B, [DKMMO:2008A]" });
    ...
    template <QuarkFlavor q1_, QuarkFlavor q2_, QuarkFlavor qs_>
    const std::vector<OptionSpecification> Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>::options{
    ...
    AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>::AnalyticFormFactorBToPseudoscalarDKMMO2008(const Parameters & p, const Options & o) :
        PrivateImplementationPattern<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>(
                new Implementation<AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>>(p, o, *this))
    {
    }

    template <QuarkFlavor q1_, QuarkFlavor q2_, QuarkFlavor qs_> AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>::~AnalyticFormFactorBToPseudoscalarDKMMO2008() = default;
    ...
    template <QuarkFlavor q1_, QuarkFlavor q2_, QuarkFlavor qs_>
    const std::set<ReferenceName> AnalyticFormFactorBToPseudoscalarDKMMO2008<q1_, q2_, qs_>::references{ "DKMMO:2008A"_rn, "DM:2008A"_rn, "LMvD:2021A"_rn };

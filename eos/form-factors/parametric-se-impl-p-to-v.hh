...
#ifndef EOS_GUARD_EOS_FORM_FACTORS_PARAMETRIC_SE_IMPL_P_TO_V_HH
#define EOS_GUARD_EOS_FORM_FACTORS_PARAMETRIC_SE_IMPL_P_TO_V_HH 1
...
namespace eos
{
    ...
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string> SEFormFactorTraits<Process_, PToV>::resonance_0m_names{
    ...
    template <typename Process_>
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string> SEFormFactorTraits<Process_, PToV>::resonance_1m_names{
        { std::make_tuple(QuarkFlavor::bottom,      QuarkFlavor::up), "mass::B_u^*@HME" },
        { std::make_tuple(QuarkFlavor::bottom,    QuarkFlavor::down), "mass::B_d^*@HME" },
        { std::make_tuple(QuarkFlavor::bottom, QuarkFlavor::strange), "mass::B_s^*@HME" },
        { std::make_tuple(QuarkFlavor::bottom,   QuarkFlavor::charm), "mass::B_c^*@HME" },
    ...
    template <typename Process_>
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string> SEFormFactorTraits<Process_, PToV>::resonance_1p_names{
    ...
    template <typename Process_>
    SEFormFactors<Process_, PToV>::SEFormFactors(const Parameters & p, const Options &) :
        ...
        _traits(SEFormFactorTraits<Process_, PToV>(p)),
        _mB(_traits.m_B),
        _mV(_traits.m_V)
    {
    }
    ...
    template <typename Process_>
    FormFactors<PToV> *
    SEFormFactors<Process_, PToV>::make(const Parameters & parameters, const Options & options)
    {
    ...
    template <typename Process_>
    QualifiedName
    SEFormFactors<Process_, PToV>::_par_name(const std::string & ff_name, unsigned idx) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::_phi(const double & t, const double & threshold_tp, const double & chi, const int & A, const unsigned B, const unsigned C, const unsigned k,
                                        const unsigned p, const unsigned n, const unsigned m) const
    {
        // [GvDV:2020A]
        const double z = _traits.calc_z(t, threshold_tp, _traits.t0), kinematic_tp = power_of<2>(_mB + _mV);
        const double norm = std::sqrt(Process_::eta * k * pow(kinematic_tp, A) * pow(_traits.tm(), B) * pow(4 * _mB * _mV, C) / 96 / M_PI / M_PI / chi);
        ...
        const double lambda_term =
                (kinematic_tp - t) * power_of<2>(std::sqrt(threshold_tp - t) + std::sqrt(threshold_tp - _traits.tm())); // simplification of lambda / z(t, threshold_tp, tm);
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_v(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_a_0(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_a_1(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_a_12(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_t_1(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_t_2(const double & q2) const
    {
    ...
    template <typename Process_>
    inline double
    SEFormFactors<Process_, PToV>::_phi_t_23(const double & q2) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::_a_A12_0() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::_a_T2_0() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::_a_A1_0() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::_a_T23_0() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::v(const double & q2) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::a_0(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::a_1(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::a_12(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::t_1(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::t_2(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::t_23(const double & q2) const
    {
        std::array<double, 5> coefficients;
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::a_2(const double & s) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::t_3(const double & s) const
    {
        const double lambda = eos::lambda(power_of<2>(_mB), power_of<2>(_mV), s);
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_perp(const double & s) const
    {
        const double lambda = eos::lambda(power_of<2>(_mB), power_of<2>(_mV), s);
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_para(const double & s) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_long(const double & s) const
    {
        const double lambda = eos::lambda(power_of<2>(_mB), power_of<2>(_mV), s);

        return ((power_of<2>(_mB) - power_of<2>(_mV) - s) * pow(_mB + _mV, 2) * a_1(s) - lambda * a_2(s)) / (2 * _mV * power_of<2>(_mB) * (_mB + _mV));
    }

    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_perp_T(const double & s) const
    {
        const double lambda = eos::lambda(power_of<2>(_mB), power_of<2>(_mV), s);
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_para_T(const double & s) const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::f_long_T(const double & s) const
    {
        const double lambda = eos::lambda(power_of<2>(_mB), power_of<2>(_mV), s);
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_0p_v() const
    {
        return 0.0;
    }
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_0m_a() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_1m_v() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_1p_a() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_1m_t() const
    {
    ...
    template <typename Process_>
    double
    SEFormFactors<Process_, PToV>::saturation_1p_t5() const
    {
    ...
    template <typename Process_>
    Diagnostics
    SEFormFactors<Process_, PToV>::diagnostics() const
    {
        Diagnostics results;
    ...
    template <typename Process_> const std::set<ReferenceName> SEFormFactors<Process_, PToV>::references{ "BFW:2010A"_rn };

    template <typename Process_> const std::vector<OptionSpecification> SEFormFactors<Process_, PToV>::options{};

    template <typename Process_>
    std::vector<OptionSpecification>::const_iterator
    SEFormFactors<Process_, PToV>::begin_options()
    {
    ...
    template <typename Process_>
    std::vector<OptionSpecification>::const_iterator
    SEFormFactors<Process_, PToV>::end_options()
    {
        return options.cend();
    }

...
namespace eos
{
    ...
    // P -> PP
    template <typename Process_> class HKVT2025FormFactorTraits<Process_, PToPP> : public virtual ParameterUser
    {
        public:
            ...
            double
            tm(const double & k2) const
            {
                return power_of<2>(m_B - std::sqrt(k2));
            }

            double
            lam_b(const double & k2, const double & q2) const
            {
                return (q2 - power_of<2>(m_B + std::sqrt(k2))) * (q2 - power_of<2>(m_B - std::sqrt(k2)));
            }

            double
            kappa(const double & k2, const double & q2) const
            {
            ...
            complex<double>
            calc_y(const complex<double> & k2, const complex<double> & k2in, const complex<double> & k20) const
            {
                return (std::sqrt(k2in - k2) - std::sqrt(k2in - k20)) / (std::sqrt(k2in - k2) + std::sqrt(k2in - k20));
            }

            complex<double>
            calc_z(const complex<double> & q2, const complex<double> & q2p, const complex<double> & q20) const
            {
                return (std::sqrt(q2p - q2) - std::sqrt(q2p - q20)) / (std::sqrt(q2p - q2) + std::sqrt(q2p - q20));
            }

            double
            calc_y(const double & k2, const double & k2in, const double & k20) const
            {
                if (k2 > k2in)
                {
            ...
            double
            calc_z(const double & q2, const double & q2p, const double & q20) const
            {
                if (q2 > q2p)
                {
            ...
            std::array<double, 3>
            orthonormal_polynomials_v(const double & z, const double & k2) const
            {
            ...
            std::array<double, 3>
            orthonormal_polynomials_a(const double & z, const double & k2) const
            {
            ...
            threshold_improved_polynomials(const complex<double> & y, const unsigned & l) const
            {
                if (l == 1)
                {
                ...
                {
                    return std::array<complex<double>, 3>{ 1.0,
                                                           y - (15.0 * power_of<3>(y) / 3.0 + 2.0 * power_of<4>(y)) / 7.0,
                                                           power_of<2>(y) + 2.0 * (4.0 * power_of<3>(y) + 5.0 * power_of<4>(y) / 4.0) / 7.0 };
                }
                return std::array<complex<double>, 3>{ 1.0, y, power_of<2>(y) };
            }

            std::array<double, 3>
            threshold_improved_polynomials(const double & y, const unsigned & l) const
            {
                if (l == 1)
                {
    ...
    template <typename Process_> class HKVT2025FormFactors<Process_, PToPP> : public FormFactors<PToPP>, public virtual ParameterUser
    {
        private:
            ...
            double _phi(const int & l, const double & k2, const double & q2, const double & q2p, const double & chi, const unsigned a, const unsigned b, const double N) const;

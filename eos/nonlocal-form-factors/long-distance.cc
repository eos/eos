...
namespace eos
{
    ...
    LongDistance::g_had_ccbar(const double & s, const double & m_c)
    {
        ...
        static const std::vector<double> m_cc_hat{
            /* Masses in GeV */
            3.0969 / m_b, 3.6861 / m_b, 3.771 / m_b, 4.039 / m_b, 4.153 / m_b, 4.421 / m_b,
        };
        static const std::vector<double> gamma_cc_hat{
            /* Decay widths in GeV */
            9.34e-5 / m_b, 3.37e-4 / m_b, 2.30e-2 / m_b, 8.00e-2 / m_b, 1.03e-1 / m_b, 6.20e-2 / m_b,
        };
        static const std::vector<double> br_cc{
            5.935e-2, 7.325e-3, 1.050e-5, 1.070e-5, 8.100e-6, 9.400e-6,
        };
        ...
        double real_cont = (0.571896 + 34.0 / 3.0 * ((s_1_hat - s_hat) * std::log(std::abs(s_1_hat - s_hat)) - (s_2_hat - s_hat) * std::log(std::abs(s_2_hat - s_hat)))) / s_hat;

        return complex<double>(s_hat * (real_cont + fudge * real_res) / 3.0, M_PI / 3.0 * (fudge * imag_res + imag_cont)) - 8.0 / 9.0 * log(m_c / m_b) - 4.0 / 9.0;
    }
} // namespace eos

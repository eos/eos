...
namespace eos
{
    ...
    template <> struct WrappedForwardIteratorTraits<ConstraintEntry::ObservableNameIteratorTag>
    {
    ...
    template <> struct WrappedForwardIteratorTraits<ConstraintEntry::ReferenceNameIteratorTag>
    {
    ...
    /// {{{ ConstraintEntryBase
    class ConstraintEntryBase : public ConstraintEntry
    {
        protected:
            QualifiedName _name;
            ...
            ConstraintEntryBase(const QualifiedName & name, std::vector<QualifiedName> && observable_names, std::vector<ReferenceName> && reference_names) :
                _name(name),
        ...
        public:
            ConstraintEntryBase(const QualifiedName & name, const QualifiedName & observable_name, const std::vector<ReferenceName> & reference_names) :
                ConstraintEntryBase(name, std::vector<QualifiedName>{ observable_name }, reference_names)
            {
            }

            ConstraintEntryBase(const QualifiedName & name, const std::vector<QualifiedName> & observable_names, const std::vector<ReferenceName> & reference_names) :
                _name(name),
            ...
            ConstraintEntryBase(const QualifiedName & name, const std::array<QualifiedName, n_> & observable_names) :
            ...
            virtual ConstraintEntry::ObservableNameIterator
            begin_observable_names() const
            {
                return _observable_names.begin();
            }

            virtual ConstraintEntry::ObservableNameIterator
            end_observable_names() const
            {
                return _observable_names.end();
            }

            virtual ConstraintEntry::ReferenceNameIterator
            begin_references() const final
            {
                return _reference_names.begin();
            }

            virtual ConstraintEntry::ReferenceNameIterator
            end_references() const final
            {
                return _reference_names.end();
            }

            virtual void
            serialize(YAML::Emitter & out) const
            {
                out << YAML::BeginMap;
                out << YAML::Key << "type";
    ...
    struct GaussianConstraintEntry : public ConstraintEntryBase
    {
            ...
            GaussianConstraintEntry(const std::string & name, const QualifiedName & observable, const Kinematics & kinematics, const Options & options, const double & central,
                                    const double & sigma_hi_stat, const double & sigma_lo_stat, const double & sigma_hi_sys, const double & sigma_lo_sys,
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (const auto & [key, value] : this->options)
                {
                    ...
                    {
                        Log::instance()->message("[GaussianConstraintEntry.make]", ll_debug) << "Constraint '" << name << "' provides option key '" << key << "' with value '"
                                                                                             << value << "'; user is overriding this preset with '" << options[key] << "'";
                    }
                }
    ...
    struct LogGammaConstraintEntry : public ConstraintEntryBase
    {
            QualifiedName observable;
            ...
            LogGammaConstraintEntry(const std::string & name, const QualifiedName & observable, const Kinematics & kinematics, const Options & options, const double & central,
                                    const double & sigma_hi, const double & sigma_lo, const double & alpha, const double & lambda, const std::vector<ReferenceName> & references) :
                ConstraintEntryBase(name, observable, references),
                observable(observable),
                kinematics(kinematics),
                options(options),
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (const auto & [key, value] : this->options)
                {
                    if (options.has(key) && (value != options[key]))
                    {
                        Log::instance()->message("[LogGammaConstraintEntry.make]", ll_debug) << "Constraint '" << name << "' provides option key '" << key << "' with value '"
                                                                                             << value << "'; user is overriding this preset with '" << options[key] << "'";
                    }
                }
    ...
    struct AmorosoConstraintEntry : public ConstraintEntryBase
    {
            QualifiedName observable;
            ...
            AmorosoConstraintEntry(const QualifiedName & name, const QualifiedName & observable, const Kinematics & kinematics, const Options & options,
                                   const double & physical_limit, const double & theta, const double & alpha, const double & beta, const std::vector<ReferenceName> & references) :
                ConstraintEntryBase(name, observable, references),
                observable(observable),
                kinematics(kinematics),
                options(options),
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (const auto & [key, value] : this->options)
                {
                    if (options.has(key) && (value != options[key]))
                    {
                        Log::instance()->message("[AmorosoConstraintEntry.make]", ll_debug) << "Constraint '" << name << "' provides option key '" << key << "' with value '"
                                                                                            << value << "'; user is overriding this preset with '" << options[key] << "'";
                    }
                }
    ...
    struct MultivariateGaussianConstraintEntry : public ConstraintEntryBase
    {
            ...
            MultivariateGaussianConstraintEntry(const QualifiedName & name, const std::vector<QualifiedName> & observable_names, const std::vector<Kinematics> & kinematics,
                                                const std::vector<Options> & options, const std::vector<double> & means, const std::vector<double> & sigma_stat_hi,
                                                const std::vector<double> & sigma_stat_lo, const std::vector<double> & sigma_sys,
                                                const std::vector<std::vector<double>> & correlation, const unsigned number_of_observations,
                                                const std::vector<ReferenceName> & references) :
            ...
            {
                if (dim != kinematics.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of kinematics");
                }

                if (dim != options.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of options");
                }

                if (dim != means.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of means");
                }

                if (dim != sigma_stat_hi.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of sigma-stat(hi)");
                }

                if (dim != sigma_stat_lo.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of sigma-stat(lo)");
                }

                if (dim != sigma_sys.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of sigma-sys");
                }

                if (dim < number_of_observations)
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of observations");
                }

                if (dim != correlation.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: wrong number of rows in correlation");
                }
                for (auto i = 0u; i < dim; ++i)
                {
                    if (dim != correlation[i].size())
                    {
                        throw InternalError("MultivariateGaussianConstraintEntry: wrong number of columns in correlation row " + stringify(i));
                    }
                }
            }
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (auto i = begin; i < end; ++i)
                {
                    for (const auto & [key, value] : this->options[i])
                    {
                        if (options.has(key) && (value != options[key]))
                        {
                            Log::instance()->message("[MultivariateGaussianConstraintEntry.make]", ll_debug)
                                    << "Constraint '" << name << "' in observable '" << this->observable_names[i] << "' provides option key '" << key << "' with value '" << value
                                    << "'; user is overriding this preset with '" << options[key] << "'";
                        }
                    }
                    ...
                    {
                        throw InternalError("make_multivariate_gaussian_constraint<" + stringify(dim) + ">: " + name.str() + ": '" + this->observable_names[i].str()
                                            + "' is not a valid observable name");
                    }
                }
    ...
    struct MultivariateGaussianCovarianceConstraintEntry : public ConstraintEntryBase
    {
            std::vector<QualifiedName> observables;
            ...
            MultivariateGaussianCovarianceConstraintEntry(const QualifiedName & name, const std::vector<QualifiedName> & observables, const std::vector<Kinematics> & kinematics,
                                                          const std::vector<Options> & options, gsl_vector * const means, gsl_matrix * const covariance,
                                                          gsl_matrix * const response, const std::vector<ReferenceName> & references, const unsigned number_of_observations) :
            ...
            {
                if ((nullptr == response) && (dim_meas != dim_pred))
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of measurements does not equal number of predictions in absence of a response matrix");
                }

                if (dim_meas != covariance->size1)
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of rows in covariance does not equal number of measurements");
                }
                if (dim_meas != covariance->size2)
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of columns in covariance does not equal number of measurements");
                }

                if ((nullptr != response) && (dim_meas != response->size1))
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of rows in response does not equal number of measurements");
                }
                if ((nullptr != response) && (dim_pred != response->size2))
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of columns in response does not equal number of predictions");
                }

                if (dim_pred != kinematics.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of kinematics entries does not equal number of predictions");
                }

                if (dim_pred != options.size())
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of options entries does not equal number of predictions");
                }

                if (dim_meas < number_of_observations)
                {
                    throw InternalError("MultivariateGaussianConstraintEntry: number of observations larger than number of measurements");
                }
            }
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (auto i = 0u; i < dim_pred; ++i)
                {
                    for (const auto & [key, value] : this->options[i])
                    {
                        if (options.has(key) && (value != options[key]))
                        {
                            Log::instance()->message("[MultivariateGaussianCovarianceConstraintEntry.make]", ll_debug)
                                    << "Constraint '" << name << "' in observable '" << this->observables[i] << "' provides option key '" << key << "' with value '" << value
                                    << "'; user is overriding this preset with '" << options[key] << "'";
                        }
                    }
                    ...
                    {
                        throw InternalError("make_multivariate_gaussian_covariance_constraint<measurements=" + stringify(dim_meas) + ",predictions=" + stringify(dim_pred)
                                            + ">: " + name.str() + ": '" + this->observables[i].str() + "' is not a valid observable name");
                    }
                }
    ...
    struct UniformBoundConstraintEntry : public ConstraintEntryBase
    {
            std::vector<QualifiedName> observable_names;
            ...
            UniformBoundConstraintEntry(const std::string & name, const std::vector<QualifiedName> & observable_names, const std::vector<Kinematics> & kinematics,
                                        const std::vector<Options> & options, const double & bound, const double & uncertainty, const std::vector<ReferenceName> & references) :
                ConstraintEntryBase(name, observable_names, references),
                observable_names(observable_names),
                kinematics(kinematics),
                options(options),
            ...
            {
                if (number_of_observables != kinematics.size())
                {
                    throw InternalError("UniformBoundConstraintEntry: wrong number of kinematics");
                }

                if (number_of_observables != options.size())
                {
                    throw InternalError("UniformBoundConstraintEntry: wrong number of options");
                }
            }
            ...
            make(const QualifiedName & name, const Options & options) const
            {
                ...
                for (auto i = 0u; i < number_of_observables; ++i)
                {
                    ...
                    if (! observables[i].get())
                    {
                        throw InternalError("make_uniform_bound_constraint<" + stringify(number_of_observables) + ">: " + name.str() + ": '" + this->observable_names[i].str()
                                            + "' is not a valid observable name");
                    }
                }
    ...
    struct MixtureConstraintEntry : public ConstraintEntryBase
    {
            std::vector<QualifiedName> observables;
            ...
            explicit MixtureConstraintEntry(const QualifiedName & name, const std::vector<QualifiedName> & observables, const std::vector<Kinematics> & kinematics,
                                            const std::vector<Options> & options, std::vector<GSLVectorPtr> && _means, std::vector<GSLMatrixPtr> && _covariances,
                                            const std::vector<double> & weights, const std::vector<std::array<double, 2>> & test_stat, const unsigned number_of_observations,
                                            const std::vector<ReferenceName> & references) :
                ConstraintEntryBase(name, observables, references),
                observables(observables),
                kinematics(kinematics),
                ...
                dim_pred(observables.size())
            {
                ...
                {
                    throw InternalError("MixtureConstraintEntry: number of components does not agree between means and covariances");
                }
                if (means.size() != weights.size())
                {
                    throw InternalError("MixtureConstraintEntry: number of components does not agree between means and weights");
                }

                if (means.empty())
                {
                    throw InternalError("MixtureConstraintEntry: need at least one component");
                }

                dim_meas = means.front()->size;
                for (unsigned int i = 0; i < means.size(); ++i)
                {
                    if (dim_meas != means[i]->size)
                    {
                        throw InternalError("MixtureConstraintEntry: mean vectors are not all equal in size");
                    }
                }

                if (dim_meas != dim_pred)
                {
                    throw InternalError("MixtureConstraintEntry: number of measurements does not equal number of predictions");
                }

                for (unsigned int i = 0; i < means.size(); ++i)
                {
                    if (dim_meas != covariances[i]->size1)
                    {
                        throw InternalError("MixtureConstraintEntry: number of rows in at least one covariance does not equal number of measurements");
                    }
                    if (dim_meas != covariances[i]->size2)
                    {
                        throw InternalError("MixtureConstraintEntry: number of columns in at least one covariance does not equal number of measurements");
                    }
                }

                if (dim_pred != kinematics.size())
                {
                    throw InternalError("MixtureConstraintEntry: number of kinematics entries does not equal number of predictions");
                }

                if (dim_pred != options.size())
                {
                    throw InternalError("MixtureConstraintEntry: number of options entries does not equal number of predictions");
                }

                if (dim_meas < number_of_observations)
                {
                    throw InternalError("MixtureConstraintEntry: number of observations larger than number of measurements");
                }
            }
                ...
                try
                {
                    ...
                    for (auto && c : n["components"])
                    {
                        ...
                        for (auto i = 0u; i < _covariance.size(); ++i)
                        {
                            ...
                            {
                                throw ConstraintDeserializationError(name,
                                                                     "covariance matrix is not square; row " + stringify(i) + " has " + stringify(_row.size())
                                                                             + " columns; expected " + stringify(_covariance.size()));
                            }
    ...
    /* Constraint */
    template <> struct WrappedForwardIteratorTraits<Constraint::BlockIteratorTag>
    {
    ...
    template <> struct WrappedForwardIteratorTraits<Constraint::ObservableIteratorTag>
    {
    ...
    template <> struct Implementation<Constraint>
    {
            QualifiedName name;
            ...
            Implementation(const QualifiedName & name, const std::vector<ObservablePtr> & observables, const std::vector<LogLikelihoodBlockPtr> & blocks) :
                name(name),
                blocks(blocks)
            {
    ...
    Constraint::Constraint(const QualifiedName & name, const std::vector<ObservablePtr> & observables, const std::vector<LogLikelihoodBlockPtr> & blocks) :
    ...
    class ConstraintEntries : public InstantiationPolicy<ConstraintEntries, Singleton>
    {
        private:
        ...
        public:
            ...
            void
            insert(const QualifiedName & key, const std::shared_ptr<const ConstraintEntry> & value)
            {
                _entries[key] = value;
            }
    };
    ...
    template <> struct WrappedForwardIteratorTraits<Constraints::ConstraintIteratorTag>
    {
    ...
    template <> struct Implementation<Constraints>
    {

...
namespace eos
{
    ...
    double GMKPRDEY2011ScatteringAmplitudes::_phase_S0(const double & s) const
    {
        ...
        if ( s <= _sM_S0 )
        {
            double k = std::sqrt(s / 4 - mpi2);
            double ws = _calc_w(s, 4 * mk2);
            if ( s <= 0.6 ) // Avoid divide by 0 for s = 4*mpi2 and s approx 0.7
            {
                 return std::atan( 2 * k / sqrts * (s - mpi2 / 2) / mpi2 / (_mPi / sqrts + _params_S0[0] + _params_S0[1] * ws + _params_S0[2] * power_of<2>(ws) + _params_S0[3] * power_of<3>(ws)) );
            }
            else
            {
                 return M_PI/2 - std::atan( sqrts * mpi2 / 2 / k / (s - mpi2 / 2) * (_mPi / sqrts + _params_S0[0] + _params_S0[1] * ws + _params_S0[2] * power_of<2>(ws) + _params_S0[3] * power_of<3>(ws)) );
            }
        }
        else if ( s <= 4 * mk2 )
        {
            ...
            double kMpi = std::sqrt(_sM_S0 / 4 - mpi2);
            double sqrts0sM = std::sqrt(4 * mk2 - _sM_S0);
            double wM = _calc_w(_sM_S0, 4 * mk2);
            ...
            double x1 = mpi2 * ( (mpi2 - 2 * _sM_S0) * _sM_S0 - 4 * power_of<2>(kMpi) * (mpi2 + 2 * _sM_S0)) * Bsum / (8 * sqrtsM * power_of<2>(mpi2 - 2 * _sM_S0) * power_of<3>(kMpi));
            double x2 = mpi3 / kMpi / 2 / _sM_S0 / (mpi2 - 2 * _sM_S0);
            double x3 = -mpi2 / 2 / (mpi2 - 2 * _sM_S0) / kMpi / sqrts0sM * ((1 - wM) * sqrts0sM + (1 + wM) * sqrtsM) / (sqrtsM + sqrts0sM);
            double x4 = _sM_S0 * power_of<2>(mpi2 * Bsum / (mpi2 - 2 * _sM_S0) / kMpi);

            // Derivative of the phase at s = _sM_s0
            double delpM = -(x1 + x2 + x3 * (_params_S0[1] + 2 * wM * _params_S0[2] + 3 * power_of<2>(wM) * _params_S0[3])) / (1 + x4);
            
            // Remaining quantities entering the parametrization of the phase
            double absk2 = std::sqrt(mk2 - s / 4);
            double kM = std::sqrt(mk2 - _sM_S0 / 4);
            double mk3 = mk2 * _mK;

            return _params_S0[4] * power_of<2>( 1 - absk2 / kM ) + _phase_S0(_sM_S0) * absk2 / kM * (2 - absk2 / kM) + absk2 * (kM - absk2) * ( 8 * delpM + _params_S0[5] * (kM - absk2) / mk3);
        }
        else if ( s <= 4 * meta2 )
        {
            double k22norm = (s / 4 / mk2 - 1);
        ...
        {
            double k22norm = (s / 4 / mk2 - 1);
            double k32norm = (s / 4 / meta2 - 1);

            return _params_S0[4] + _params_S0[6] * k22norm + _params_S0[7] * power_of<2>(k22norm) + _params_S0[8] * k32norm;
        }
        else
        {
            return 2 * M_PI + (_phase_S0(2.0164) - 2 * M_PI) * 2 / (1 + std::pow(s / 2.0164, _cont_pow_S0));
        }
    }
    ...
    double GMKPRDEY2011ScatteringAmplitudes::_phase_P1(const double & s) const
    {
        ...
        if ( s <= 4 * mk2 )
        {
            ...
            {
                return std::atan( 2 * power_of<3>(k) / sqrts / (mrho2 - s) / (2 * mpi3 / mrho2 / sqrts + _params_P1[0] + _params_P1[1] * _calc_w(s, _s0_P1)) );
            }
            else
            {
                return M_PI / 2 - std::atan( sqrts / 2 / power_of<3>(k) * (mrho2 - s) * (2 * mpi3 / mrho2 / sqrts + _params_P1[0] + _params_P1[1] * _calc_w(s, _s0_P1)) );
            }
        }
        else if ( s <= 2.0164 )
        {
            return _phase_P1(4 * mk2) + _params_P1[2] * (sqrts / 2 / _mK - 1) + _params_P1[3] * power_of<2>(sqrts / 2 / _mK - 1);
        }
        else
        {
            return M_PI + (_phase_P1(2.0164) - M_PI) * 2 / (1 + std::pow(s / 2.0164, _cont_pow_P1));
        }
    }
    ...
    double GMKPRDEY2011ScatteringAmplitudes::_phase_D0(const double & s) const
    {
        ...
        if ( s <= _s0_D0 )
        {
            return std::atan( 2 * power_of<5>(k) / sqrts / (mf22 - s) / mpi2 / (_params_D0[0] + _params_D0[1] * _calc_w(s, _s0_D0)) );
        }
        else if ( s <= 2.0164 )
        {
            double Bh0 = _params_D0[0] + _params_D0[1] - _params_D0[2] * _calc_w(_s0_D0, _sh_D0);
            return M_PI/2 - std::atan( sqrts / 2 / power_of<5>(k) * (mf22 - s) * mpi2 * (Bh0 + _params_D0[2] * _calc_w(s, _sh_D0)) );
        }
        else
        {
            return M_PI + (_phase_D0(2.0164) - M_PI) * 2 / (1 + std::pow(s / 2.0164, _cont_pow_D0));
        }
    }
    ...
    complex<double> GMKPRDEY2011ScatteringAmplitudes::scattering_amplitude(const double & s, const unsigned & l, const std::string & i) const
    {
        if ( s <= 4 * _mPi * _mPi )
        {
            return 0.0;
    ...
    complex<double> GMKPRDEY2011ScatteringAmplitudes::omnes_factor(const double & s, const unsigned & l, const std::string & i) const
    {
        if ( (l == 0) && (i== "0") )
        {
            throw InternalError("Current Omnes factor solution stratgy does not allow for phases exceeding 2 Pi! Consider implementing coupled-channel treatment!");
        }
        else if ( (l == 1) && (i== "1") )
        {
            return _omnes_P1(s);

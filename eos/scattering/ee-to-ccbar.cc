...
namespace eos
{
    ...
    template <> struct Implementation<EEToCCBar>
    {
            UsedParameter hbar;
            ...
            inline static const std::array<std::string, 3> resonance_names = { "J/psi", "psi(2S)", "psi(3770)" };
            ...
            auto
            _resonance_masses(const Parameters & p, ParameterUser & u, std::integer_sequence<T, indices...>) -> std::array<UsedParameter, sizeof...(indices)>
            {
                ...
                return std::array<UsedParameter, sizeof...(indices)>{ { UsedParameter(p["mass::" + resonance_names[indices]], u)... } };
            }

            template <typename T, T... indices>
            auto
            _channel_effective_momentum(const Parameters & p, ParameterUser & u, std::integer_sequence<T, indices...>) -> std::array<UsedParameter, sizeof...(indices)>
            {
                ...
                return std::array<UsedParameter, sizeof...(indices)>{ { // TODO: The q0 widths can be made channel dependent.
            ...
            long unsigned
            _filter_channel_index(Channels channel)
            {
                if (assume_isospin)
                {
                    switch (channel)
            ...
            auto
            _g0_row(const Parameters & p, ParameterUser & u, T row_index, std::integer_sequence<T, column_indices...>) -> std::array<UsedParameter, sizeof...(column_indices)>
            {
            ...
            auto
            _g0_matrix(const Parameters & p, ParameterUser & u, std::integer_sequence<T, row_indices...>, std::integer_sequence<T, column_indices...> column_seq)
                    -> std::array<std::array<UsedParameter, sizeof...(column_indices)>, sizeof...(row_indices)>
            {
                ...
                return std::array<std::array<UsedParameter, sizeof...(column_indices)>, sizeof...(row_indices)>{ { _g0_row(p, u, row_indices, column_seq)... } };
            }

            template <typename T>
            std::string
            _channel_name_tuple(const T & a, const T & b)
            {
                if (b > a)
                {
            ...
            template <typename T, T... column_indices>
            auto
            _c_row(const Parameters & p, ParameterUser & u, T row_index, std::integer_sequence<T, column_indices...>) -> std::array<UsedParameter, sizeof...(column_indices)>
            {
                ...
                return std::array<UsedParameter, sizeof...(column_indices)>{ { UsedParameter(p["ee->ccbar::c" + _channel_name_tuple(row_index, column_indices)], u)... } };
            }

            template <typename T, T... row_indices, T... column_indices>
            auto
            _c_matrix(const Parameters & p, ParameterUser & u, std::integer_sequence<T, row_indices...>, std::integer_sequence<T, column_indices...> column_seq)
                    -> std::array<std::array<UsedParameter, sizeof...(column_indices)>, sizeof...(row_indices)>
            {
                ...
                return std::array<std::array<UsedParameter, sizeof...(column_indices)>, sizeof...(row_indices)>{ { _c_row(p, u, row_indices, column_seq)... } };
            }

            template <typename T, T... row_indices>
            auto
            _get_g0_column(T column_index, std::integer_sequence<T, row_indices...>) -> std::array<Parameter, sizeof...(row_indices)>
            {
                return std::array<Parameter, sizeof...(row_indices)>{ { g0[row_indices][column_index]... } };
            }

            template <typename T, T... column_indices>
            auto
            _get_bkgcst_row(T row_index, std::integer_sequence<T, column_indices...>) -> std::array<Parameter, sizeof...(column_indices)>
            {
                return std::array<Parameter, sizeof...(column_indices)>{ { bkgcst[row_index][column_indices]... } };
            }

            template <typename T, T... row_indices, T... column_indices>
            auto
            _get_bkgcst_matrix(std::integer_sequence<T, row_indices...>, std::integer_sequence<T, column_indices...> column_seq)
                    -> std::array<std::array<Parameter, sizeof...(column_indices)>, sizeof...(row_indices)>
            {
                return std::array<std::array<Parameter, sizeof...(column_indices)>, sizeof...(row_indices)>{ { _get_bkgcst_row(row_indices, column_seq)... } };
            }
                ...
                exclusive_norm(p["ee->ccbar::exclusive_norm"], u)
            {
                ...
                for (unsigned i = 0; i < EEToCCBar::nchannels; i++)
                {
                    ...
                    {
                        case ee:
                            channel_array[i] = std::make_shared<EEChannel<EEToCCBar::nchannels, EEToCCBar::nresonances>>(
                                    channel_names[i],
                                    m_e,
                                    m_e,
                                    q[i],
                        ...
                        case eff2S:
                        case eff3770:
                            channel_array[i] = std::make_shared<EffChannel<EEToCCBar::nchannels, EEToCCBar::nresonances>>(
                                    channel_names[i],
                                    m_eff,
                                    m_eff,
                                    q[i],
                        ...
                        default: throw InternalError("The number of requested channels (array) is larger than the number of known channel names.");
                    }
                }

                K = std::shared_ptr<KMatrix<EEToCCBar::nchannels, EEToCCBar::nresonances>>(new KMatrix<EEToCCBar::nchannels, EEToCCBar::nresonances>(
                        channel_array,
                        resonance_array,
            ...
            prepare(const complex<double> & E)
            {
            ...
            double
            rho(const IntermediateResult * intermediate_result, const Channels & channel)
            {
                return std::real(K->_channels[channel]->rho(intermediate_result->s));
            }

            complex<double>
            chew_mandelstam(const IntermediateResult * intermediate_result, const Channels & channel)
            {
                return K->_channels[channel]->chew_mandelstam(intermediate_result->s);
            }

            complex<double>
            chew_mandelstam_II(const IntermediateResult * intermediate_result, const Channels & channel)
            {
                ...
                return get_channel->chew_mandelstam(s) + complex<double>(0.0, 2.0) * get_channel->rho(s) * power_of<2>(pow(q / q0, li) * Fi);
            }

            inline double
            sigma_eetomumu_leading_order(const double & E)
            {
            ...
            complex<double>
            T_eetochannel(const IntermediateResult * intermediate_result, const Channels & channel)
            {
            ...
            complex<double>
            T_II_eetochannel(const IntermediateResult * intermediate_result, const Channels & channel)
            {
            ...
            double
            sigma_eetochannel(const IntermediateResult * intermediate_result, const Channels & channel)
            {
                // Conversion factor between GeV^2 and nb
                static const double speedoflight = 299792458.; // Exact value
                const double        GeVtonb      = 10.0 * power_of<2>(1.0e18 * hbar * speedoflight);
            ...
            double
            res_partial_width(const Resonances & resonance, const Channels & channel)
            {
                return K->partial_width(resonance, channel);
            }

            double
            res_total_width(const Resonances & resonance)
            {
            ...
            // Ratios
            double
            R(const IntermediateResult * intermediate_result)
            {
            ...
            // Spectral function
            double
            spectral_function(const double E, const Resonances & resonance)
            {
    ...
    const std::vector<OptionSpecification> Implementation<EEToCCBar>::options{
    ...
    EEToCCBar::sigma_eetoeff(const EEToCCBar::IntermediateResult * ir) const
    {
        return _imp->exclusive_norm
               * (_imp->sigma_eetochannel(ir, Channels::effJpsi) + _imp->sigma_eetochannel(ir, Channels::eff2S) + _imp->sigma_eetochannel(ir, Channels::eff3770));
    }

...
namespace eos
{
    ...
    LambdaBToLambda1520GammaAmplitudes<tag::Naive>::amplitudes() const
    {
        ...
        const complex<double> wilson7_plus = (c7eff + wc.c7prime()), wilson7_minus = (c7eff - wc.c7prime());

        double lambda_t2 = std::norm(model->ckm_tb() * conj(model->ckm_ts()));

        const double norm = g_fermi() * sqrt(1.0 / 3.0 / 128.0 / power_of<4>(M_PI) / power_of<3>(m_Lb) * lambda_t2 * alpha_e() * (power_of<2>(m_Lb) - power_of<2>(m_Lstar)));

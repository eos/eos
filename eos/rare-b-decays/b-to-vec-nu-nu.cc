...
namespace eos
{
    ...
    template <> struct Implementation<BToVectorDineutrino>
    {
                ...
                form_factors(FormFactorFactory<PToV>::create(_process() + "::" + o.get("form-factors"_ok, "BSZ2015"_ov).str(), p, o))
            {
                ...
                switch (opt_D.value())
                {
                    ...
                    default: throw InternalError("Unexpected quark flavor: '" + opt_D.str() + "'");
                }
            ...
            differential_decay_width(const double & q2) const
            {
                ...
                const double norm = power_of<2>(4.0 * g_fermi() * alpha_e / (2.0 * M_PI)) / 2.0
                                    * std::norm(lambda_t())
                ...
                const double contr_axial =
                        8.0 * m_B2 * m_V2 / 3.0 * A12 * A12 * std::norm(wc.cVL() - wc.cVR()) + q2 * (m_B + m_V) * (m_B + m_V) / (12.0) * A1 * A1 * std::norm(wc.cVL() - wc.cVR());

                // third term in square brackets in [FLS:2021A], eq. (13)
                const double contr_scalar = q2 * lambda / (8.0 * (m_b + m_D) * (m_b + m_D)) * A0 * A0 * std::norm(wc.cSR() - wc.cSL());

                // fourth term in square brackets in [FLS:2021A], eq. (13)
                const double contr_tensor = q2
                                            * (32.0 * m_B2 * m_V2 / (3.0 * (m_B + m_V) * (m_B + m_V)) * T23 * T23
            ...
            double
            differential_longitudinal_width(const double & q2) const
            {
                const double m_B = this->m_B(), m_B2 = m_B * m_B;
                const double m_V = this->m_V(), m_V2 = m_V * m_V;
    ...
    const std::vector<OptionSpecification> Implementation<BToVectorDineutrino>::options{
        Model::option_specification(),  FormFactorFactory<PToV>::option_specification(), { "cp-conjugate"_ok, { "true"_ov, "false"_ov }, "false"_ov },
        { "D"_ok, { "s"_ov }, "s"_ov },
    ...
    {
        std::function<double(const double &)> f = std::bind(&Implementation<BToVectorDineutrino>::differential_branching_ratio, _imp.get(), std::placeholders::_1);
    ...
    {
        std::function<double(const double &)> longitudinal_width =
                std::bind(&Implementation<BToVectorDineutrino>::differential_longitudinal_width, _imp.get(), std::placeholders::_1);
        std::function<double(const double &)> decay_width = std::bind(&Implementation<BToVectorDineutrino>::differential_decay_width, _imp.get(), std::placeholders::_1);

        return integrate<GSL::QAGS>(longitudinal_width, q2_min, q2_max, _imp->int_config) / integrate<GSL::QAGS>(decay_width, q2_min, q2_max, _imp->int_config);
    }

    const std::string BToVectorDineutrino::description = "\
    The decay B->P nu nu, where both B=(B qbar) and P=(U qbar) are pseudoscalars, and l=e,mu,tau is a lepton.";

    const std::string BToVectorDineutrino::kinematics_description_q2 = "\
    The invariant mass of the nu-nubar pair in GeV^2.";

    const std::set<ReferenceName> BToVectorDineutrino::references{ "BGNS:2014A"_rn };

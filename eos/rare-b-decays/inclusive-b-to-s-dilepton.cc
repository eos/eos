...
namespace eos
{
    ...
    template <> struct Implementation<BToXsDilepton<HLMW2005>>
    {
            ...
            double
            omega1_99(const double & s_hat) const
            {
                double li2 = gsl_sf_dilog(s_hat);
                double ln = log(s_hat), ln1 = log(1.0 - s_hat);
                double s_hat2 = s_hat * s_hat;

                return -4.0 / 3.0 * li2 - 2.0 / 3.0 * ln1 * ln - 2.0 / 9.0 * M_PI * M_PI - (5.0 + 4.0 * s_hat) / (3.0 * (1.0 + 2.0 * s_hat)) * ln1
            ...
            double
            omega2_99(const double & s_hat) const
            {
                double ln = log(s_hat);
                ;
            ...
            double
            omega1_77(const double & s_hat) const
            {
                double li2 = gsl_sf_dilog(s_hat);
                double ln = log(s_hat), ln1 = log(1.0 - s_hat);
                double s_hat2 = s_hat * s_hat;

                return -4.0 / 3.0 * li2 - 2.0 / 3.0 * ln1 * ln - 2.0 / 9.0 * M_PI * M_PI - (8.0 + s_hat) / (3.0 * (2.0 + s_hat)) * ln1
            ...
            double
            omega1_79(const double & s_hat) const
            {
                double li2 = gsl_sf_dilog(s_hat);
                double ln = log(s_hat), ln1 = log(1.0 - s_hat);

                return -4.0 / 3.0 * li2 - 2.0 / 3.0 * ln1 * ln - 2.0 / 9.0 * M_PI * M_PI - (2.0 + 7.0 * s_hat) / (9.0 * s_hat) * ln1
            ...
            // i = { 1 .. 10, Q3 .. Q6, b }
            complex<double>
            f(unsigned i, const double & s_hat) const
            {
                if ((7 == i) || (8 == i))
                {
                ...
                return gamma9[i - 1] * log(m_b_MSbar / mu) + rho_c[i - 1] * (g_c + 8.0 / 9.0 * log(m_b / m_c)) + rho_b[i - 1] * g_b
                       + rho_0[i - 1] * complex<double>(log(s_hat), -M_PI) + rho_sharp[i - 1];
            }

            complex<double>
            f9pen(const double & s_hat) const
            {
                complex<double> g_tau = g(4.0 * power_of<2>(m_tau / m_b_pole()) / s_hat);

                return 8.0 * log(m_b_MSbar / mu) - 3.0 * (g_tau + 8.0 / 9.0 * log(m_b_MSbar / m_tau)) + 8.0 / 3.0 * std::complex<double>(log(s_hat), -M_PI) - 40.0 / 9.0;
            }

            // cf. [HLMW:2005A], Eq. (132), p. 29
            complex<double>
            F(const double & s_hat) const
            {
                ...
                {
                    result = 3.0 / (2.0 * r) * (complex<double>(log((1.0 - x) / (1.0 + x)), M_PI) / (2.0 * sqrt(r * (r - 1.0))) - 1.0);
                }
            ...
            phi_ll(const double & q2) const
            {
                ...
                double s77 = power_of<2>(1.0 - s_hat) * (4.0 + 8.0 / s_hat)
                                     * (1.0 + 8.0 * alpha_s_tilde * (omega1_77(s_hat) + u1) + kappa * uem
                                        + 8.0 * alpha_s_tilde * kappa * EMContributions::omegaem_77(s_hat, log_m_l_hat))
                             + lambda_1_hat * power_of<2>(1.0 - s_hat) * (2.0 + 4.0 / s_hat) + lambda_2_hat * (30.0 * s_hat2 - 18.0 - 36.0 / s_hat);

                double s79 = 12.0 * power_of<2>(1.0 - s_hat)
                                     * (1.0 + 8.0 * alpha_s_tilde * (omega1_79(s_hat) + u1) + kappa * uem
                                        + 8.0 * alpha_s_tilde * kappa * EMContributions::omegaem_79(s_hat, log_m_l_hat))
                             + 6.0 * lambda_1_hat * power_of<2>(1.0 - s_hat) + 6.0 * lambda_2_hat * (-5.0 - 6.0 * s_hat + 7.0 * s_hat2);

                double s99 =
                        power_of<2>(1.0 - s_hat) * (1.0 + 2.0 * s_hat)
                                * (1.0 + 8.0 * alpha_s_tilde * (omega1_99(s_hat) + u1) + kappa * uem + 8.0 * alpha_s_tilde * kappa * EMContributions::omegaem_99(s_hat, log_m_l_hat)
                                   + 16.0 * power_of<2>(alpha_s_tilde) * (omega2_99(s_hat) + u2 + 4.0 * u1 * omega1_99(s_hat)))
                        + lambda_1_hat * 0.5 * power_of<2>(1.0 - s_hat) * (1.0 + 2.0 * s_hat) + lambda_2_hat * 1.5 * (1.0 - 15.0 * s_hat2 + 10.0 * s_hat3);

                double s1010 = s99
                               + 8.0 * alpha_s_tilde * kappa * power_of<2>(1.0 - s_hat) * (1.0 + 2.0 * s_hat)
                                         * (EMContributions::omegaem_1010(s_hat, log_m_l_hat) - EMContributions::omegaem_99(s_hat, log_m_l_hat));

                /* Wilson coefficients */
                std::vector<complex<double>> wc = { w.c1(),
                                                    w.c2(),
                                                    w.c3(),
                                                    w.c4(),
                                                    w.c5(),
                ...
                {
                    /* diagonal */
                    phi_ll += norm(wc[i]) * real(s77 * norm(m7[i]) + s99 * norm(m9[i]) + s1010 * norm(m10[i]) + s79 * m7[i] * conj(m9[i]));

                    /* upper */
                    for (unsigned j(i + 1); j < 14; ++j)
                    {
                        phi_ll += real(wc[i] * conj(wc[j])
                                       * (2.0 * s77 * m7[i] * conj(m7[j]) + 2.0 * s99 * m9[i] * conj(m9[j]) + 2.0 * s1010 * m10[i] * conj(m10[j])
                 ...
                 */
                {
                    ...
                    phi_ll += norm(wc7prime) * s77 * norm(m7[7 - 1]) + norm(wc9prime) * s99 * norm(m9[9 - 1]) + norm(wc10prime) * s1010 * norm(m10[10 - 1])
                    ...
                    phi_ll += real(wc[7 - 1] * m7[7 - 1] * conj(wc7prime * m7[7 - 1]) * s77p) + real(wc[7 - 1] * m7[7 - 1] * conj(wc9prime * m9[9 - 1]) * s79p)
                              + real(wc[9 - 1] * m9[9 - 1] * conj(wc7prime * m7[7 - 1]) * s97p) + real(wc[9 - 1] * m9[9 - 1] * conj(wc9prime * m9[9 - 1]) * s99p)
                              + real(wc[10 - 1] * m10[10 - 1] * conj(wc10prime * m10[10 - 1]) * s1010p);

                    /* we also include c8' contributions */
                    phi_ll += norm(wc8prime) * (norm(m7[8 - 1]) * s77 + norm(m9[8 - 1]) * s99 + real(m7[8 - 1] * conj(m9[8 - 1])) * s79);
                }
                ...
                // relation.
                double          c27 =
                        -power_of<2>(alpha_s_tilde * kappa) * 8.0 * mu2_g / (27.0 * power_of<2>(m_c)) * power_of<2>(1.0 - s_hat) * (1.0 + 6.0 * s_hat - s_hat2) / s_hat * real(cF);
                ...
                complex<double> e29 =
                        8.0 * power_of<2>(1.0 - s_hat) * (1.0 + 2.0 * s_hat) * power_of<2>(alpha_s_tilde * kappa) * EMContributions::omegaem_29(s_hat, log_m_l_hat, mu);
            ...
            double
            branching_ratio(const double & q2) const
            {
                static const double pi3 = power_of<3>(M_PI);

                return power_of<2>(gfermi()) * power_of<5>(m_b_pole()) * std::norm(model->ckm_tb() * std::conj(model->ckm_ts())) * phi_ll(q2) * tau_B() / (48.0 * pi3 * hbar());
            }
    ...
    const std::vector<OptionSpecification> Implementation<BToXsDilepton<HLMW2005>>::options{
    ...
    {
        return integrate<1>(std::function<double(const double &)>(std::bind(&BToXsDilepton<HLMW2005>::differential_branching_ratio, this, std::placeholders::_1)),
                            q2_min,
                            q2_max,
                            _imp->cub_conf);
    }
    ...
    const std::set<ReferenceName> BToXsDilepton<HLMW2005>::references{};

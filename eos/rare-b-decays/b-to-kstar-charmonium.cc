...
namespace eos
{
     ...
     */
    template <> struct Implementation<BToKstarCharmonium>
    {
            ...
            // The amplitudes in the conventions of [T:2002A], eq. (2.38)
            ...
            AmplitudesExperimental
            amplitudes_experimental() const
            {
                static complex<double> I(0.0, 1.0);

                const auto amps = this->amplitudes_bcvdv2016();

                return { -I * amps.A_perp, -I * amps.A_para, +I * (m_B / m_psi) * amps.A_long };
            }
            ...
            branching_ratio() const
            {
                ...
                const auto prefactor = pow(g_fermi * abs(model->ckm_cb() * conj(model->ckm_cs())), 2) * tau_B() / hbar() * sqrt(lambda) / (2.0 * M_PI * m_B);
    ...
    const std::vector<OptionSpecification> Implementation<BToKstarCharmonium>::options{
    ...
    const std::set<ReferenceName> BToKstarCharmonium::references{ "KMPW:2010A"_rn, "GvDV:2020A"_rn };

...
namespace eos
{
    ...
    BsToPhiDileptonAmplitudes<tag::Naive>::options
    {
    };
    ...
    BsToPhiDileptonAmplitudes<tag::Naive>::sb_contributions(const double & q2, const WilsonCoefficients<BToS> & wc) const
    {
        ...
        complex<double> Y_top = Y_top_b * CharmLoops::h(mu, q2, m_b_PS)
        ...
        complex<double> vector_contribution = alpha_s_mu / 4.0 / M_PI * (
            ...
            + lambda_hat_u * ( wc.c1() * (memoise(CharmLoops::F19_massive_Qsb, q2) - CharmLoops::F19_massless(mu, q2, m_b_PS)) + wc.c2() * (memoise(CharmLoops::F29_massive_Qsb, q2) - CharmLoops::F29_massless(mu, q2, m_b_PS)))
        );
    ...
    BsToPhiDileptonAmplitudes<tag::Naive>::amplitudes(const double & q2) const
    {
        ...
        // local form factors
        const double
            ff_V  = form_factors->v(q2),
            ff_A0 = form_factors->a_0(q2),
            ff_A1 = form_factors->a_1(q2),
            ff_A2 = form_factors->a_2(q2),
        ...
        const double
            ...
            lambda      = eos::lambda(m_B2, m_V2, q2),
        ...
        const double
            ...
            calF_long = ((m_B2 - m_V2 - q2) * power_of<2>(m_B + m_V) * ff_A1 - lambda * ff_A2)
        ...
        const double
            ...
            calF_T_long = q2 / (2.0 * power_of<3>(m_B) * m_V) *
                        ((m_B2 + 3.0 * m_V2 - q2) * ff_T2 - lambda / (m_B2 - m_V2) * ff_T3);

        // Contributions not probortional to Qc
        auto sb_c = sb_contributions(q2, wc);

        const complex<double>
            calH_perp = nonlocal_formfactor->H_perp(q2) - 1.0 / 16.0 / power_of<2>(M_PI) * (calF_perp * sb_c.t + calF_T_perp * sb_c.t_T),
        ...
        const double calN = g_fermi() * alpha_e * abs(model->ckm_tb() * conj(model->ckm_ts()))
                * sqrt(q2 * beta_l(q2) * sqrt_lambda / (3.0 * 1024 * power_of<5>(M_PI) * m_B));

        // vector amplitudes, cf. KM2005A (3.2) - (3.4)
        result.a_long_right = -calN * m_B / sqrt_s * (
                c910_m_r * calF_long
                + 2.0 * m_B / q2 * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_long - 16.0 * power_of<2>(M_PI) * m_B * calH_long)
        );
        ...
        result.a_para_right = -calN * (
                c910_m_r * calF_para
                + 2.0 * m_B / q2 * ((m_b_msbar - m_s_msbar) * c7_m * calF_T_para - 16.0 * power_of<2>(M_PI) * m_B * calH_para)
        );
        ...
        result.a_perp_right = +calN * (
                c910_p_r * calF_perp
                + 2.0 * m_B / q2 * ((m_b_msbar + m_s_msbar) * c7_p * calF_T_perp - 16.0 * power_of<2>(M_PI) * m_B * calH_perp)
        );

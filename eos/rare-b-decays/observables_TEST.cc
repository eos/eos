...
static const std::set<std::string> expected_failures{
    // these are built from expressions, whose mandatory 'q' the entry does not expose
    "B->K^*ll::NormalizedBR",
    "B->K^*psi::BR",
    "B->K^*psi::S_1c@LHCb",
    "B->K^*psi::S_1s@LHCb",
    ...
    "B->K^*psi::S_8@LHCb",
    "B->K^*psi::S_9@LHCb",
    "B->K^*psi::delta_para_long",
    "B->K^*psi::delta_perp_long",
    "B->K^*psi::long_phase",
    "B->K^*psi::long_polarization",
    "B->K^*psi::para_polarization",
    "B->K^*psi::perp_polarization",

    // requests the parameter 'exp::BR(B->X_clnu)', which no file defines and which is not a valid name
    "B->X_sgamma::BR@Minimal",
};

class RareBDecaysObservablesTest : public TestCase
{
    public:
        RareBDecaysObservablesTest() :
            TestCase("rare_b_decays_observables_test")
        {
        }
        ...
        run() const
        {
            /* Test that every observable in this section can be constructed */
            {
                ...
                // these classes select their amplitude generator through 'tag', which has no default
                ...
                for (const auto & group : make_rare_b_decays_section())
                {
...
} rare_b_decays_observables_test;

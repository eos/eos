...
namespace eos
{
     ...
     */
    template <> struct Implementation<BToKCharmonium>
    {
            ...
            branching_ratio() const
            {
                ...
                const auto prefactor =
                        m_B * power_of<2>(g_fermi * abs(model->ckm_cb() * conj(model->ckm_cs())) / f_psi / power_of<2>(m_psi)) * tau_B() / hbar() * pow(lambda, 1.5) / (2.0 * M_PI);
    ...
    const std::vector<OptionSpecification> Implementation<BToKCharmonium>::options{
    ...
    const std::set<ReferenceName> BToKCharmonium::references{ "KMPW:2010A"_rn, "GvDV:2020A"_rn };

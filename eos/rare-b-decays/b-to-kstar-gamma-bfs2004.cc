...
namespace eos
{
        ...
        mu(p["sb::mu"], *this)
    {
        ...
        {
            qcdf_photon_massless_case = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Mixed>::photon_massless_case, _1, _2, _3, _4, _5, _6, _7);
            qcdf_photon_charm_case    = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Mixed>::photon_charm_case, _1, _2, _3, _4, _5, _6, _7, _8);
            qcdf_photon_bottom_case   = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Mixed>::photon_bottom_case, _1, _2, _3, _4, _5, _6, _7, _8);
        }
        else if ("analytical" == qcdf_integrals)
        {
            qcdf_photon_massless_case = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Analytical>::photon_massless_case, _1, _2, _3, _4, _5, _6, _7);
            qcdf_photon_charm_case    = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Analytical>::photon_charm_case, _1, _2, _3, _4, _5, _6, _7, _8);
            qcdf_photon_bottom_case   = std::bind(&QCDFIntegralCalculator<BToKstarDilepton, tag::Analytical>::photon_bottom_case, _1, _2, _3, _4, _5, _6, _7, _8);
        }
        else
        {
    ...
    BToKstarGammaAmplitudes<tag::BFS2004>::amplitudes() const
    {
        ...
        // cf. [BFS:2001A], Eqs. (12), (15), p. 5, in comparison with \delta_1 = 1, s -> 0, +/- -> left/right handed
        complex<double> C0_top_perp_left   = c7eff;
        complex<double> C0_top_perp_right  = wc.c7prime();
        // cf. [BFS:2004A], Eq. (44), p. 24, s -> 0
        ...
        // cf. [BFS:2001A], Eqs. (34), (37), p. 9, s -> 0
        complex<double> C1nf_top_perp_left =
                (-1.0 / QCD::casimir_f)
        ...
        // cf. [BFS:2004A], comment before Eq. (43), p. 24, s -> 0
        ...
        complex<double>       C1nf_up_perp_left =
                (-1.0 / QCD::casimir_f) * ((wc.c2() - wc.c1() / 6.0) * (memoise(CharmLoops::F27_massive, mu(), 0.0, m_b_PS, m_c_pole) - CharmLoops::F27_massless(mu, 0.0, m_b_PS)));
        const complex<double> C1nf_up_perp_right = 0.0;

        // compute the factorizing contributions
        complex<double> C_perp_left  = C0_top_perp_left + lambda_hat_u * C0_up_perp_left + a_mu * (C1f_top_perp_left + C1nf_top_perp_left + lambda_hat_u * C1nf_up_perp_left);
        complex<double> C_perp_right = C0_top_perp_right + lambda_hat_u * C0_up_perp_right + a_mu * (C1f_top_perp_right + C1nf_top_perp_right + lambda_hat_u * C1nf_up_perp_right);
        ...
        complex<double> Delta_T_ann_top_perp = e_q * M_PI * M_PI * f_B / 3.0 / m_b_PS / m_B
                                               * (-4.0 * f_Kstar_perp * (wc.c3() + 4.0 / 3.0 * (wc.c4() + 3.0 * wc.c5() + 4.0 * wc.c6())) * qcdf_0.j0_perp
                                                  + 2.0 * f_Kstar_par * (wc.c3() + 4.0 / 3.0 * (wc.c4() + 12.0 * wc.c5() + 16.0 * wc.c6())) * (m_Kstar * lambda_B_p_inv));
        complex<double> Delta_T_ann_up_perp = -e_q * 2.0 * M_PI * M_PI * f_B * f_Kstar_par / 3.0 / m_b_PS / m_B * (m_Kstar * lambda_B_p_inv) * 3.0 * delta_qu * wc.c2();
        ...
        complex<double> Delta_T_hsa_up_perp = e_q * a_mu_f * (M_PI * M_PI * f_B / (3.0 * m_b_PS * m_B))
                                              * (+8.0 * f_Kstar_perp * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0) * (qcdf_c.j5_perp - qcdf_0.j5_perp)
                                                 - (4.0 * m_Kstar * f_Kstar_par * lambda_B_p_inv) * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0) * (qcdf_c.j6_perp - qcdf_0.j6_perp));
        ...
        double calN = sqrt(alpha_e * power_of<3>(m_B) * power_of<3>(1.0 - m_Kstar * m_Kstar / (m_B * m_B)) / (32.0 * power_of<4>(M_PI))) * g_fermi * model->m_b_msbar(mu())
        ...
        complex<double> a_left = complex<double>(0.0, +1.0) * calN
                                 * (xi_perp_zero * C_perp_left
                                    + (uncertainty_perp() + uncertainty_para()) / 2.0 * (power_of<2>(M_PI) / 3.0 * (f_B * f_Kstar_perp) / m_B * T_perp_left + Delta_T_perp)
                                    + (uncertainty_perp() - uncertainty_para()) / 2.0 * (power_of<2>(M_PI) / 3.0 * (f_B * f_Kstar_perp) / m_B * T_perp_right + Delta_T_perp));

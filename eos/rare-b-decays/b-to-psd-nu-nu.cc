...
namespace eos
{
    ...
    template <> struct Implementation<BToPseudoscalarDineutrino>
    {
                ...
                form_factors(FormFactorFactory<PToP>::create(_process() + "::" + o.get("form-factors"_ok, "BSZ2015"_ov).str(), p, o))
            {
                ...
                switch (_D())
                {
                    ...
                    default: throw InternalError("Unexpected quark flavor: '" + stringify(_D()) + "'");
                }
    ...
    const std::vector<OptionSpecification> Implementation<BToPseudoscalarDineutrino>::options{
        Model::option_specification(),
        FormFactorFactory<PToP>::option_specification(),
        { "cp-conjugate"_ok, { "true"_ov, "false"_ov }, "false"_ov },
        { "P"_ok, { "K"_ov, "eta"_ov, "eta_prime"_ov } },
    ...
    const std::string BToPseudoscalarDineutrino::kinematics_description_q2 = "\
    The invariant mass of the nu-nubar pair in GeV^2.";

    const std::set<ReferenceName> BToPseudoscalarDineutrino::references{ "BGNS:2014A"_rn };

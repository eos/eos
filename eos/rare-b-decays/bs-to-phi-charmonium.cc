...
namespace eos
{
     ...
     */
    template <> struct Implementation<BsToPhiCharmonium>
    {
            ...
            amplitudes_experimental() const
            {
                ...
                return { -I * amps.A_perp, -I * amps.A_para, +I * (m_Bs / m_psi) * amps.A_long };
            }
            ...
            branching_ratio() const
            {
                ...
                const auto prefactor = power_of<2>(g_fermi * abs(model->ckm_cb() * conj(model->ckm_cs()))) * tau_Bs() / hbar() * sqrt(lambda) / (2.0 * M_PI * m_Bs);
    ...
    const std::vector<OptionSpecification> Implementation<BsToPhiCharmonium>::options{
    ...
    const std::set<ReferenceName> BsToPhiCharmonium::references{ "KMPW:2010A"_rn, "GvDV:2020A"_rn };

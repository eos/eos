...
namespace eos
{
    ...
    BsToPhiDileptonAmplitudes<tag::BFS2004>::BsToPhiDileptonAmplitudes(const Parameters & p, const Options & o) :
    ...
    const std::vector<OptionSpecification> BsToPhiDileptonAmplitudes<tag::BFS2004>::options{
        { "ccbar-resonance"_ok, { "true"_ov, "false"_ov }, "false"_ov },
        {             "nlo"_ok, { "true"_ov, "false"_ov },  "true"_ov },
    };

    BsToPhiDilepton::DipoleFormFactors
    BsToPhiDileptonAmplitudes<tag::BFS2004>::dipole_form_factors(const double & q2, const WilsonCoefficients<BToS> & wc) const
    {
        ...
        QCDFIntegrals<BToKstarDilepton> qcdf_0     = this->qcdf_dilepton_massless_case(q2, m_B, m_V, mu, a_1_perp, a_2_perp, a_1_par, a_2_par);
        QCDFIntegrals<BToKstarDilepton> qcdf_c     = this->qcdf_dilepton_charm_case(q2, m_c_pole, m_B, m_V, mu, a_1_perp, a_2_perp, a_1_par, a_2_par);
        QCDFIntegrals<BToKstarDilepton> qcdf_b     = this->qcdf_dilepton_bottom_case(q2, m_b_PS, m_B, m_V, mu, a_1_perp, a_2_perp, a_1_par, a_2_par);
        ...
        complex<double> T0_top_par_m = -e_s * 4.0 * m_B / m_b_PS
                                       * (wc.c3() + 4.0 / 3.0 * wc.c4() + 16.0 * wc.c5() + 64.0 / 3.0 * wc.c6() + 12.0 * wc.c3() + 120.0 * wc.c5() // TODO CHECK
                                          )
                                       * lambda_B_m_inv;
        ...
        complex<double> T1nf_top_par_m = e_s
        ...
        complex<double> Delta_T_ann_top_perp =
                e_s * M_PI * M_PI * f_Bs / 3.0 / m_b_PS / m_B
                * (-4.0 * f_phi_perp * (wc.c3() + 4.0 / 3.0 * (wc.c4() + 3.0 * wc.c5() + 4.0 * wc.c6())) * qcdf_0.j0_perp
                   + 2.0 * f_phi_par
                             * (wc.c3() + 4.0 / 3.0 * wc.c4() + 16.0 * wc.c5() + 64.0 / 3.0 * wc.c6() + 12.0 * wc.c3() + 120.0 * wc.c5() // TODO CHECK
                                )
                             * (m_V / (1.0 - q2 / (m_B * m_B)) * lambda_B_p_inv));
        complex<double> Delta_T_ann_up_perp = -e_s * 2.0 * M_PI * M_PI * f_Bs * f_phi_par / 3.0 / m_b_PS / m_B * (m_V / (1.0 - q2 / (m_B * m_B)) * lambda_B_p_inv)
                                              * (-4.0 / 3.0 * wc.c1() - wc.c2() // TODO CHECK
                                              );
        ...
        complex<double> Delta_T_hsa_top_perp = e_s * a_mu_f * (M_PI * M_PI * f_Bs / (3.0 * m_b_PS * m_B)) * (
                ...
                - (4.0 * m_V * f_phi_par / (1.0 - q2 / (m_B * m_B)) * lambda_B_p_inv) * (3.0 / 4.0) * (
        ...
        complex<double> Delta_T_hsa_up_perp =
                e_s * a_mu_f * (M_PI * M_PI * f_Bs / (3.0 * m_b_PS * m_B))
                * (+8.0 * f_phi_perp * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0) * (qcdf_c.j5_perp - qcdf_0.j5_perp)
                   - (4.0 * m_V * f_phi_par / (1.0 - q2 / (m_B * m_B)) * lambda_B_p_inv) * (3.0 / 4.0) * (wc.c2() - wc.c1() / 6.0) * (qcdf_c.j6_perp - qcdf_0.j6_perp));
        ...
        result.calT_perp_left  = xi_perp(q2) * C_perp_left + power_of<2>(M_PI) / 3.0 * (f_Bs * f_phi_perp) / m_B * T_perp_left + Delta_T_perp;
        result.calT_perp_right = xi_perp(q2) * C_perp_right + power_of<2>(M_PI) / 3.0 * (f_Bs * f_phi_perp) / m_B * T_perp_right + Delta_T_perp;
        result.calT_parallel   = xi_par(q2) * C_par + power_of<2>(M_PI) / 3.0 * (f_Bs * f_phi_par * m_V) / (m_B * energy) * T_par;

        return result;
    }

    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::xi_perp(const double & q2) const
    {
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::xi_par(const double & q2) const
    {
        const double factor1 = (m_B() + m_V()) / (2.0 * energy(q2));
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::norm(const double & q2) const
    {
    ...
    BsToPhiDileptonAmplitudes<tag::BFS2004>::amplitudes(const double & q2) const
    {
        ...
        const double shat = s_hat(q2), mbhat = m_b_PS() / m_B, mKhat2 = power_of<2>(m_V() / m_B()), m_K2 = power_of<2>(m_V()), m_B2 = power_of<2>(m_B()), m2_diff = m_B2 - m_K2,
                     norm_s = this->norm(q2), sqrt_lam = std::sqrt(lambda(q2)), sqrt_s = std::sqrt(q2);
        ...
        const double prefactor_long = -norm_s / (2.0 * m_V() * std::sqrt(q2));
        ...
        const double ff_T1 = form_factors->t_1(q2), ff_T2 = form_factors->t_2(q2), ff_T3 = form_factors->t_3(q2),
                     ...
                     kin_tensor_1 = norm_s / m_V() * ((m_B2 + 3.0 * m_K2 - q2) * ff_T2 - lambda(q2) / m2_diff * ff_T3), kin_tensor_2 = 2.0 * norm_s * sqrt_lam / sqrt_s * ff_T1,
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::real_C9_perp(const double & q2) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::imag_C9_perp(const double & q2) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::real_C9_para(const double & q2) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);
    ...
    double
    BsToPhiDileptonAmplitudes<tag::BFS2004>::imag_C9_para(const double & q2) const
    {
        WilsonCoefficients<BToS> wc = model->wilson_coefficients_b_to_s(mu(), lepton_flavor, cp_conjugate);

...
class BToKstarDileptonLowRecoilTest : public TestCase
{
    public:
...
class BToKstarDileptonLowRecoilPolynomialTest : public TestCase
{
    public:
        ...
        run() const
        {
            // Test make_polynomial
            {
                ...
                for (const auto & name : names)
                {
                    ...
                    auto          polynomial = make_polynomial(observable,
            ...
            // Test ratios
            {
                ...
                Options options;
                options.declare("model"_ok, "WET"_ov);
                options.declare("tag"_ok, "GP2004"_ov);
                ...
                std::vector<QualifiedName> coefficients{ "b->s::Re{c7}"_qn,     "b->s::Im{c7}"_qn,      "b->smumu::Re{c9}"_qn,
                                                         "b->smumu::Im{c9}"_qn, "b->smumu::Re{c10}"_qn, "b->smumu::Im{c10}"_qn };
...
class BToKstarDileptonLowRecoilBobethCompatibilityTest : public TestCase
{
    public:
        ...
        run() const
        {
            static const std::vector<std::string> variation_names{
                "b->s::Re{c7}",      "b->s::Im{c7}",      "b->s::Re{c7'}",     "b->s::Im{c7'}",     "b->smumu::Re{c9}",   "b->smumu::Im{c9}",
                "b->smumu::Re{c9'}", "b->smumu::Im{c9'}", "b->smumu::Re{c10}", "b->smumu::Im{c10}", "b->smumu::Re{c10'}", "b->smumu::Im{c10'}",
            };
            ...
            Options o;
            o.declare("model"_ok, "WET"_ov);
            o.declare("l"_ok, "mu"_ov);
            o.declare("q"_ok, "d"_ov);
            o.declare("tag"_ok, "GP2004"_ov);
            o.declare("form-factors"_ok, "BSZ2015"_ov);
            ...
            std::vector<std::string>   observable_names = { "B->K^*ll::BR_CP_specific", "B->K^*ll::A_FB_CP_specific", "B->K^*ll::F_L_CP_specific" };
...
class BToKstarDileptonTensorLowRecoilBobethCompatibilityTest : public TestCase
{
    public:
        ...
        run() const
        {
            ...
            p["B->K^*::b^V_1@KMPW2010"]   = -4.8;
            p["B->K^*::F^A0(0)@KMPW2010"] = +0.29;
            p["B->K^*::b^A0_1@KMPW2010"]  = -18.2;
            p["B->K^*::F^A1(0)@KMPW2010"] = +0.25;
            p["B->K^*::b^A1_1@KMPW2010"]  = +0.34;
            p["B->K^*::F^A2(0)@KMPW2010"] = +0.23;
            p["B->K^*::b^A2_1@KMPW2010"]  = -0.85;
            p["B->K^*::F^T1(0)@KMPW2010"] = +0.31;
            p["B->K^*::b^T1_1@KMPW2010"]  = -4.6;
            p["B->K^*::F^T2(0)@KMPW2010"] = +0.31;
            p["B->K^*::b^T2_1@KMPW2010"]  = -3.2;
            p["B->K^*::F^T3(0)@KMPW2010"] = +0.22;
            p["B->K^*::b^T3_1@KMPW2010"]  = -10.3;

            Options oo;
            oo.declare("model"_ok, "WET"_ov);
            oo.declare("scan-mode"_ok, "cartesian"_ov);
            oo.declare("tag"_ok, "GP2004"_ov);
            oo.declare("form-factors"_ok, "KMPW2010"_ov);
            ...
            {
                oo.declare("cp-conjugate"_ok, "true"_ov);
                BToKstarDilepton d(p, oo);
...

            {
                ...
                observables.insert("B->K^*ll::J_6c_cp_averaged(q2)",
                                   R"()",
                                   Unit::None(),
                                   Options(),
                                   R"(
                ...
                observables.insert("B->K^*ll::J1c_plus_J2c_cp_averaged(q2)",
                                   R"()",
                                   Unit::None(),
                                   Options(),
                                   R"(
                ...
                observables.insert("B->K^*ll::J1s_minus_3J2s_cp_averaged(q2)",
                                   R"()",
                                   Unit::None(),
                                   Options(),
                                   R"(
                ...
                TEST_CHECK_RELATIVE_ERROR(J1c_plus_J2c_avg->evaluate(), 0.5 * (4.48478951e-20 + 4.668428684e-20 + 6.966335387e-21 + 6.893410893e-21), eps);
                TEST_CHECK_RELATIVE_ERROR(J1s_minus_3J2s_avg->evaluate(), 0.5 * (6.080153751e-20 + 6.154137843e-20 - 3.0 * (-6.418495462e-21 - 6.424911528e-21)), eps);
            }
        }

...
namespace eos
{
    ...
    {
        struct GaussianBlock : public LogLikelihoodBlock
        {
        ...
        struct LogGammaBlock : public LogLikelihoodBlock
        {
                ObservableCache cache;
                ...
                LogGammaBlock(const ObservableCache & cache, ObservableCache::ObservableId id, const double & min, const double & central, const double & max, const double & alpha,
                              const double & lambda, const unsigned & number_of_observations) :
                    cache(cache),
                    id(id),
                    central(central),
                    sigma_lower(central - min),
                    ...
                    _number_of_observations(number_of_observations)
                {
                    ...
                    {
                        Log::instance()->message("LogLikelihoodBlock::LogGamma.ctor", ll_warning) << "For nearly symmetric uncertainties (" << sigma_lower << " vs " << sigma_upper
                    ...
                    if (std::abs(cdf(central + sigma_upper) - cdf(central - sigma_lower) - 0.68268949213708585) > eps_cdf)
                    {
                        throw InternalError("LogLikelihoodBlock::LogGamma.ctor: For the current parameter values, "
                                            "the interval [lower, upper] doesn't contain approx. 68%; contents is "
                                            + stringify(cdf(central + sigma_upper) - cdf(central - sigma_lower)));
                    }
                    ...
                    if (std::fabs(alpha * z_plus - std::exp(z_plus) - alpha * z_minus + std::exp(z_minus)) > eps_pdf)
                    {
                        throw InternalError("LogLikelihoodBlock::LogGamma.ctor: For the current parameter values, "
                                            ...
                                            + stringify(std::fabs(alpha * z_plus - std::exp(z_plus) - alpha * z_minus + std::exp(z_minus))));
                    }
                ...
                significance() const
                {
                    ...
                    if (GSL_SUCCESS != status)
                    {
                        Log::instance()->message("LogGammaBlock::significance", ll_error)
                                << "Could not find the mirror point, stopped after " << iter << " iterations with f(" << mirror << ") = " << f.f(mirror, f.params);
                    }
                 ...
                 */
                static double
                significance_function_f(double x, void * data)
                {
                ...
                static double
                significance_function_df(double x, void * data)
                {
                    LogGammaBlock * log_gamma = static_cast<LogGammaBlock *>(data);
                ...
                static void
                significance_function_fdf(double x, void * data, double * f, double * df)
                {
                    LogGammaBlock * log_gamma = static_cast<LogGammaBlock *>(data);
                ...
                clone(ObservableCache cache) const
                {
                    ...
                    return LogLikelihoodBlockPtr(
                            new LogGammaBlock(cache, cache.add(observable), central - sigma_lower, central, central + sigma_upper, alpha, lambda, _number_of_observations));
                }
        };

        struct AmorosoBlock : public LogLikelihoodBlock
        {
                ObservableCache cache;
                ...
                AmorosoBlock(const ObservableCache & cache, ObservableCache::ObservableId id, const double & physical_limit, const double & theta, const double & alpha,
                             const double & beta, const unsigned & number_of_observations) :
                    cache(cache),
                    id(id),
                    ...
                    _number_of_observations(number_of_observations)
                {
                    if (theta <= 0)
                    {
                        throw InternalError("LogLikelihoodBlock::Amoroso: scale parameter theta (" + stringify(theta) + ") must be positive for an upper limit");
                    }
                    if (alpha <= 0)
                    {
                        throw InternalError("LogLikelihoodBlock::Amoroso: shape parameter alpha (" + stringify(alpha) + ") must be positive");
                    }
                    if (beta <= 0)
                    {
                        throw InternalError("LogLikelihoodBlock::Amoroso: shape parameter beta (" + stringify(beta) + ") must be positive");
                    }
                ...
                significance() const
                {
                    ...
                    if (GSL_SUCCESS != status)
                    {
                        throw InternalError("Could not find the mirror point, stopped after " + stringify(iter) + " iterations with f(" + stringify(estimate)
                                            + ") = " + stringify(f.function(estimate, f.params)));
                    }
                ...
                significance_function_f(double x, void * data)
                {
                    ...
                    // the function value
                    return (a->alpha * a->beta - 1) * (std::log(zp) - std::log(zm)) + std::pow(zm, a->beta) - std::pow(zp, a->beta);
                }
        ...
        struct MixtureBlock : public LogLikelihoodBlock
        {
                ...
                MixtureBlock(const std::vector<LogLikelihoodBlockPtr> & components, const std::vector<double> & weights, const std::vector<std::array<double, 2>> & test_stat) :
                ...
                primary_test_statistic() const
                {
                    return test_statistics::ChiSquare(gsl_cdf_chisq_Pinv(gsl_cdf_chisq_P(power_of<2>(significance()), 1), number_of_observations()), number_of_observations());
                }
        };

        struct MultivariateGaussianBlock : public LogLikelihoodBlock
        {
                ...
                MultivariateGaussianBlock(const ObservableCache & cache, const std::vector<ObservableCache::ObservableId> && ids, gsl_vector * mean, gsl_matrix * covariance,
        ...
        struct UniformBoundBlock : public LogLikelihoodBlock
        {
                ObservableCache cache;
                ...
                UniformBoundBlock(const ObservableCache & cache, const std::vector<ObservableCache::ObservableId> && ids, const double & bound, const double & uncertainty) :
                    cache(cache),
                    ids(ids),
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::Gaussian(ObservableCache cache, const ObservablePtr & observable, const double & min, const double & central, const double & max,
                                 const unsigned & number_of_observations)
    {
        // check input
        if (min >= central)
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::LogGamma(ObservableCache cache, const ObservablePtr & observable, const double & min, const double & central, const double & max, const double & alpha,
                                 const double & lambda, const unsigned & number_of_observations)
    {
        // check input
        if (min >= central)
        ...
        if (alpha <= 0)
        {
            throw InternalError("LogLikelihoodBlock::LogGamma: shape parameter alpha (" + stringify(alpha) + ") must be positive");
        }
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::Amoroso(ObservableCache cache, const ObservablePtr & observable, const double & physical_limit, const double & upper_limit_10,
                                ...
                                const unsigned & number_of_observations)
    {
        ...
        {
            throw InternalError("LogLikelihood::Amoroso.ctor: For the current parameter values, cdf(x_10) = " + stringify(a->cdf(upper_limit_10)) + " deviates from 10%.");
        }
        if (std::abs(a->cdf(upper_limit_50) - 0.50) > 1e-4)
        {
            throw InternalError("LogLikelihood::Amoroso.ctor: For the current parameter values, cdf(x_50) = " + stringify(a->cdf(upper_limit_50)) + " deviates from 50%.");
        }
        if (std::abs(a->cdf(upper_limit_90) - 0.90) > 1e-4)
        {
            throw InternalError("LogLikelihood::Amoroso.ctor: For the current parameter values, cdf(x_90) = " + stringify(a->cdf(upper_limit_90)) + " deviates from 90%.");
        }
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::Amoroso(ObservableCache cache, const ObservablePtr & observable, const double & physical_limit, const double & theta, const double & alpha,
                                const double & beta, const unsigned & number_of_observations)
    {
        auto id = cache.add(observable);
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::Mixture(const std::vector<LogLikelihoodBlockPtr> & components, const std::vector<double> & weights, const std::vector<std::array<double, 2>> & test_stat)
    {
        ...
        std::transform(weights.cbegin(), weights.cend(), norm_weights.begin(), std::bind(std::multiplies<double>(), 1.0 / sum, std::placeholders::_1));
        Log::instance()->message("MixtureBlock()", ll_debug) << "sum = " << sum << ", norm. weights " << stringify_container(norm_weights);

        return LogLikelihoodBlockPtr(new implementation::MixtureBlock(components, norm_weights, test_stat));
    }

    LogLikelihoodBlockPtr
    LogLikelihoodBlock::MultivariateGaussian(ObservableCache cache, const std::vector<ObservablePtr> & observables, gsl_vector * mean, gsl_matrix * covariance,
                                             gsl_matrix * response, const unsigned & number_of_observations)
    {
        std::vector<ObservableCache::ObservableId> ids;
    ...
    LogLikelihoodBlockPtr
    LogLikelihoodBlock::UniformBound(ObservableCache cache, const std::vector<ObservablePtr> & observables, const double & bound, const double & uncertainty)
    {
        std::vector<ObservableCache::ObservableId> ids;
        for (auto & o : observables)
        {
    ...
    template <> struct WrappedForwardIteratorTraits<LogLikelihood::ConstraintIteratorTag>
    {
    ...
    template <> struct Implementation<LogLikelihood>
    {
            ...
            bootstrap_p_value(const unsigned & datasets)
            {
                ...
                Log::instance()->message("log_likelihood.bootstrap_pvalue", ll_informational) << "The value of the test statistic (total likelihood) "
                ...
                Log::instance()->message("log_likelihood.bootstrap_pvalue", ll_informational) << "Begin sampling " << datasets << " simulated "
                ...
                Log::instance()->message("log_likelihood.bootstrap_pvalue", ll_informational) << "The simulated p-value is " << p << " with uncertainty " << uncertainty;
    ...
    void
    LogLikelihood::add(const ObservablePtr & observable, const double & min, const double & central, const double & max, const unsigned & number_of_observations)
    {
        LogLikelihoodBlockPtr b = LogLikelihoodBlock::Gaussian(_imp->cache, observable, min, central, max, number_of_observations);
        _imp->constraints.push_back(Constraint(observable->name(), std::vector<ObservablePtr>{ observable }, std::vector<LogLikelihoodBlockPtr>{ b }));
    }

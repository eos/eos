...
namespace eos
{
    ...
    QCDFCoefficients<PToPP>::QCDFCoefficients(const Parameters & p, const Options & o) :
        ...
        mu(p[stringify(opt_q.value() == QuarkFlavor::down ? "s" : "d") + "bcu::mu"], *this),
    ...
    complex<double>
    QCDFCoefficients<PToPP>::ordered_amplitude() const
    {
        throw InternalError("QCDFCoefficients<PToPP>::ordered_amplitude() not implemented.");
        return 0.0;
    }

    complex<double>
    QCDFCoefficients<PToPP>::inverse_amplitude() const
    {
        throw InternalError("QCDFCoefficients<PToPP>::inverse_amplitude() not implemented.");
        return 0.0;
    }
    ...
    QCDFCoefficients<PToPP>::gvertex(const double & x) const
    {
        return 3.0 * ((1.0 - 2.0 * x) / (1.0 - x) * log(x) - complex<double>(0.0, 1.0) * M_PI) + 2.0 * gsl_sf_dilog(x) - power_of<2>(log(x)) + 2.0 * log(x) / (1.0 - x)
               - log(x) * (3.0 + 2.0 * complex<double>(0.0, 1.0) * M_PI)
               - (2.0 * gsl_sf_dilog(1.0 - x) - power_of<2>(log(1.0 - x)) + 2.0 * log(1.0 - x) / x - log(1.0 - x) * (3.0 + 2.0 * complex<double>(0.0, 1.0) * M_PI));
    }
    ...
    QCDFCoefficients<PToPP>::Vertex(const double & x) const
    {
        ...
        for (unsigned i = 0; i < 11; i++)
        {
            ...
            {
                Vi[i] = integrate<1, 1, complex<double>>(std::function<complex<double>(const double &)>([=, this](const double & x)
    ...
    QCDFCoefficients<PToPP>::HardSpec(const double & x, const double & y) const
    {
        ...
        for (unsigned i = 0; i < 11; i++)
        {
            ...
            if (i == 1 || i == 2 || i == 3 || i == 4 || i == 9 || i == 10)
            {
                ...
                {
                    auto inner_integral = std::function<complex<double>(const double &)>([=, this, x](const double & y)
                    { return lcdasP1->phi(y, mu()) * lcdasP2->phi(x, mu()) / ((1 - x) * (1 - y)) + rchi * lcdasP2->phi(x, mu()) * XH / x; });
                    return integrate<1, 1, complex<double>>(inner_integral, 0.0, 1.0);
                };

                Haux[i] = prefac * integrate<1, 1, complex<double>>(outer_integral, 0.0, 1.0);
            }
            ...
            else if (i == 5 || i == 7)
            {
                ...
                Haux[i] = -1.0 * prefac * integrate<1, 1, complex<double>>(outer_integral, 0.0, 1.0);
            }
    ...
    {
        return integrate<1, 1, complex<double>>(std::function<complex<double>(const double &)>([=, this](const double & x) { return lcdasP2->phi(x, mu()) * this->Gsx(s, x); }),
                                                0.0,
                                                1.0);
    }
    ...
    {
        double eps = 1e-5;
        return integrate<1, 1, complex<double>>(std::function<complex<double>(const double &)>([=, this](const double & x) { return this->Gsx(s - eps, 1.0 - x); }), 0.0, 1.0);
    }
    ...
    QCDFCoefficients<PToPP>::Penguin(const double & x) const
    {
        ...
        for (unsigned i = 0; i < 11; i++)
        {
            if (i == 4)
            {
                peng[i] = CF * alphas / (4.0 * M_PI * Nc)
                          * (wc2.c1() * (4.0 / 3.0 * log(mb / mus) + 2.0 / 3.0 - this->GM2(sp)) + wc[3] * (8.0 / 3.0 * log(mb / mus) + 4.0 / 3.0 - this->GM2(0.0) - this->GM2(1.0))
                             + (wc[4] + wc[6]) * (4.0 * nf / 3.0 * log(mb / mus) - (nf - 2.0) * this->GM2(0.0) - this->GM2(sc) - this->GM2(1.0))
                             - wc[8]
                                       * integrate<1, 1, complex<double>>(
                                               std::function<complex<double>(const double &)>([=, this](const double & x) { return lcdasP2->phi(x, mu()) / (1.0 - x); }),
                                               0.0,
                                               1.0));
            }
            ...
            {
                peng[i] = CF * alphas / (4.0 * M_PI * Nc)
                          * (wc[1] * (4.0 / 3.0 * log(mb / mus) + 2.0 / 3.0 - this->GM2hat(sp))
                             + wc[3] * (8.0 / 3.0 * log(mb / mus) + 4.0 / 3.0 - this->GM2hat(0.0) - this->GM2hat(1.0))
                             + (wc[4] + wc[6]) * (4.0 * nf / 3.0 * log(mb / mus) - (nf - 2.0) * this->GM2hat(0.0) - this->GM2hat(sc) - this->GM2hat(1.0)) - 2.0 * wc[8]);
            }
            ...
            {
                peng[i] = alpha / (9.0 * M_PI * Nc)
                          * ((wc[1] + Nc * wc[2]) * (4.0 / 3.0 * log(mb / mus) + 2.0 / 3.0 - this->GM2(sp))
                             - 3.0 * wc[7]
                                       * integrate<1, 1, complex<double>>(
                                               std::function<complex<double>(const double &)>([=, this](const double & x) { return lcdasP2->phi(x, mu()) / (1.0 - x); }),
                                               0.0,

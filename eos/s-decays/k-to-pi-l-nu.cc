...
namespace eos
{
    ...
    template <> struct Implementation<KToPiLeptonNeutrino>
    {
            ...
            Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
                ...
                tau_K(p["life_time::K_" + opt_q.str()], u),
                m_pi(p["mass::pi^" + std::string(opt_q.value() == QuarkFlavor::up ? "0" : "-")], u),
                opt_l(o, options, "l"),
                m_l(p["mass::" + opt_l.str()], u),
                g_fermi(p["WET::G_Fermi"], u),
                hbar(p["QM::hbar"], u),
                isospin_factor(opt_q.value() == QuarkFlavor::up ? 1.0 / std::sqrt(2.0) : 1.0), // K+ -> pi0 vs K0 -> pi^-
            ...
            amplitudes(const double & s) const
            {
                ...
                // form factors
                const complex<double> fp = form_factors->f_p(s);
                const complex<double> f0 = form_factors->f_0(s);
                // const complex<double> fT = form_factors->f_t(s);
                ...
                {
                    result.h_0 = isospin * 2.0 * m_K * p * fp * (1.0 + gV) / std::sqrt(s);
                    result.h_t = isospin * (1.0 + gV) * (m_K2 - m_pi2) * f0 / std::sqrt(s);
                    result.h_S = -isospin * gS * (m_K2 - m_pi2) * f0 / (msatmu - muatmu);
                    result.h_T = -isospin * 2.0 * m_K * p * fT * gT / (m_K + m_pi);
            ...
            pdf_w(const double & w) const
            {
                const double m_K = this->m_K(), m_K2 = m_K * m_K;
                const double m_pi = this->m_pi(), m_pi2 = m_pi * m_pi;
                const double q2 = m_K2 + m_pi2 - 2.0 * m_K * m_pi * w;

                return 2.0 * m_K * m_pi * pdf_q2(q2);
            }
            ...
            integrated_pdf_w(const double & w_min, const double & w_max) const
            {
                const double m_K = this->m_K(), m_K2 = m_K * m_K;
                const double m_pi = this->m_pi(), m_pi2 = m_pi * m_pi;
                const double q2_max = m_K2 + m_pi2 - 2.0 * m_K * m_pi * w_min;
                const double q2_min = m_K2 + m_pi2 - 2.0 * m_K * m_pi * w_max;
    ...
    const std::vector<OptionSpecification> Implementation<KToPiLeptonNeutrino>::options{
        Model::option_specification(), FormFactorFactory<PToP>::option_specification(), { "cp-conjugate", { "true", "false" }, "false" },
          {            "l",       { "e", "mu" },    "mu" },
        {            "q",        { "u", "d" },     "u" },
    };
    ...
    double
    KToPiLeptonNeutrino::differential_pdf_w(const double & w) const
    {
        return _imp->pdf_w(w);
    }
    ...
    double
    KToPiLeptonNeutrino::integrated_pdf_w(const double & w_min, const double & w_max) const
    {
        return _imp->integrated_pdf_w(w_min, w_max);
    }

    const std::string KToPiLeptonNeutrino::description = "\
    The decay K->pi l nu, where both K=(sbar q) and pi are pseudoscalars, and l=e,mu is a lepton.";

    const std::string KToPiLeptonNeutrino::kinematics_description_q2 = "\
    The invariant mass of the l-nubar pair in GeV^2.";

    const std::string KToPiLeptonNeutrino::kinematics_description_w = "\
    The recoil parameter of the K and pi states, with w=1 corresponding to zero recoil.";

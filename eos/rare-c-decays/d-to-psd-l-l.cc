...
namespace eos
{
    ...
    template <> struct Implementation<DToPseudoscalarLeptonLepton>
    {
            ...
            Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
                ...
                mu(p["uc::mu"], u)
            {
                Context ctx("When constructing D->psd l l observables");
            ...
            angular_coefficients_array(const DToPseudoscalarLeptonLepton::Amplitudes & A, const double & q2) const
            {
                ...
                result[1] = 2.0 * amplitude_generator->normalisation(q2)
                            * (q2 * (power_of<2>(beta_l) * std::real(A.F_S * std::conj(A.F_T)) + std::real(A.F_P * std::conj(A.F_T5)))
                               + m_l
                                         * (sqrt(lambda_D) * beta_l * std::real(A.F_S * std::conj(A.F_V))
                                            + (m_D() * m_D() - m_P() * m_P() + q2) * std::real(A.F_T5 * std::conj(A.F_A))));

                // c_l
                result[2] = amplitude_generator->normalisation(q2)
                            * (q2 * (power_of<2>(beta_l) * norm(A.F_T) + norm(A.F_T5))
                               - 0.25 * lambda_D * power_of<2>(beta_l) * (norm(A.F_A) + norm(A.F_V))
    ...
    const std::map<std::tuple<QuarkFlavor, std::string>, std::tuple<std::string, std::string>>
            Implementation<DToPseudoscalarLeptonLepton>::Implementation::process_map{
    ...
    double
    DToPseudoscalarLeptonLepton::two_differential_decay_width(const double & q2, const double & c_theta_l_LHCb) const
    {
        AngularCoefficients a = _imp->differential_angular_coefficients(q2);
    ...
    double
    DToPseudoscalarLeptonLepton::integrated_flat_term(const double & q2_min, const double & q2_max) const
    {
        AngularCoefficients a = _imp->integrated_angular_coefficients(q2_min, q2_max);
    ...
    double
    DToPseudoscalarLeptonLepton::integrated_forward_backward_asymmetry(const double & q2_min, const double & q2_max) const
    {
        AngularCoefficients a = _imp->integrated_angular_coefficients(q2_min, q2_max);

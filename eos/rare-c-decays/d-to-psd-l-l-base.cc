...
namespace eos
{
    ...
    DToPseudoscalarLeptonLepton::AmplitudeGenerator::AmplitudeGenerator(const Parameters & p, const Options & o) :
        ...
        mu(p["uc::mu"], *this),
        alpha_e(p["QED::alpha_e(m_c)"], *this),
        ...
        ckm_factor_ds(model->ckm_ud() * std::conj(model->ckm_cd()) + model->ckm_us() * std::conj(model->ckm_cs()))
    {
        ...
        if (cp_conjugate)
        {
            ckm_factor_ds = std::conj(ckm_factor_ds);
        }
    ...
    const std::map<std::tuple<QuarkFlavor, std::string>, std::tuple<std::string, std::string, std::string, std::string, double>>
            DToPseudoscalarLeptonLepton::AmplitudeGenerator::process_map{
                {     { QuarkFlavor::up, "pi" }, { "D_u->pi", "D->pi", "D_u", "pi^0", 1.0 / std::sqrt(2.0) } },
                {   { QuarkFlavor::down, "pi" },                  { "D_d->pi", "D->pi", "D_d", "pi^+", 1.0 } },
                { { QuarkFlavor::strange, "K" },                   { "D_s->K", "D_s->K", "D_s", "K_u", 1.0 } },
    };
    ...
    // std::complex<double> ckm_factor_ds = model->ckm_ud() * std::conj(model->ckm_cd()) + model->ckm_us() * std::conj(model->ckm_cs());
    // if (cp_conjugate)
    //     {
    //         ckm_factor_ds = std::conj(ckm_factor_ds);
    //     };
    ...
    DToPseudoscalarLeptonLepton::AmplitudeGenerator::normalisation(const double & q2) const
    {
        ...
        return power_of<2>(this->isospin_factor) * power_of<2>(g_fermi * alpha_e() * abs(ckm_factor_ds)) * sqrt(lambda(q2)) * beta_l(q2) * xi_pseudo(q2) * xi_pseudo(q2)
               / (512.0 * power_of<5>(M_PI) * power_of<3>(m_D()));
    }
} // namespace eos

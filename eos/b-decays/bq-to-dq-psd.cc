...
namespace eos
{
     ...
     */
    template <> struct Implementation<BqToDqPseudoscalar>
    {
            ...
            a_1() const
            {
                ...
                const complex<double> a_1_lo =
                        ...
                        + ((wc.c5() - wc.c5p()) * (-1.0 + mu_L / (mb - mc))) / 6.0 + (2.0 * (wc.c6() - wc.c6p()) * (-1.0 + mu_L / (mb - mc))) / 9.0
                ...
                const complex<double> a_1_nlp = ((wc.c1() - wc.c1p()) * TVLL_nlp) / 3.0 - ((wc.c2() - wc.c2p()) * TVLL_nlp) / 18.0 + (16.0 * (wc.c3() - wc.c3p()) * TVLL_nlp) / 3.0
            ...
            decay_width() const
            {
                ...
                const complex<double> amplitude        = g_fermi() / sqrt(2.0) * ckm_factor() * f_P() * ff_f_0->evaluate() * (m_B * m_B - m_D * m_D) * this->a_1();
    ...
    const std::vector<OptionSpecification> Implementation<BqToDqPseudoscalar>::options{
    ...
    const std::set<ReferenceName> BqToDqPseudoscalar::references{ "BBNS:2000A"_rn };

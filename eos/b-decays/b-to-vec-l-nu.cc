...
namespace eos
{
    ...
    template <> struct Implementation<BToVectorLeptonNeutrino>
    {
            ...
            std::array<double, 12>
            _differential_angular_observables(const double & q2) const
            {
            ...
            std::array<double, 12>
            _integrated_angular_observables(const double & q2_min, const double & q2_max) const
            {
            ...
            inline b_to_vec_l_nu::AngularObservables
            differential_angular_observables(const double & q2) const
            {
                return b_to_vec_l_nu::AngularObservables{ _differential_angular_observables(q2) };
            }

            inline b_to_vec_l_nu::AngularObservables
            integrated_angular_observables(const double & q2_min, const double & q2_max) const
            {
            ...
            double
            normalized_decay_width(const double & q2) const
            {
            ...
            double
            differential_pdf_l(const double & c_theta_l, const double & q2_min, const double & q2_max)
            {
            ...
            double
            differential_pdf_v(const double & c_theta_v, const double & q2_min, const double & q2_max)
            {
                b_to_vec_l_nu::AngularObservables ao = integrated_angular_observables(q2_min, q2_max);
            ...
            double
            differential_pdf_phi(const double & phi, const double & q2_min, const double & q2_max)
            {
                b_to_vec_l_nu::AngularObservables ao = integrated_angular_observables(q2_min, q2_max);
            ...
            double
            integrated_pdf_l(const double & c_theta_l_min, const double & c_theta_l_max, const double & q2_min, const double & q2_max)
            {
                b_to_vec_l_nu::AngularObservables ao = integrated_angular_observables(q2_min, q2_max);
                ...
                const double integral = 0.5 * (c_theta_l_max - c_theta_l_min) + a_fb * 0.5 * (c_theta_l_max * c_theta_l_max - c_theta_l_min * c_theta_l_min)
            ...
            double
            integrated_pdf_v(const double & c_theta_v_min, const double & c_theta_v_max, const double & q2_min, const double & q2_max)
            {
                b_to_vec_l_nu::AngularObservables ao = integrated_angular_observables(q2_min, q2_max);
                ...
                const double integral =
                        3.0 / 4.0 * ((1.0 - f_L) * (c_theta_v_max - c_theta_v_min) - (1.0 - 3.0 * f_L) * (power_of<3>(c_theta_v_max) - power_of<3>(c_theta_v_min)) / 3.0);
            ...
            double
            integrated_pdf_phi(const double & phi_min, const double & phi_max, const double & q2_min, const double & q2_max)
            {
                b_to_vec_l_nu::AngularObservables ao = integrated_angular_observables(q2_min, q2_max);
                ...
                const double integral = (0.5 * (phi_max - phi_min) + 1.0 / 3.0 * J3 * (std::sin(2.0 * phi_max) - std::sin(2.0 * phi_min))
    ...
    const std::vector<OptionSpecification> Implementation<BToVectorLeptonNeutrino>::options{
        Model::option_specification(),
        FormFactorFactory<PToV>::option_specification(),
        { "V"_ok, { "D^*"_ov, "D_s^*"_ov, "rho"_ov, "omega"_ov, "K^*"_ov, "J/psi"_ov } },
    ...
    BToVectorLeptonNeutrino::normalized_four_differential_decay_width(const double & q2, const double & c_theta_l, const double & c_theta_d, const double & phi) const
    {
        ...
        double result =
                9.0 / 32.0 / M_PI
                * ((a_o.vv10() + a_o.vv20() * c_2_theta_l + a_o.vv30() * c_theta_l) * c_theta_d_2 + (a_o.vv1T() + a_o.vv2T() * c_2_theta_l + a_o.vv3T() * c_theta_l) * s_theta_d_2
                   + a_o.vv4T() * s_theta_d_2 * s_theta_l_2 * c_2_phi + a_o.vv10T() * s_2_theta_d * s_2_theta_l * c_phi + a_o.vv20T() * s_2_theta_d * s_theta_l * c_phi
                   + a_o.vv5T() * s_theta_d_2 * s_theta_l_2 * s_2_phi + a_o.vv30T() * s_2_theta_d * s_theta_l * s_phi + a_o.vv40T() * s_2_theta_d * s_2_theta_l * s_phi);

        return result;
    }

    const std::string BToVectorLeptonNeutrino::description = "\
    The decay B->D^* l nu, where l=e,mu,tau is a lepton.";

    const std::string BToVectorLeptonNeutrino::kinematics_description_q2 = "\
    The invariant mass of the l-nubar pair in GeV^2.";

    const std::string BToVectorLeptonNeutrino::kinematics_description_c_theta_l = "\
    The cosine of the charged lepton's helicity angle theta_l in the l-nubar rest frame.";

    const std::string BToVectorLeptonNeutrino::kinematics_description_c_theta_d = "\

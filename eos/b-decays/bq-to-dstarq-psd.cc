...
namespace eos
{
     ...
     */
    template <> struct Implementation<BqToDstarqPseudoscalar>
    {
            ...
            complex<double>
            AVLL(const double & u, const double & z) const
            {
                // evaluate AVLL chosing the correct branch for z = m_c / m_b < 1 and z = - m_c / m_b > -1
                if ((-1.0 < z) && (z < 1.0))
                {
                    return log(u * (1.0 - z * z)) / (1.0 - u * (1.0 - z * z)) - pow(log(u * (1.0 - z * z)), 2) - dilog(1.0 - u * (1.0 - z * z));
                }
                // evaluate AVLL chosing the correct branch for z = m_b / m_c > 1 and z = - m_b / m_c < -1
                else if ((z > 1.0) || (z < -1.0))
                {
                    return (log(-(u * (1.0 - z * z))) - 1.0i * M_PI) / (1.0 - u * (1.0 - z * z)) + pow(log(1.0 - u * (1.0 - (z * z))), 2) / 2.0
                           - pow(log(-(u * (1.0 - z * z))) - 1.0i * M_PI, 2) + dilog(1.0 / (1.0 - u * (1.0 - z * z))) + -1.0 / 3.0 * M_PI * M_PI
                           - 1.0i * M_PI * log(1.0 - u * (1.0 - z * z));
                }
            ...
            complex<double>
            fVLL(const double & u, const double & z) const
            {
                // evaluate fVLL chosing the correct branch for z = m_c / m_b < 1 and z = - m_c / m_b > -1
                if ((-1.0 < z) && (z < 1.0))
                {
                    return 2.0 * (AVLL(u, z) - AVLL(1.0 - u, z)) - (z / (1.0 - u * (1.0 - z * z)))
            ...
            complex<double>
            ASLR(const double & u, const double & z) const
            {
                // evaluate ASLR chosing the correct branch for z = m_c / m_b < 1 and z = - m_c / m_b > -1
                if ((-1.0 < z) && (z < 1.0))
                {
                    return z * z / (pow(1.0 + z, 2) * (1.0 - u * (1.0 - z * z)))
                ...
                if ((z > 1.0) || (z < -1.0))
                {
                    return z * z / (pow(1.0 + z, 2) * (1.0 - u * (1.0 - z * z)))
                           ...
                           + 2.0
                                     * (-1.0 / 3.0 * M_PI * M_PI + (2.0 * (-1.0i * M_PI + log(-(u * (1.0 - z * z))))) / (1.0 - u * (1.0 - z * z))
            ...
            complex<double>
            fSLR(const double & u, const double & z) const
            {
                return ASLR(u, z) - ASLR(1.0 - u, z);
            }

            complex<double>
            ATLL(const double & u, const double & z) const
            {
                // evaluate ATLL chosing the correct branch for z = m_c / m_b < 1 and z = - m_c / m_b > -1
                if ((-1.0 < z) && (z < 1.0))
                {
                    return ((-1.0 + u * (2.0 - u - 2.0 * z + (-2.0 + u) * z * z)) * log(u * (1.0 - z * z))) / (1.0 - u * (1.0 - z * z))
                ...
                {
                    return ((-1.0 + u * (2.0 - u - 2.0 * z + (-2.0 + u) * z * z)) * (-1.0i * M_PI + log(-(u * (1.0 - z * z))))) / (1.0 - u * (1.0 - z * z))
                           + (1.0 - 2.0 * u)
                                     * (M_PI * M_PI / 3.0 + pow(-1.0i * M_PI + log(-(u * (1.0 - z * z))), 2) + 1.0i * M_PI * log(1.0 - u * (1.0 - z * z))
            ...
            complex<double>
            fTLL(const double & u, const double & z) const
            {
                return -((8.0 * (4.0 * u + 3.0)) / (1.0 + z)) + (8.0 * (1.0 - z)) / (1.0 + z) * (ATLL(u, z) + ATLL(1.0 - u, z));
            }
            ...
            a_1() const
            {
                ...
                const complex<double> a_1_lo =
                        (wc.c1() + wc.c1p()) * (-1.0 / 3.0 + (2.0 * mu_L) / (3.0 * (mb + mc))) + (wc.c2() + wc.c2p()) * (-4.0 / 9.0 + (8.0 * mu_L) / (9.0 * (mb + mc)))
                        + (8.0 * (wc.c3() + wc.c3p()) * (-2.0 + mu_L / (mb + mc))) / 3.0 + (32.0 * (wc.c4() + wc.c4p()) * (-2.0 + mu_L / (mb + mc))) / 9.0
                        - ((wc.c5() + wc.c5p()) * (mb + mc + mu_L)) / (6.0 * (mb + mc)) - (2.0 * (wc.c6() + wc.c6p()) * (mb + mc + mu_L)) / (9.0 * (mb + mc))
                        + (-2.0 * (wc.c7() + wc.c7p()) * mu_L) / (mb + mc) - (8.0 * (wc.c8() + wc.c8p()) * mu_L) / (3.0 * (mb + mc))
                        + (8.0 * (wc.c9() + wc.c9p()) * (-1.0 + (8.0 * mu_L) / (mb + mc))) / 3.0 + (32.0 * (wc.c10() + wc.c10p()) * (-1.0 + (8.0 * mu_L) / (mb + mc))) / 9.0;
                ...
                auto a_1_nlo_integrand = [&](const double & u) -> complex<double>
                {
                    ...
                    {
                        TVLL = (-18.0 - 6.0 * 2.0 * log(mu() / mb) + fVLL(1.0 - u, -1.0 / z) + fVLL(u, -z) + (3.0 + 2.0 * log(u / (1.0 - u))) * log(z * z))
                               * this->lcdas->phi(u, mu());
                    };
                    ...
                    {
                        TVLR = (6.0 + 6.0 * 2.0 * log(mu() / mb) - (3.0 + 2.0 * log((1.0 - u) / u)) * log(z * z) - fVLL((1.0 - u), -z) - fVLL(u, -1.0 / z))
                               * this->lcdas->phi(u, mu());
                    };
                    ...
                    {
                        TTLL = -48.0 * 2.0 * log(mu() / mb) + 8.0 * (3.0 + ((u - (1.0 - u)) * (1.0 + z)) / (1.0 - z) * log(u / (1.0 - u))) * log(z * z) + fTLL(u, -z)
                               + fTLL(1.0 - u, -1.0 / z);
                    };
                    return (4.0 * (wc.c1() + wc.c1p()) * (4.0 - TVLL + (2.0 * (4.0 + TSLR) * mu_L) / (mb + mc))) / 9.0
                ...
                const complex<double> a_1_nlp = -(4.0 * (wc.c1() + wc.c1p()) * TVLL_nlp) / 3.0 + (2.0 * (wc.c2() + wc.c2p()) * TVLL_nlp) / 9.0
                                                - (64.0 * (wc.c3() + wc.c3p()) * TVLL_nlp) / 3.0 + (32.0 * (wc.c4() + wc.c4p()) * TVLL_nlp) / 9.0
            ...
            decay_width() const
            {
                ...
                const complex<double> amplitude =
                        g_fermi() / sqrt(2.0) * ckm_factor() * f_P() * ff_a_0->evaluate() * sqrt(lambda(m_B * m_B, m_Dstar * m_Dstar, m_P * m_P)) * this->a_1();
    ...
    const std::vector<OptionSpecification> Implementation<BqToDstarqPseudoscalar>::options{
    ...
    const std::set<ReferenceName> BqToDstarqPseudoscalar::references{ "BBNS:2000A"_rn };

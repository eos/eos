...
namespace eos
{
    ...
    const std::vector<OptionSpecification> Implementation<BToPiLeptonInclusiveNeutrinos>::options{
    ...
    double
    BToPiLeptonInclusiveNeutrinos::differential_decay_width_3nu(const double & s, const double & snunubar, const double & z, const double & phi, const double & zst) const
    {
...
numerical implementations for this process follow [BIvD:2016A].";
    ...
    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_s = "\
The invariant mass of the tau-nubar_tau pair in GeV^2.";

    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_snunubar = "\
The invariant mass of the nu_tau-nubar_mu pair in GeV^2.";

    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_c_theta = "\
The cosine of the charged lepton l's helicity angle theta in the l-nubar_l rest frame.";

    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_c_theta_tau = "\
The cosine of the tau's helicity angle theta_tau in the tau-nubar_tau rest frame.";

    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_c_theta_mu_star = "\
The cosine of the muon's helicity angle theta_mu^* in the tau's rest frame.";

    const std::string BToPiLeptonInclusiveNeutrinos::kinematics_description_phi = "\
The azimuthal angle between the B-D plane and the tau-nubar_tau plane.";

    const std::set<ReferenceName> BToPiLeptonInclusiveNeutrinos::references{};

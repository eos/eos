...
namespace eos
{
    ...
    template <> struct Implementation<BToPiPiLeptonNeutrino>
    {
            ...
            double
            normalized_differential_decay_width(const double & q2, const double & k2, const double & z) const
            {
                const double m_B = this->m_B(), m_B2 = m_B * m_B, m_B3 = m_B * m_B2;
                const double lambda = eos::lambda(m_B2, q2, k2), sqrt_lambda = sqrt(lambda);
                const double m_l2   = m_l() * m_l();
                const double beta_l = 1.0 - m_l2 / q2;
                const double norm   = power_of<2>(g_fermi()) * beta_l * q2 * sqrt_lambda / (3072.0 * power_of<5>(M_PI) * m_B3);
                ...
                return norm * beta_l / 4.0
                       * ((3.0 - beta_l) * std::norm(F_long) + (1.0 - z * z) * (3.0 - beta_l) * (std::norm(F_perp) + std::norm(F_para)) + 3.0 * m_l2 * std::norm(F_time));
            }

            double
            differential_branching_ratio(const double & q2, const double & k2, const double & z) const
            {
                return differential_decay_width(q2, k2, z) * tau_B / hbar;
            }

            double
            differential_decay_width(const double & q2, const double & k2, const double & z) const
            {
                return normalized_differential_decay_width(q2, k2, z) * std::norm(model->ckm_ub());
            }

            static double
            normalized_differential_decay_width_gsl_adapter(double * parameters, size_t dim, void * _imp)
            {
                if (dim != 3u)
                {
            ...
            double
            normalized_integrated_decay_width(const double & q2min, const double & q2max, const double k2min, const double & k2max, const double & zmin, const double & zmax) const
            {
            ...
            double
            normalized_integrated_forward_backward_asymmetry(const double & q2min, const double & q2max, const double k2min, const double & k2max) const
            {
                // Yields a numerical error of approximately 0.2%.
                static const size_t calls = 50000;
    ...
    const std::vector<OptionSpecification> Implementation<BToPiPiLeptonNeutrino>::options{
    ...
    BToPiPiLeptonNeutrino::double_differential_branching_ratio(const double & q2, const double & k2) const
    {
        std::function<double(const double &)> integrand =
                std::bind(&Implementation<BToPiPiLeptonNeutrino>::differential_branching_ratio, _imp.get(), q2, k2, std::placeholders::_1);
    ...
    {
        std::function<double(const double &)> integrand =
                std::bind(&Implementation<BToPiPiLeptonNeutrino>::normalized_differential_decay_width, _imp.get(), q2, k2, std::placeholders::_1);
    ...
    BToPiPiLeptonNeutrino::partial_waves(const double & q2, const double & k2, const double & z) const
    {
        ...
        return integrand(z) / integrate<GSL::QNG>(integrand, -1.0, +1.0);
    }

    double
    BToPiPiLeptonNeutrino::integrated_branching_ratio(const double & q2min, const double & q2max, const double & k2min, const double & k2max, const double & zmin,
                                                      const double & zmax) const
    {
        return _imp->normalized_integrated_decay_width(q2min, q2max, k2min, k2max, zmin, zmax) * std::norm(_imp->model->ckm_ub()) * _imp->tau_B / _imp->hbar;
    }

    double
    BToPiPiLeptonNeutrino::integrated_forward_backward_asymmetry(const double & q2min, const double & q2max, const double & k2min, const double & k2max) const
    {
        return _imp->normalized_integrated_forward_backward_asymmetry(q2min, q2max, k2min, k2max);
    }

    const std::string BToPiPiLeptonNeutrino::description = "\
The decay B->pi pi l nubar, where l=e,mu is a light lepton, see e.g. [FFKMvD:2013A].";

    const std::string BToPiPiLeptonNeutrino::kinematics_description_q2 = "\
The invariant mass of the l-nubar pair in GeV^2.";

    const std::string BToPiPiLeptonNeutrino::kinematics_description_k2 = "\
The invariant mass of the pi-pi pair in GeV^2.";

    const std::string BToPiPiLeptonNeutrino::kinematics_description_z = "\
The cosine of the pion helicity angle in the pi-pi rest frame.";

    const std::set<ReferenceName> BToPiPiLeptonNeutrino::references{};

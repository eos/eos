...
namespace eos
{
    ...
    struct Implementation<BToDLeptonInclusiveNeutrinos>
    {
        ...
        double differential_decay_width_3nu(const double & s) const
        {
            ...
            return sqrt(lam) * v2 * ((3.0 - v) * fp * fp * lam + 3.0 * (1.0 - v) * f0 * f0 * power_of<2>(m_B() * m_B() - m_D() * m_D()));
        }
    ...
    double
    BToDLeptonInclusiveNeutrinos::normalized_differential_decay_width_1nu(const double & s, const double & c_theta_mu) const
    {
        return _imp->normalized_differential_decay_width_1nu(s, c_theta_mu);
    }

    double
    BToDLeptonInclusiveNeutrinos::normalized_differential_decay_width_3nu(const double & s, const double & snunubar,
            const double & z, const double & phi, const double & zst) const
    {

...
namespace eos
{
    ...
    template <> struct Implementation<BToDLeptonInclusiveNeutrinos>
    {
            ...
            Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
                form_factors(FormFactorFactory<PToP>::create("B->D::" + o.get("form-factors"_ok, "SSE"_ov).str(), p, o)),
            ...
            double
            differential_decay_width_1nu_1var(const double & s) const
            {
            ...
            double
            differential_decay_width_1nu(const double & s, const double & c_theta_mu) const
            {
                double fp  = form_factors->f_p(s);
                double lam = lambda(m_B * m_B, m_D * m_D, s);
            ...
            double
            differential_decay_width_3nu_1var(const double & s) const
            {
                double fp    = form_factors->f_p(s);
            ...
            double
            differential_decay_width_3nu(const double & s, const double & snunubar, const double & z, const double & phi, const double & zst) const
            {
                ...
                const double b = 2.0 * power_of<2>((mtau2 - s) * (mtau2 - snunubar)) * lam * f0 * fp * (mB2 - mD2) / (mtau6 * M_PI * s3)
                                 * (+(mtau2 + 2.0 * snunubar) - (mtau2 - 2.0 * snunubar) * zst);

                // multiplying z^2
                const double c = power_of<2>((mtau2 - s) * (mtau2 - snunubar)) * lam * sqrtlam * fp2 / (mtau8 * M_PI * s3)
                                 * (+(mtau2 + 2.0 * snunubar) * (mtau2 - s) - (mtau2 - 2.0 * snunubar) * (mtau2 + s) * zst);
    ...
    const std::vector<OptionSpecification> Implementation<BToDLeptonInclusiveNeutrinos>::options{
    ...
    double
    BToDLeptonInclusiveNeutrinos::differential_decay_width_3nu(const double & s, const double & snunubar, const double & z, const double & phi, const double & zst) const
    {
...
numerical implementations for this process follow [BIvD:2016A].";
    ...
    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_s = "\
The invariant mass of the tau-nubar_tau pair in GeV^2.";

    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_snunubar = "\
The invariant mass of the nu_tau-nubar_mu pair in GeV^2.";

    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_c_theta = "\
The cosine of the charged lepton l's helicity angle theta in the l-nubar_l rest frame.";

    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_c_theta_tau = "\
The cosine of the tau's helicity angle theta_tau in the tau-nubar_tau rest frame.";

    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_c_theta_mu_star = "\
The cosine of the muon's helicity angle theta_mu^* in the tau's rest frame.";

    const std::string BToDLeptonInclusiveNeutrinos::kinematics_description_phi = "\
The azimuthal angle between the B-D plane and the tau-nubar_tau plane.";

    const std::set<ReferenceName> BToDLeptonInclusiveNeutrinos::references{};

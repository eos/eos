...
namespace eos
{
    namespace btovlnu
    {
        ...
        inline double
        decay_width(const AngularCoefficients & a_c)
        {
        ...
        angular_coefficients_array(const Amplitudes & a)
        {
            // cf. [BHvD:2010A], p. 26, eqs. (A1)-(A11)
            std::array<double, 12> result;

            // j1s
            result[0]  = 3.0 / 16.0 * (3.0 * (norm(a.a_perp_left) + norm(a.a_para_left)) + 16.0 * (norm(a.a_longpara) + norm(a.a_timeperp)));
            // j1c
            result[1]  = 3.0 / 4.0 * (norm(a.a_long_left) + 2.0 * norm(a.a_time_left) + 8.0 * norm(a.a_paraperp));
            // j2s
            result[2]  = 3.0 / 16.0 * (norm(a.a_perp_left) + norm(a.a_para_left) - 16.0 * norm(a.a_longpara) - 16.0 * norm(a.a_timeperp));
            // j2c
            result[3]  = -3.0 / 4.0 * (norm(a.a_long_left) - 8.0 * norm(a.a_paraperp));
            // j3
            result[4]  = 3.0 / 8.0 * (norm(a.a_perp_left) - norm(a.a_para_left) + 16.0 * norm(a.a_longpara) - 16.0 * norm(a.a_timeperp));
            // j4
            result[5]  = 3.0 / (4.0 * sqrt(2.0)) * real(a.a_long_left * conj(a.a_para_left) - 8.0 * sqrt(2.0) * a.a_paraperp * conj(a.a_longpara));
            // j5
            result[6]  = 3.0 * sqrt(2.0) / 4.0 * real(a.a_long_left * conj(a.a_perp_left) + 2.0 * sqrt(2.0) * a.a_longpara * conj(a.a_time_left));
            // j6s
            result[7]  = 3.0 / 2.0 * real(a.a_para_left * conj(a.a_perp_left));
            // j6c
            result[8]  = -6.0 * real(a.a_paraperp * conj(a.a_time_left));
            // j7
            result[9]  = 3.0 * sqrt(2.0) / 4.0 * imag(a.a_long_left * conj(a.a_para_left) - 2.0 * sqrt(2.0) * a.a_timeperp * a.a_time_left);
            // j8

...
namespace eos
{
    ...
    /*
     * Decay: B_q -> D*_q V, cf. [BBNS:2000A] (class I only, V = rho^- or Kstar^-)
     */
    template <>
    struct Implementation<BqToDstarqVector>
    {
        ...
        {
            Context ctx("When constructing B_q->Dstar_q V observable");
            ...
            switch (opt_q.value())
            {
                case QuarkFlavor::strange:
                    ...
                    ff_a_0     = std::make_shared<UsedParameter>(p["B_s->D_s^*rho::a_0(Mrho2)"], u);
                ...
                case QuarkFlavor::down:
                    ...
                    ff_a_0     = std::make_shared<UsedParameter>(p["B->D^*K^*::a_0(MKstar2)"], u);
        ...
        {
            // evaluate ASLL chosing the correct branch for z = m_c / m_b < 1 and z = - m_c / m_b > -1
            if ((-1.0 < z) && (z < 1.0))
            {
                return
            ...
            // evaluate ASLL chosing the correct branch for z = m_b / m_c > 1 and z = - m_b / m_c < -1
            if ((z > 1.0) || (z < -1.0))
            {
                return
        ...
        complex<double> a_1() const
        {
            ...
            auto im_integrand_TLL = [this, & integrand_TLL](const double & u) -> const double
            {
                return imag(integrand_TLL(u));
            };
            ...
            const double TTLL_im = integrate<GSL::QAGS>(im_integrand_TLL, 0.0, 1.0);
            ...
            const complex<double> a_1_nlo =
                    4.0 / 9.0 * (wc.c1() - wc.c1p()) * (-4.0 - (2.0 * mu_L * TSLR) / (mb + mc) + TVLL) -
                    2.0 / 27.0 * (wc.c2() - wc.c2p()) * (-2.0 * mu_L * TSLR + (mb + mc) * (14.0 + TVLL)) / (mb + mc) +
                    32.0 / 9.0 * (wc.c3() - wc.c3p()) * (-(mu_L * TSLR) + 2.0 * (mb + mc) * (5.0 + TVLL)) / (mb + mc) -
                    16.0 / 27.0 * (wc.c4() - wc.c4p()) * (19.0 * (mb + mc) - mu_L * TSLR + 2.0 * (mb + mc) * TVLL) / (mb + mc) +
                    1.0 / 18.0 * (wc.c5() - wc.c5p()) * (mu_L * (4.0 * TSLL - TTLL) + 4.0 * (mb + mc) * (6.0 + TVLR)) / (mb + mc) +
                    1.0 / 108.0 * (wc.c6() - wc.c6p()) * (84.0 * (mb + mc) - 4.0 * mu_L * TSLL + mu_L * TTLL - 4.0 * (mb + mc) * TVLR) / (mb + mc) +
                    2.0 / 9.0 * (wc.c7() - wc.c7p()) * (16.0 * (mb + mc) + mu_L * (12.0 * TSLL + TTLL)) / (mb + mc) +
                    1.0 / 27.0 * (wc.c8() - wc.c8p()) * (56.0 * (mb + mc) - mu_L * (12.0 * TSLL + TTLL)) / (mb + mc) +
                    32.0 / 9.0 * (wc.c9() -wc.c9p()) * (-2.0 * mu_L * (4.0 * TSLL + TTLL) + (mb + mc) * (-40.0 + TVLR))/ (mb + mc) -
                    16.0 / 27.0 * (wc.c10() - wc.c10p()) * (-2.0 * mu_L * (4.0 * TSLL + TTLL) + (mb + mc) * (-76.0 + TVLR)) / (mb + mc);
        ...
        double decay_width() const
        {
            ...
            const complex<double> amplitude = g_fermi() / sqrt(2.0) * ckm_factor() * f_V() * ff_a_0->evaluate()
                * (m_B * m_B - m_Dstar * m_Dstar) * this->a_1();
            // cf. [BBNS:2000A], eq. (216), p. 80
            const double breakup_momentum = sqrt(lambda(m_B * m_B, m_Dstar * m_Dstar, m_V * m_V)) / (2.0 * m_B);
    ...
    Implementation<BqToDstarqVector>::options
    {
    ...
    BqToDstarqVector::BqToDstarqVector(const Parameters & parameters, const Options & options) :
        PrivateImplementationPattern<BqToDstarqVector>(new Implementation<BqToDstarqVector>(parameters, options, *this))
    {
    }

    BqToDstarqVector::~BqToDstarqVector()
    {
    }

    double
    BqToDstarqVector::branching_ratio() const
    {
        return _imp->branching_ratio();
    }

    double
    BqToDstarqVector::decay_width() const
    {
        return _imp->decay_width();
    }

    double
    BqToDstarqVector::re_a_1() const
    {
        return real(_imp->a_1());
    }

    double
    BqToDstarqVector::im_a_1() const
    {
        return imag(_imp->a_1());
    }

    const std::set<ReferenceName>
    BqToDstarqVector::references
    {
        "BBNS:2000A"_rn
    };

    std::vector<OptionSpecification>::const_iterator
    BqToDstarqVector::begin_options()
    {
        return Implementation<BqToDstarqVector>::options.cbegin();
    }

    std::vector<OptionSpecification>::const_iterator
    BqToDstarqVector::end_options()
    {
        return Implementation<BqToDstarqVector>::options.cend();
    }
}

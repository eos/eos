...
namespace eos
{
    template <> struct Implementation<LambdaBToLambdaC2625LeptonNeutrino>
    {
            ...
            double
            F12T(const double & q2) const
            {
                return form_factors->f_time12_v(q2);
            }

            double
            F120(const double & q2) const
            {
                return form_factors->f_long12_v(q2);
            }

            double
            F12P(const double & q2) const
            {
                return form_factors->f_perp12_v(q2);
            }

            double
            F32P(const double & q2) const
            {
                return form_factors->f_perp32_v(q2);
            }

            double
            G12T(const double & q2) const
            {
                return form_factors->f_time12_a(q2);
            }

            double
            G120(const double & q2) const
            {
                return form_factors->f_long12_a(q2);
            }

            double
            G12P(const double & q2) const
            {
                return form_factors->f_perp12_a(q2);
            }

            double
            G32P(const double & q2) const
            {
                return form_factors->f_perp32_a(q2);
            }

            double
            s_plus(const double & q2) const
            {
                return power_of<2>(m_LambdaB + m_LambdaC2625) - q2;
            }

            double
            s_minus(const double & q2) const
            {
                return power_of<2>(m_LambdaB - m_LambdaC2625) - q2;
            }
            ...
            double
            normalized_differential_forward_backward_asymmetry(const double & q2) const
            {
                if ((q2 < m_l * m_l) || (lambda(q2) < 0.0))
                {
                    return 0.0;
            ...
            double
            normalized_double_differential_decay_width(const double & q2, const double & cos_theta_l) const
            {
                if ((q2 < m_l * m_l) || (lambda(q2) < 0.0))
                {
                    return 0.0;
            ...
            double
            double_differential_decay_width(const double & q2, const double & cos_theta_l) const
            {
            ...
            double
            differential_forward_backward_asymmetry(const double & q2) const
            {
                if ((q2 < m_l * m_l) || (lambda(q2) < 0.0))
                {
                    return 0.0;
            ...
            double
            double_differential_branching_ratio(const double & q2, const double & cos_theta_l) const
            {
                return double_differential_decay_width(q2, cos_theta_l) * tau_LambdaB / hbar;
            }

            double
            integrated_branching_ratio(const double & q2_min, const double & q2_max) const
            {
                std::function<double(const double &)> f =
                        std::bind(&Implementation<LambdaBToLambdaC2625LeptonNeutrino>::differential_branching_ratio, *this, std::placeholders::_1);

                return integrate<1, 1>(f, q2_min, q2_max, cub_conf);
            }

            double
            integrated_forward_backward_asymmetry(const double & q2_min, const double & q2_max) const
            {
                std::function<double(const double &)> numerator =
                        std::bind(&Implementation<LambdaBToLambdaC2625LeptonNeutrino>::normalized_differential_forward_backward_asymmetry, *this, std::placeholders::_1);
                std::function<double(const double &)> denominator =
                        std::bind(&Implementation<LambdaBToLambdaC2625LeptonNeutrino>::normalized_differential_decay_width, *this, std::placeholders::_1);

                return integrate<1, 1>(numerator, q2_min, q2_max, cub_conf) / integrate<1, 1>(denominator, q2_min, q2_max, cub_conf);
            }
    };

    const std::vector<OptionSpecification> Implementation<LambdaBToLambdaC2625LeptonNeutrino>::options{
    ...
    const std::string LambdaBToLambdaC2625LeptonNeutrino::description = "\
The decay Lambda_b -> Lambda_c(2625) l nu, where l=e,mu,tau is a lepton.";

    const std::string LambdaBToLambdaC2625LeptonNeutrino::kinematics_description_q2 = "\
The invariant mass of the l-nubar pair in GeV^2.";

    const std::string LambdaBToLambdaC2625LeptonNeutrino::kinematics_description_c_theta_l = "\
The cosine of the helicity angle between the direction of flight of the muon and of the Lambda_c(2625) in the l-nubar rest frame.";
    ...
    const std::set<ReferenceName> LambdaBToLambdaC2625LeptonNeutrino::references{};

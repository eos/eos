...
namespace eos
{
    template <> struct Implementation<LambdaBToLambdaC2595LeptonNeutrino>
    {
            ...
            {
                return form_factors->f_time_v(q2);
            }

            double
            F120(const double & q2) const
            {
                return form_factors->f_long_v(q2);
            }

            double
            F12P(const double & q2) const
            {
                return form_factors->f_perp_v(q2);
            }

            double
            G12T(const double & q2) const
            {
                return form_factors->f_time_a(q2);
            }

            double
            G120(const double & q2) const
            {
                return form_factors->f_long_a(q2);
            }

            double
            G12P(const double & q2) const
            {
                return form_factors->f_perp_a(q2);
            }

            double
            s_plus(const double & q2) const
            {
                return power_of<2>(m_LambdaB + m_LambdaC2595) - q2;
            }
            ...
            integrated_forward_backward_asymmetry(const double & q2_min, const double & q2_max) const
            {
                std::function<double(const double &)> numerator =
                        std::bind(&Implementation<LambdaBToLambdaC2595LeptonNeutrino>::normalized_differential_forward_backward_asymmetry, *this, std::placeholders::_1);
    ...
    const std::vector<OptionSpecification> Implementation<LambdaBToLambdaC2595LeptonNeutrino>::options{
    ...
    const std::string LambdaBToLambdaC2595LeptonNeutrino::description = "\
The decay Lambda_b -> Lambda_c(2595) l nu, where l=e,mu,tau is a lepton.";

    const std::string LambdaBToLambdaC2595LeptonNeutrino::kinematics_description_q2 = "\
The invariant mass of the l-nubar pair in GeV^2.";

    const std::string LambdaBToLambdaC2595LeptonNeutrino::kinematics_description_c_theta_l = "\
The cosine of the helicity angle between the direction of flight of the muon and of the Lambda_c(2595) in the l-nubar rest frame.";
    ...
    const std::set<ReferenceName> LambdaBToLambdaC2595LeptonNeutrino::references{};

...
namespace eos
{
    ...
    template <> struct Implementation<BToPPLeptonNeutrino>
    {
                ...
                form_factors(FormFactorFactory<PToPP>::create(_process() + "::" + o.get("form-factors"_ok, "HKvT2025"_ov).str(), p, o))
            {
                ...
                switch (opt_U.value())
                {
                    case QuarkFlavor::up: v_Ub = std::bind(&ModelComponent<components::CKM>::ckm_ub, model.get()); break;

                    default: throw InternalError("Unexpected quark flavor: '" + opt_U.str() + "'");
                }
            ...
            b_to_psd_psd_l_nu::Amplitudes
            amplitudes(const double & q2, const double & k2) const
            {
                ...
                if ((q2 >= power_of<2>(m_l)) && (q2 <= power_of<2>(m_B - m_P1 - m_P2)) && (k2 >= power_of<2>(m_P1 + m_P2)) && (k2 <= power_of<2>(m_B - std::sqrt(q2))))
                {
                    ...
                    result.pref    = std::norm(v_Ub()) * power_of<2>(g_fermi()) / power_of<3>(m_B) * q2 * power_of<2>(result.beta_l) * result.beta_pi * std::sqrt(lamq3)
                                  / power_of<5>(4.0 * M_PI) / 4.0;
                }
                else
                {
            ...
            std::array<std::array<double, 5>, 9>
            _differential_angular_observables(const double & q2, const double & k2) const
            {
                return b_to_psd_psd_l_nu::AngularObservables(this->amplitudes(q2, k2), this->ints)._M;
            }

            inline b_to_psd_psd_l_nu::AngularObservables
            differential_angular_observables(const double & q2, const double & k2) const
            {
                return b_to_psd_psd_l_nu::AngularObservables(_differential_angular_observables(q2, k2), this->ints);
            }
    };

    const std::vector<OptionSpecification> Implementation<BToPPLeptonNeutrino>::options{
    ...
    double
    BToPPLeptonNeutrino::double_differential_decay_width(const double & q2, const double & k2) const
    {
        return _imp->differential_angular_observables(q2, k2).double_differential_decay_width();
    }

    double
    BToPPLeptonNeutrino::double_differential_mesonic_afb(const double & q2, const double & k2) const
    {
        return _imp->differential_angular_observables(q2, k2).double_differential_mesonic_afb();
    }

    double
    BToPPLeptonNeutrino::double_differential_branching_ratio(const double & q2, const double & k2) const
    {
        return _imp->differential_angular_observables(q2, k2).double_differential_decay_width() * _imp->tau_B / _imp->hbar;
    }

    double
    BToPPLeptonNeutrino::integrated_branching_ratio(const double & q2_min, const double & q2_max, const double & sqrt_k2_min, const double & sqrt_k2_max) const
    {
        ...
        double res = integrate<2>(integrand, x_min, x_max, _imp->cub_conf);
        return res;
    }

    double
    BToPPLeptonNeutrino::integrated_mesonic_afb(const double & q2_min, const double & q2_max, const double & sqrt_k2_min, const double & sqrt_k2_max) const
    {
        std::function<double(const std::array<double, 2u> &)> integrand = [&](const std::array<double, 2u> & x)
        {
    ...
    double
    BToPPLeptonNeutrino::fully_integrated_branching_ratio() const
    {
        return integrated_branching_ratio(1e-4, power_of<2>(_imp->m_B - _imp->m_P1 - _imp->m_P2), _imp->m_P1 + _imp->m_P2, _imp->m_B);
    }

    double
    BToPPLeptonNeutrino::fully_integrated_mesonic_afb() const
    {
        return integrated_mesonic_afb(1e-4, power_of<2>(_imp->m_B - _imp->m_P1 - _imp->m_P2), _imp->m_P1 + _imp->m_P2, _imp->m_B);
    }

    double
    BToPPLeptonNeutrino::q2_integrated_branching_ratio(const double & sqrt_k2_min, const double & sqrt_k2_max) const
    {
        return integrated_branching_ratio(1e-4, power_of<2>(_imp->m_B - _imp->m_P1 - _imp->m_P2), sqrt_k2_min, sqrt_k2_max);
    }

    double
    BToPPLeptonNeutrino::q2_integrated_mesonic_afb(const double & sqrt_k2_min, const double & sqrt_k2_max) const
    {
        return integrated_mesonic_afb(1e-4, power_of<2>(_imp->m_B - _imp->m_P1 - _imp->m_P2), sqrt_k2_min, sqrt_k2_max);
    }

    double
    BToPPLeptonNeutrino::s_integrated_branching_ratio(const double & q2_min, const double & q2_max) const
    {
        return integrated_branching_ratio(q2_min, q2_max, _imp->m_P1 + _imp->m_P2, _imp->m_B);
    }

    double
    BToPPLeptonNeutrino::s_integrated_mesonic_afb(const double & q2_min, const double & q2_max) const
    {
        return integrated_mesonic_afb(q2_min, q2_max, _imp->m_P1 + _imp->m_P2, _imp->m_B);
    }

    double
    BToPPLeptonNeutrino::integrated_branching_ratio_q2(const double & q2) const
    {
        double k2_min = power_of<2>(_imp->m_P1 + _imp->m_P2);
        double k2_max = power_of<2>(_imp->m_B - std::sqrt(q2));

        std::function<double(const double &)> integrand = [&](const double & k2) { return this->double_differential_branching_ratio(q2, k2); };

        double res = integrate<1, 1>(integrand, k2_min, k2_max, _imp->cub_conf);
        return res;
    }

    double
    BToPPLeptonNeutrino::integrated_mesonic_afb_q2(const double & q2) const
    {
        double k2_min = power_of<2>(_imp->m_P1 + _imp->m_P2);
        double k2_max = power_of<2>(_imp->m_B - std::sqrt(q2));

        std::function<double(const double &)> integrand = [&](const double & k2) { return this->double_differential_mesonic_afb(q2, k2); };
    ...
    double
    BToPPLeptonNeutrino::integrated_branching_ratio_sqrt_k2(const double & sqrt_k2) const
    {
        double q2_min = 1e-4;
        double q2_max = power_of<2>(_imp->m_B - sqrt_k2);

        std::function<double(const double &)> integrand = [&](const double & q2) { return 2 * sqrt_k2 * this->double_differential_branching_ratio(q2, sqrt_k2 * sqrt_k2); };

        double res = integrate<1, 1>(integrand, q2_min, q2_max, _imp->cub_conf);
        return res;
    }

    double
    BToPPLeptonNeutrino::integrated_mesonic_afb_sqrt_k2(const double & sqrt_k2) const
    {
        double q2_min = 1e-4;
        double q2_max = power_of<2>(_imp->m_B - sqrt_k2);

        std::function<double(const double &)> integrand = [&](const double & q2) { return 2 * sqrt_k2 * this->double_differential_mesonic_afb(q2, sqrt_k2 * sqrt_k2); };

        double res = integrate<1, 1>(integrand, q2_min, q2_max, _imp->cub_conf);
        return res;
    }

    const std::string BToPPLeptonNeutrino::description = "\
    The decay B->D^* l nu, where l=e,mu,tau is a lepton.";

    const std::string BToPPLeptonNeutrino::kinematics_description_q2 = "\
    The invariant mass of the l-nubar pair in GeV^2.";

    const std::string BToPPLeptonNeutrino::kinematics_description_k2 = "\
    The invariant mass of the p-p pair in GeV^2.";

    const std::string BToPPLeptonNeutrino::kinematics_description_c_theta_l = "\
    The cosine of the charged lepton's helicity angle theta_l in the l-nubar rest frame.";

    const std::string BToPPLeptonNeutrino::kinematics_description_c_theta_nu = "\
    The cosine of the D's helicity angle theta_d in the D-pi rest frame.";

    const std::string BToPPLeptonNeutrino::kinematics_description_phi = "\
    The azimuthal angle between the D-pi plane and the l-nubar  plane.";

    const std::set<ReferenceName> BToPPLeptonNeutrino::references{ "HKvT:2025A"_rn };

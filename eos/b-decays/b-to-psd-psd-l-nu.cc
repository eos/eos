...
namespace eos
{
    ...
    b_to_psd_psd_l_nu::Amplitudes amplitudes(const double & q2, const double & k2) const
    {
            ...
            if ((q2 >= power_of<2>(m_l)) && (q2 <= power_of<2>(m_B - m_P1 - m_P2)) && (k2 >= power_of<2>(m_P1 + m_P2)) && (k2 <= power_of<2>(m_B - std::sqrt(q2)))) {
                ...
                result.q2 = q2;
                result.betaL = (m_l > 0.0) ? (1.0 - m_l * m_l / q2) : 1.0;
                result.betaPi = std::sqrt(lams12) / k2;
                result.pref = std::norm(v_Ub()) * power_of<2>(g_fermi()) / power_of<3>(m_B) * q2 * power_of<2>(result.betaL) * result.betaPi * std::sqrt(lamq3) / power_of<5>(4.0 * M_PI) / 4.0;
            }
            else
            {
    ...
    double BToPPLeptonNeutrino::integrated_branching_ratio_q2(const double & q2) const
    {
        ...
        double res = integrate1D(integrand, _imp->int_points, k2_min, k2_max);
        return res;
    }

...
bool is_pi(const double & value)
{
    return std::abs(value - M_PI) < 0.01;
}
...
main(int argc, char * argv[])
{
    try
    {
        ...
        double arg_c7 = CommandLine::instance()->arg_c7;
        std::cout << "# Realifying for arg_c7 = " << arg_c7 << std::endl;
        ...
        for (auto f = CommandLine::instance()->files.cbegin(), f_end = CommandLine::instance()->files.cend() ; f != f_end ; ++f)
        {
            try
            {
                ...
                for (auto d = file.begin(), d_end = file.end() ; d != d_end ; ++d)
                {
                    ...
                    {
                        if (std::abs(record[1] - arg_c7) > 0.01)
                            continue;

                        double x, y;
                        if (is_zero(record[x_index + 1]))
                            x = record[x_index];
                        else if (is_pi(record[x_index + 1]))
                            x = -1.0 * record[x_index];
                        else
                            continue;

                        if (is_zero(record[y_index + 1]))
                            y = record[y_index];
                        else if (is_pi(record[y_index + 1]))
                            y = -1.0 * record[y_index];
                        else
                            continue;
    ...
    catch(DoUsage & e)
    {
        ...
        std::cout << "Usage: eos-realify" << std::endl;
        std::cout << "  --x NAME MIN MAX COUNT" << std::endl;
        std::cout << "  --y NAME MIN MAX COUNT" << std::endl;
        std::cout << "  [--arg-c7 VALUE]" << std::endl;
        std::cout << "  [--file NAME]+" << std::endl;
        std::cout << std::endl;
        std::cout << "Example:" << std::endl;
        std::cout << "  eos-realify --x \"Abs{c9}\" 0.0 15.0 60 --y \"Arg{c9}\" 0.0 6.28 32 --file input1.hdf5 --file input2.hdf5" << std::endl;
    }
    catch(Exception & e)
    {

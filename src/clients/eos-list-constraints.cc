    ...
    public InstantiationPolicy<CommandLine, Singleton>
{
    public:
        ...
        {
            Log::instance()->set_program_name("eos-list-signal-pdfs");
...
main(int argc, char * argv[])
{
    try
    {
        ...
        for (auto c = constraints.begin(), c_end = constraints.end() ; c != c_end ; ++c)
        {
            std::cout
                << *c->second << "\n"
                << std::endl;
    ...
    catch(DoUsage & e)
    {
        ...
        std::cout << "Usage: eos-list-constraints" << std::endl;

        std::cout << "Print the database of implemented statistical constraints." << std::endl;
    }
    catch(Exception & e)
    {
